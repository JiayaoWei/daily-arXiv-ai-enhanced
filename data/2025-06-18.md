...
Motivation: 全球气候变化问题日益严峻，但现有研究在高效生成高精度气候指令数据方面不足，限制了气候专用大语言模型的发展。

Method: 通过从文档中提取事实和背景知识生成指令，结合网络爬取和种子指令收集增强数据多样性，构建ClimateChat-Corpus数据集，并微调开源大语言模型。

Result: ClimateChat在气候问答任务中表现显著提升，同时验证了不同基础模型和指令数据对性能的影响。

Conclusion: 研究为构建气候指令数据和训练专用大语言模型提供了重要参考，强调了选择合适基础模型的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs为何在跨语言数字系统的语言-数学谜题中表现不如人类。

Method: 通过实验分离语言和数学因素，分析数字构造与组合参数对性能的影响。

Result: LLMs需显式数学符号标记才能解决问题，缺乏对隐含数字结构的理解。

Conclusion: 当前推理模型在从隐含模式中灵活推断组合规则的能力仍有挑战。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言奖励模型（VL-RM）在强化微调（RFT）中对齐视觉语言模型至关重要，但面临自举困境和模态偏差的挑战，导致训练数据质量受限和偏好数据误导。

Method: 提出了一种迭代训练框架，利用视觉专家、链式思维（CoT）和基于边缘的拒绝采样，优化偏好数据集并增强结构化反馈。

Result: 实验表明，该方法在幻觉检测和多模态推理方面表现优异，显著提升了视觉语言模型的对齐能力。
...
Method: 利用基于大型语言模型（LLM）的情感分析器识别适当情感，并通过PromptTTS合成上下文适配的情感语音。

Result: 实验表明，所提出的情感SDS在情感调节和参与度上优于基线系统。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型进入高风险领域，现有评估方法（如拒绝率、毒性分类器等）存在盲点，无法可靠检测对齐问题。

Method: 引入AQI，通过几何和提示不变性指标分析潜在空间中安全与不安全激活的分离，结合多种聚类质量指标。同时提出LITMUS数据集支持评估。

Result: AQI能检测隐藏的对齐问题和越狱风险，并在不同训练条件下与外部评估结果相关。

Conclusion: AQI和LITMUS为对齐评估提供了更鲁棒的工具，有助于未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 在机器人辅助日常活动时，结合视觉和语言信息的多模态分类任务对意图理解至关重要，但大规模数据收集困难。

Method: 利用大型语言模型模拟对话和环境上下文，结合稳定扩散模型生成环境图像，通过增强数据优化多模态模型。

Result: 实验证明该方法显著提升了机器人动作选择能力，达到最先进性能。

Conclusion: 提出的数据增强框架有效解决了多模态数据不足的问题，提升了机器人意图理解的准确性。
...
<details>
  <summary>Details</summary>
Motivation: 法律研究中，法官对法律概念的扩展解释是重要内容，但传统方法依赖人工标注，成本高且需重复。

Method: 通过实验确定最优标注数量、标注句子选择方式（随机或优选），以及利用LLM自动化标注的效果。

Result: 实验结果显示，优选标注句子能提升模型性能，LLM自动化标注具有一定效果。

Conclusion: 自动化标注可以减少人工成本，优选标注句子能提高模型性能，LLM是可行的辅助工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究人类与AI情感对齐的必要性，以促进安全有效的人机协作。

Method: 通过跨语言文化群体和模型家族的比较，使用可解释的LLM特征评估情感对齐，并利用人类情感概念调控模型输出。

Result: LLM的情感空间与人类感知结构一致，且能通过情感概念稳定调控模型的情感表达。
...
<details>
  <summary>Details</summary>
Motivation: 代码切换在 multilingual 社区和在线内容中普遍存在，LLM 经常处理此类混合语言文本，因此需要了解其处理能力。

Method: 通过生成代码切换版本的推理和理解基准，评估LLM的表现。

Result: 外语词汇干扰会降低模型表现，但将英语嵌入其他语言可能提升理解；微调能稳定缓解性能下降。

Conclusion: LLM对代码切换文本的理解受语言干扰影响，微调是改善性能的有效途径。
...
Method: 提出MultiFinBen基准，包含多语言和多模态任务（如PolyFiQA-Easy、PolyFiQA-Expert、EnglishOCR、SpanishOCR），并采用动态难度选择机制。

Result: 对22个先进模型的评估显示，即使最强模型在复杂跨语言和多模态金融任务中表现不佳。
...
Method: 通过零样本学习、文化适应、结构化评估和交互式上下文分析常识推理；通过开放集模型、生成式方法、聚类和人本系统研究意图检测。

Result: 总结了适应性、多语言和上下文感知模型的新趋势，并指出了在基础、泛化和基准设计方面的关键差距。

Conclusion: 强调了未来研究需要关注模型的基础性、泛化能力和更全面的基准设计。
...
<details>
  <summary>Details</summary>
Motivation: 评估短对话文本的难度，特别是用于训练和过滤大型语言模型（LLMs）。
...
Result: 在手工标注的数据集GECS上评估了工具，并从开源语料库Dolma中提取了大型自然嵌入式从句数据集。

Conclusion: 该方法填补了当前研究的空白，提供了统计信息和自然例句，有助于语言学研究的深入。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在临床领域中的幻觉问题可能导致严重后果，自动生成出院摘要可以减轻医生负担，但需确保内容的可信度和来源。

Method: 结合语言图和深度学习模型，以解决内容来源和可信度问题。

Result: 在MIMIC-III公开数据集和匿名医院的临床笔记上展示了高可靠性。

Conclusion: 该方法为自动生成临床摘要提供了可信且可追踪的解决方案。
...
Method: 使用EAI-Distill-0.5b模型为数据集标注分类法标签，并通过SQL式过滤生成特定领域数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现代语言模型普遍使用分词，但分词可能导致生成失真（如Prompt Boundary Problem）和模型间不兼容（如分词器不匹配）。

Method: 在推理时转换BPE分词的模型为字符级或字节级模型，保持文本生成分布不变。

Result: 实验表明，集成和代理调优的模型在下游任务中优于其组成部分。

Conclusion: 该方法有效解决了分词失真和模型兼容性问题，提升了模型集成和调优的效果。
...
<details>
  <summary>Details</summary>
Motivation: 当前的大型语言模型（LLMs）驱动的深度研究系统缺乏处理地理时空约束的能力，这在公共健康、环境科学和社会经济分析等领域至关重要。

Method: 提出增强检索和合成过程的能力，以处理地理时空约束，并支持开放、可复现的基础设施和严格的评估协议。

Result: 提出了下一代系统的愿景，明确了整合地理时空推理到深度研究流程中的技术、基础设施和评估挑战。

Conclusion: 通过增强地理时空能力，未来的深度研究系统将更先进，对AI驱动的信息访问具有潜在影响。
...
Result: 实验表明，DCRM与学习效果正相关，且提出的配对方法能进一步提升模型性能。

Conclusion: DCRM是一种有效的偏好数据集质量评估指标，其配对方法能显著提升模型表现。
...
<details>
  <summary>Details</summary>
Motivation: 自回归大语言模型推理延迟高，现有推测采样方法未充分利用文本生成的连贯性。

Method: 提出S$^4$C框架，采用多头草稿生成和连续验证树优化推测采样。

Result: 在主流任务中表现优于基线方法，加速比达2.26x-2.60x。

Conclusion: S$^4$C在效率和并行性上显著提升，适合实时应用。
...
Method: 采用刻板印象内容模型（SCM），设计两种间接任务（WABT和AAT）评估能力、社交性和道德维度的偏差。

Result: 在8个先进LLM上的实验揭示了复杂的偏见结构，包括普遍社交性偏见和多维不对称性。
...
Method: 基于LLM的生成模型，先通过大规模无监督学习预训练，再用有监督学习微调，并引入标签平滑优化排序损失。

Result: 模型在响应排序、人类反馈强化学习和任务适应等多个任务中表现优异，显著优于基线模型。

Conclusion: 生成式奖励模型通过结合无监督和有监督学习，提供了一种更通用的奖励建模方法，适用于多种任务。
...
<details>
  <summary>Details</summary>
Motivation: 由于语码转换（CS）数据的稀缺性和高成本，研究旨在通过合成数据解决这一问题，推动多语言ASR的发展。

Method: 采用短语级混合方法生成合成CS数据，并结合大型预训练ASR模型（如Whisper、MMS、SeamlessM4T）进行微调。

Result: 实验表明，该方法显著提升了ASR在单语和CS测试中的性能，尤其在马来语-英语（BM-EN）上表现最佳。

Conclusion: 研究为CS-ASR提供了一种经济高效的开发方案，对学术和工业界具有重要价值。
...
Method: 采用三阶段方法：1) 用代码切换文本训练解码器自注意力层；2) 用有限语音文本数据对齐解码器和编码器；3) 完整微调模型。

Result: 在马来语-英语代码切换任务中，Whisper模型相对WER降低9.02%，同时提升单语性能。

Conclusion: AsyncSwitch通过高效利用文本数据，显著提升代码切换和多语言ASR性能，为数据稀缺问题提供解决方案。
...
Method: 提出MAS-LitEval，利用多智能体系统和大型语言模型评估翻译质量，测试了《小王子》和《康州美国佬在亚瑟王朝》的翻译。
...
<details>
  <summary>Details</summary>
Motivation: 探索语言模型是否能根据学习者的不同信息需求和知识背景定制回答，以提升其教育适用性。

Method: 引入ELI-Why基准，包含13.4K个“为什么”问题，并通过两项人类研究评估语言模型生成的解释对不同教育背景的适用性。

Result: GPT-4生成的解释仅50%匹配目标教育背景，而人工解释为79%；学习者认为GPT-4解释的适用性平均低20%。

Conclusion: 当前语言模型生成的解释在不同教育背景下的区分度不足，限制了其教学效果。
...
<details>
  <summary>Details</summary>
Motivation: 失语症患者在命名物品时面临术语缺失和语义错误（语义性错语）的挑战，需要一种方法帮助准确识别目标物品。

Method: 提出两种方法：1）通过梯度选择性增强来鲁棒化模型以应对语义错误；2）利用梯度方差引导包含未见但相关的术语。

Result: 在Tip-of-the-Tongue数据集和AphasiaBank真实患者数据上，模型表现优于基线方法。

Conclusion: 该方法有效解决了失语症患者的命名困难问题，为语言模型在医疗辅助中的应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 为通用计算机使用代理提供可扩展且成本高效的任务和数据集合成方法。

Method: 利用LLM生成任务，执行代理完成任务并记录轨迹，迭代形成子任务序列，最后合成可控难度的任务。

Result: 生成了6,000多个多样且真实的任务，LLM代理在难度1到6时成功率从18%降至4%，成本仅为每轨迹0.60美元。

Conclusion: AgentSynth能高效生成高质量任务，显著降低标注成本，为代理性能评估提供有力工具。
...
<details>
  <summary>Details</summary>
Motivation: 高质量数据获取困难，尤其是混合类型表格数据，因其异构数据类型和复杂变量关系而更具挑战性。
...
<details>
  <summary>Details</summary>
Motivation: 随着数字化时代的发展，恶意行为者转向使用隐性语言策略影响公众认知，而现有模型难以有效检测这些模式。

Method: 通过增强现有数据集，利用先进语言模型的推理能力，设计了一个改进的框架来检测隐性影响模式及其在对话中的具体位置。

Result: 该方法在隐性影响模式检测上提升了6%，在影响技术和受害者脆弱性的多标签分类任务上分别提升了33%和43%。

Conclusion: 提出的方法有效提升了隐性影响模式的检测能力，为对抗数字化时代的语言操纵提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖答案重叠作为标签，但可能因偶然相同的答案导致不可靠结果，需要更可靠的推理方法。

Method: 提出Timeline Reasoning Network（TRN），分两步推理：先基于语义和句法回答问题，再通过时间线链式推理修正答案。

Result: 在TORQUE和TB-dense等任务上，TRN优于现有方法，有效解决了答案重叠问题。

Conclusion: TRN通过时间线推理显著提升了时间关系理解的准确性，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前大语言模型在复杂推理中缺乏经验积累，而专家问题解决者则能利用丰富的经验。Xolver旨在填补这一差距，实现经验感知的语言代理。

Method: Xolver整合了外部和自我检索、工具使用、协作互动、代理驱动评估和迭代优化等多种经验模态，避免从零生成解决方案。

Result: Xolver在多个基准测试中表现优异，甚至超越了一些先进模型，如GSM8K（98.1%）、AIME'24（94.4%）等。

Conclusion: Xolver展示了通过整合经验学习实现专家级推理的潜力，是迈向通用代理的重要一步。
...
Result: 在GSM8K-XL、FuncQA、KAMEL和VirtualHome数据集上的实验表明，该方法在数值推理、知识问答和计划生成任务中优于基线模型。
...
Method: 利用时间常识知识图谱的无标签事件知识，通过大语言模型合成55K事件驱动对话，训练Timer对话代理预测时间间隔并生成及时响应。

Result: Timer在轮次和对话级别评估中均优于基于提示的大语言模型和其他微调基线。

Conclusion: 论文填补了时间条件对话响应生成的空白，并公开了数据、模型和代码。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型（LLMs）驱动的对话推荐代理（CRAs）存在短视响应问题，偏好优化成本高且在多轮对话中表现不佳。

Method: 引入多轮偏好优化（MTPO）范式ECPO，结合期望确认理论建模用户满意度演化，并通过LLM模拟器AILO生成用户反馈。

Result: 实验表明ECPO显著提升了CRAs的交互能力，在效率和效果上优于现有MTPO方法。

Conclusion: ECPO通过针对性优化不满意的响应，实现了低成本、高效的多轮偏好优化。
...
<details>
  <summary>Details</summary>
Motivation: 人类语言表达具有多样性，但摘要评估中常忽略这一点。尽管多参考摘要能提高与人类判断的相关性，但参考集选择对指标的影响尚未系统研究。

Method: 分析了三个多参考摘要数据集（SummEval、GUMSum、DUC2004），评估了流行指标的敏感性，并收集了人类对LLM输出的判断以补充现有发现。

Result: 许多指标（尤其是n-gram类如ROUGE）在不同参考集下表现不稳定，模型排名因参考集而异。人类判断与指标的相关性较弱或无。

Conclusion: 建议在摘要评估中纳入参考集多样性，以提高一致性和与人类判断的相关性，尤其是在评估LLM时。
...
Conclusion: Google的守门行为通过策划社交媒体叙事影响公共话语，揭示了算法对信息可见性的重要影响。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLMs可能导致浅层学习及削弱批判性思维的问题。

Method: 提出两步框架：先由Questioner生成候选问题，再由Judge筛选最相关问题。

Result: 该系统在共享任务竞赛中排名第一。

Conclusion: LLM方法有望促进对论证文本的批判性参与。
...
Method: Thunder-NUBench通过对比标准否定与结构多样的替代形式（如局部否定、矛盾和转述），设计了一个手动筛选的句子-否定对和多选题数据集。

Result: 该基准能够深入评估模型对否定的理解能力。

Conclusion: Thunder-NUBench填补了现有基准的空白，为评估大型语言模型的否定理解提供了专门工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有检索系统依赖浅层信号（如关键词重叠），而新基准主要关注查询端处理技术。ImpliRet旨在评估文档端推理能力。

Method: ImpliRet通过简单查询设计，但相关性依赖于文档中隐含的事实（如时间、算术和世界知识关系）。评估了稀疏和密集检索模型以及长上下文模型。

Result: 所有模型表现不佳，最佳nDCG@10仅15.07%。GPT-4.1在短上下文下得分35.06%，显示文档端推理仍具挑战性。

Conclusion: 文档端推理是检索系统的新挑战，ImpliRet为未来研究提供了基准。
...
Method: 通过比较扩散LLMs与传统自回归LLMs的长上下文性能，发现扩散LLMs的独特特性，并基于RoPE缩放理论提出LongLLaDA方法。
...
Method: 提出语义游戏作为评估测试时学习的工具，设计包含有限和累积经验设置的评估框架，并招募人类参与者作为基线。

Result: LLMs表现出测试时学习能力，但在累积经验下进步不稳定且速度慢于人类。

Conclusion: LLMs具备通用学习潜力，但与人类仍存在显著智能差距，静态基准表现不足以衡量其全面能力。
...
Method: 采用同义词替换高熵词的水印嵌入方法，增强模型对水印文本的记忆能力。

Result: 在多种训练设置下，LexiMark的AUROC分数显著优于现有方法。

Conclusion: LexiMark能有效验证LLM是否使用了未经授权的水印数据，隐蔽性和抗移除性强。
...
<details>
  <summary>Details</summary>
Motivation: MLLM在推理时需要大量计算资源，攻击者可通过诱导模型生成过多输出来耗尽资源。现有攻击方法未充分利用词性和句子结构对输出的影响，效果有限。

Method: 提出POS-Aware Delay Mechanism，基于词性调整注意力权重以延迟EOS生成；引入Generative Path Pruning Mechanism，限制隐藏状态幅度以诱导重复循环。

Result: 实验表明，LingoLoop可将生成令牌数增加30倍，能耗显著提升，成功将MLLM推向生成极限。

Conclusion: LingoLoop揭示了MLLM的严重漏洞，对其可靠部署提出挑战。代码将在论文接受后公开。
...
<details>
  <summary>Details</summary>
Motivation: 解决传统深度学习模型在毫米波通信系统中波束预测精度不足的问题，尤其是在少样本场景下。

Method: 结合多模态传感器数据（图像、雷达、LiDAR、GPS），利用LLM的推理能力，通过数据编码、多模态对齐与融合以及监督微调（SFT）实现预测。

Result: M2BeamLLM在波束预测中显著优于传统DL模型，且在传感器模态多样性增加时性能持续提升。

Conclusion: M2BeamLLM为车对基础设施（V2I）毫米波通信系统提供了一种高效且智能的波束预测解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统的权重衰减方法对所有层使用统一的衰减率，忽略了LLM模块间的结构多样性和谱特性差异。

Method: 基于HT-SR理论，通过分析权重相关矩阵的谱密度，自适应地为不同模块分配权重衰减强度。

Result: 在60M到1B规模的模型上，AlphaDecay在困惑度和泛化能力上优于传统方法和其他自适应基线。

Conclusion: AlphaDecay通过模块化的自适应权重衰减分配，有效提升了LLM的训练效果。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有大语言模型在细粒度归属上的不足，提升生成文本的可验证性和可解释性。

Method: 采用模块化生成框架，首先生成可执行的程序计划，再执行模块化文本操作以生成最终响应。

Result: 在两个长问答任务和一个多文档摘要任务中显著提升了文档和句子级别的归属质量。

Conclusion: GenerationPrograms不仅能作为后处理归属方法，还能通过模块级改进进一步提升归属质量。
...
<details>
  <summary>Details</summary>
Motivation: 硬件生态快速发展，需要快速、灵活且正确地将低级程序在不同ISA间转换，以提升代码的可移植性和寿命。

Method: GG利用LLM生成候选翻译，并通过软件测试框架验证翻译的正确性。

Result: 在HumanEval和BringupBench上分别达到99%和49%的正确率，性能优于Rosetta 2。

Conclusion: GG在CISC到RISC转换中表现出色，为ISA级代码翻译研究提供了新基础。
...
<details>
  <summary>Details</summary>
Motivation: 探索AMR是否能通过提供语义信息帮助NLI任务中的预训练语言模型更好地泛化。

Method: 在微调和提示两种设置下将AMR整合到NLI任务中，并进行消融实验。

Result: 微调时AMR阻碍泛化，提示时对GPT-4o有轻微提升，但改进源于表面差异而非语义推理。

Conclusion: AMR在NLI任务中可能误导模型，放大表面差异而非真正提升语义理解。
...
Method: 采用聚合机制融合连续道德接受度评分，并通过嵌入优化程序调整偏离共识的模型的词嵌入。

Result: 实验表明，该方法能建立稳健的共识并提升单个模型的判断准确性。

Conclusion: 数据驱动的多模型道德对齐具有重要价值，有助于构建更安全、更一致的AI系统。
...
<details>
  <summary>Details</summary>
Motivation: LLMs在调查研究中可能替代耗时的手动编码和监督学习模型预训练，但现有研究多集中于英语或单一LLM，缺乏对其他语言和复杂情境的验证。

Method: 使用德语调查数据，比较多种LLMs和提示方法，以人类专家编码为基准评估性能。

Result: LLMs性能差异大，仅微调LLM表现良好；提示方法效果因LLM而异；分类性能不均衡影响类别分布。

Conclusion: LLMs在开放式回答分类中需权衡效率与准确性，研究为LLMs在调查研究的适用条件提供了参考。
...
TL;DR: 研究发现，经过微调的大语言模型（LLMs）能够生成高度可信且具有煽动性的政治内容，加剧意识形态极化，引发AI伦理和治理问题。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs在政治极化中的作用，揭示其可能通过生成偏见内容加剧社会分歧。

Method: 使用Reddit的政治讨论数据集微调开源LLM，通过语言分析、情感评分和人工标注评估模型输出。

Result: 微调后的LLMs能生成与人类写作难以区分的极化内容，显示其在政治操纵中的潜在风险。

Conclusion: 强调AI治理、平台监管和检测工具开发的必要性，以应对微调LLMs的滥用风险。
...
Conclusion: 自适应专家配置显著提升了性能，验证了GuiLoMo的有效性。
...
TL;DR: 论文研究了监督微调（SFT）的关键因素，发现任务协同效应和困惑度对效果有显著影响，并计划发布1000多个SFT模型。


<details>
  <summary>Details</summary>
Motivation: 理解SFT在调整大型语言模型（LLMs）与人类指令和价值观对齐中的作用，探索其关键因素。

Method: 训练了1000多个SFT模型，涵盖代码生成、数学推理和通用任务，分析了数据集属性和层间权重变化。

Result: 发现任务协同效应和困惑度对SFT效果至关重要，中层权重变化与性能提升相关性最强。

Conclusion: SFT效果受模型特定策略和数据集属性影响，困惑度是有效预测指标，计划发布模型以促进研究。
...
TL;DR: 论文提出了一种优化训练协议的方法，旨在提高模型在罕见和代表性不足用例上的性能和可控性。通过数据特征和任务来源的分类，结合微调技术，显著提升了长尾分布任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现代机器学习在罕见和代表性不足的特征上表现不佳，传统方法如提示工程或少量示例效果有限且不稳定。论文旨在通过优化训练协议解决这一问题。

Method: 创建数据特征和任务来源的分类体系，显式和隐式控制生成属性，并通过微调基础模型自动推断这些标记。

Result: 在开放生成质量上平均提升5.7%，在代表性不足领域提升9.1%，在特定任务如CodeRepair上提升14.1%，长度指令遵循评估上提升35.3%。

Conclusion: 提出的方法显著提升了模型在长尾分布任务上的性能和可控性，为罕见用例提供了灵活且高效的解决方案。
...
TL;DR: 研究了模型架构和数据配置对生成式Transformer记忆能力的影响，发现嵌入大小是主要决定因素，Softmax激活函数表现更优。


<details>
  <summary>Details</summary>
Motivation: 探究模型架构和数据配置如何影响生成式Transformer的记忆能力，以优化模型设计。

Method: 使用基于SNOMED知识图的合成文本数据集训练模型，比较不同架构和数据配置的效果。

Result: 嵌入大小是学习速度和容量的主要决定因素，Softmax更稳定，数据复杂性提升记忆能力。
...
<details>
  <summary>Details</summary>
Motivation: 旨在通过强化学习优化MoE模型，解决训练中的不稳定性和多领域数据整合问题。

Method: 提出联合训练管道，结合蒸馏与强化学习，并开发了C3PO算法以增强训练稳定性。采用两阶段训练范式处理多领域数据冲突。

Result: 在多个基准测试中表现优异，同时显著减少激活参数数量。
...
<details>
  <summary>Details</summary>
Motivation: 现有语言模型方法偏向利用而非探索，导致性能瓶颈。熵作为强化学习中的探索信号，可能对语言模型的探索性推理有积极作用。

Method: 通过实证分析发现高熵区域与探索性推理行为相关，提出在优势函数中增加基于熵的项，以促进更长的推理链。

Result: 该方法在Pass@K指标上取得显著提升，尤其在极大K值下表现优异。

Conclusion: 通过熵信号增强探索性推理是一种简单有效的方法，能够突破语言模型推理的边界。
...
TL;DR: 论文提出了一种自回归U-Net模型，通过动态嵌入多尺度标记，替代传统的静态分词方法（如BPE），从而提升语言模型的灵活性和多任务能力。


<details>
  <summary>Details</summary>
Motivation: 传统分词方法（如BPE）固定了输入文本的粒度，限制了模型的灵活性和预测能力。本文旨在通过动态学习多尺度标记来克服这一限制。

Method: 采用自回归U-Net模型，从原始字节开始逐步聚合为单词、词组，形成多尺度视图。深层阶段预测更远的未来（如多个单词），而浅层阶段处理细节。

Result: 在合理控制预训练计算的情况下，浅层结构与BPE基线相当，深层结构展现出潜力。模型能同时处理字符级任务和跨低资源语言的知识迁移。

Conclusion: 动态嵌入多尺度标记的方法为语言模型提供了更高的灵活性，支持多任务和多语言场景，展现了未来改进的潜力。
...
TL;DR: 论文提出了一种端到端的变分方法，自动学习编码连续语音属性以增强语义标记，解决了现有方法依赖手动提取特征的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于自监督模型的语音标记（语义标记）通常忽略韵律信息，导致生成的语音自然度降低。现有方法通过添加音高特征来弥补，但音高无法完全代表副语言属性，且特征选择需手工设计。

Method: 提出了一种端到端的变分方法，自动学习编码连续语音属性，无需手动提取和选择副语言特征。

Result: 该方法生成的语音延续更受人类评分者青睐。

Conclusion: 通过自动学习编码连续语音属性，该方法提升了语音生成的自然度，避免了手工特征设计的局限性。
...
TL;DR: 提出了一种非结合代数框架，用于高维空间中信息项的表示与计算，支持序列信息的稀疏表示，并模拟认知实验中的首因效应和近因效应。


<details>
  <summary>Details</summary>
Motivation: 解决传统关联捆绑方法在表示序列信息时丢失顺序信息的问题，同时与认知科学中关于记忆的实证发现一致。

Method: 采用非结合的乘法式绑定和干扰式捆绑操作，生成两种状态（L-state和R-state）以分别模拟近因效应和首因效应。

Result: 模型能够生成稀疏表示，保持序列的时序结构，并成功复现了认知实验中的序列位置曲线。

Conclusion: 非结合框架为高维信息表示提供了新方法，同时揭示了噪声在顺序信息表示中的积极作用。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决在CPS生命周期中微分方程与其他信息集成时缺乏可重用本体工具和方法的问题。

Method: 引入模块化语义模型和高效知识图谱生成方法，直接表示微分方程并丰富其语义。

Result: 验证表明，复杂系统的微分方程可被形式化表示并与生命周期数据集成。
...
TL;DR: 本文提出了一种名为“超我”代理的新型解决方案，用于个性化监督自主AI系统，通过动态引用用户选择的“信条宪法”来调整行为，显著减少有害输出。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统在实际部署中面临与人类价值观、安全需求和合规性对齐的挑战，现有方法难以提供深度个性化信息而不引发问题。

Method: 设计了一个“超我”代理，动态引用用户选择的“信条宪法”，并通过实时合规执行器验证计划。

Result: 在基准测试中，该系统显著减少有害输出（高达98.3%），并在主流LLM上实现近乎完美的拒绝率。

Conclusion: 该方法简化了AI个性化对齐，显著提升了安全性和适应性。
...
TL;DR: 本文主张在基础模型评估中需要更严格和透明的人类基线，并提供了相关建议和报告清单。
...
Method: 基于测量理论和AI评估文献的元综述，提出了设计、执行和报告人类基线的框架和建议，并开发了清单。

Result: 系统审查了115项人类基线研究，发现现有方法的不足，清单可帮助研究人员改进基线报告。

Conclusion: 本文旨在推动更严格的AI评估实践，以更好地服务研究社区和政策制定者。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏针对医院资金层的公开基准测试，而DRG在许多国家的报销中起关键作用。

Method: 发布NordDRG-AI-Benchmark，包含三类资源：定义表、专家手册和任务提示包。

Result: 五种先进LLM在九项任务中表现差异显著，OpenAI表现最佳，Gemini表现较差。
...
TL;DR: ICE-ID是一个用于历史身份解析的新基准数据集，涵盖220年冰岛人口普查记录，支持长期人物匹配研究。


<details>
  <summary>Details</summary>
Motivation: 填补长期人物实体匹配研究的空白，提供大规模、开放的表格数据集。

Method: 定义了身份解析任务，评估了多种方法，包括规则匹配器、机器学习集成、LLMs和NARS（一种非公理推理系统）。

Result: NARS方法表现优异，达到最先进水平。

Conclusion: ICE-ID的发布为数据链接和历史分析领域的跨学科研究提供了新机会。
...
TL;DR: Med-REFL通过细粒度反思提升医学推理能力，在MedQA-USMLE基准上平均提升4.11%，并增强模型的泛化能力。
...
Method: 提出Med-REFL方法，利用树状思维分解医学问题，定量评估每一步及其反思，自动构建优化数据以减少专家标注依赖。

Result: 在MedQA-USMLE基准上平均提升4.11%，并将7B/8B模型的性能进一步提升4.13%。

Conclusion: 提升反思质量能显著提高医学AI应用的准确性和可信度。
...
TL;DR: 论文提出了一种名为BotTrans的多源图域适应模型，用于解决社交机器人检测中标签稀缺的问题，通过利用多源网络知识提升检测性能。
...
Conclusion: BotTrans通过多源知识迁移和优化策略，显著提升了无标签目标任务的检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 目标是揭示神经连接组中的生物意义前馈结构，并通过最大化前向边的总权重来验证算法的有效性。

Method: 结合了贪心启发式、增益感知的局部优化和基于强连通分量的全局结构分析。

Result: 实验表明，最佳解决方案在前向边权重上优于之前表现最好的方法。
...
<details>
  <summary>Details</summary>
Motivation: 人类通过因果认知高效学习和泛化，而当前机器学习系统在这方面表现较弱。研究旨在通过理解人类因果认知的适应性，改进机器学习系统。

Method: 对比了结构因果模型（SCM）框架与人类因果认知的差异，分析了人类在‘人类生态位’中的因果能力。

Result: SCM框架未能完全捕捉人类因果认知的关键方面，尤其是在泛化和类比推理方面。

Conclusion: 未来研究应借鉴人类因果认知的适应性特性，开发更具泛化能力和可解释性的机器学习系统。
...
<details>
  <summary>Details</summary>
Motivation: 尽管NP难问题（如TSP）在最坏情况下难以高效解决，但实际实例常具有可挖掘的结构模式。

Method: 通过严格定义、定理和元学习驱动的求解流程，提出模式利用效率（PUE）等指标。
...
Method: RIIU是一种循环单元，通过元状态和广播缓冲区增强隐藏状态，并利用滑动窗口协方差和可微分Auto-$Φ$代理实现在线信息整合。

Result: 在八向网格世界中，四层RIIU代理在13步内恢复90%以上奖励，速度是GRU的两倍，同时保持非零Auto-$Φ$信号。

Conclusion: RIIU将“类意识”计算缩小到单元规模，将哲学问题转化为可实证的数学问题。
...
### [74] [ImpReSS: Implicit Recommender System for Support Conversations](https://arxiv.org/abs/2506.14231)
...
Result: 实验结果显示，ImpReSS在推荐相关SPCs方面表现良好，MRR@1和recall@3指标在不同领域均较高。
...
### [75] [LocationReasoner: Evaluating LLMs on Real-World Site Selection Reasoning](https://arxiv.org/abs/2506.13841)
*Miho Koda,Yu Zheng,Ruixian Ma,Mingyang Sun,Devesh Pansare,Fabio Duarte,Paolo Santi*

Main category: cs.AI

TL;DR: 论文介绍了LocationReasoner基准，用于评估大语言模型在真实世界选址任务中的推理能力，发现现有模型表现有限，并揭示了过度推理的问题。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型的推理能力主要在数学和代码生成领域测试，缺乏对复杂真实场景的评估。

Method: 设计了包含300多个查询的LocationReasoner基准，结合沙盒环境和约束搜索工具进行评估。

Result: 顶尖推理模型在真实场景中表现有限，OpenAI o4模型在30%的任务中失败，且代理策略（如ReAct和Reflexion）因过度推理表现更差。

Conclusion: 论文揭示了LLM在整体和非线性推理中的局限性，并发布LocationReasoner以促进更稳健的推理能力发展。
...
### [76] [Evaluating Explainability: A Framework for Systematic Assessment and Reporting of Explainable AI Features](https://arxiv.org/abs/2506.13917)
*Miguel A. Lago,Ghada Zamzmi,Brandon Eich,Jana G. Delfino*

Main category: cs.AI

TL;DR: 提出一个评估AI可解释性特征的框架，基于一致性、合理性、忠实性和实用性四个标准，并通过案例研究验证其有效性。
...
Method: 提出基于四个标准的评估框架，并通过Ablation CAM和Eigen CAM在合成乳腺X光片上的案例研究进行验证。

Result: 开发了一个评分卡，用于全面描述和评估AI可解释性方法，并通过临床相关场景验证了前三个标准。

Conclusion: 该框架为评估AI解释质量提供了标准，旨在促进对可解释性价值的讨论，并改进AI医疗设备的开发与评估。
...
### [77] [Integrating Knowledge Graphs and Bayesian Networks: A Hybrid Approach for Explainable Disease Risk Prediction](https://arxiv.org/abs/2506.13920)
*Mbithe Nzomo,Deshendran Moodley*

Main category: cs.AI

TL;DR: 提出了一种基于知识图谱和贝叶斯网络的多模态电子健康记录（EHR）疾病风险预测方法，结合通用医学知识与患者特定情境，有效处理不确定性并保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 解决通用医学知识在特定医疗场景和患者群体中的适应性不足问题，同时处理数据不完整性和健康结果不确定性，并确保预测系统的可解释性。

Method: 通过整合知识图谱（KGs）和贝叶斯网络（BNs），构建基于本体论知识图谱和多模态EHR数据的贝叶斯网络。

Result: 在房颤预测的实际EHR数据应用中，该方法平衡了通用医学知识与患者特定情境，有效处理不确定性，具有高度可解释性，并取得良好的预测性能。

Conclusion: 该方法为疾病风险预测提供了一种可解释且高效的解决方案，结合了通用知识与个性化需求。
...
### [78] [ProfiLLM: An LLM-Based Framework for Implicit Profiling of Chatbot Users](https://arxiv.org/abs/2506.13980)
*Shahaf David,Yair Meidan,Ido Hersko,Daniel Varnovitzky,Dudu Mimran,Yuval Elovici,Asaf Shabtai*

Main category: cs.AI

TL;DR: ProfiLLM是一个通过聊天机器人交互实现隐式和动态用户画像的新框架，特别适用于IT/网络安全领域，能快速准确地推断用户技术熟练度。


<details>
  <summary>Details</summary>
Motivation: 现有聊天机器人个性化方法依赖静态用户分类或显式自我报告信息，难以适应动态变化的用户熟练度。

Method: 提出ProfiLLM框架，包括可适应多领域的分类法和基于LLM的用户画像方法，并在ITSec领域开发了变体ProfiLLM[ITSec]。

Result: 在263个合成用户的1,760次对话中，ProfiLLM[ITSec]显著减少实际与预测分数的差距（55-65%），并持续优化。
...
### [79] [SANGAM: SystemVerilog Assertion Generation via Monte Carlo Tree Self-Refine](https://arxiv.org/abs/2506.13983)
...
Conclusion: SANGAM框架能有效生成高质量的硬件断言，优于现有技术。
...
### [80] [Machine Mirages: Defining the Undefined](https://arxiv.org/abs/2506.13990)
*Hamidou Tembine*

Main category: cs.AI

TL;DR: 论文讨论了多模态机器学习系统在达到动物或人类水平任务流畅性时出现的新认知异常现象，称为“机器幻象”，并强调需明确定义和系统评估这些错误。


<details>
  <summary>Details</summary>
Motivation: 随着多模态机器学习系统在图像、语言和声音处理任务中达到动物或人类水平，它们开始表现出新的认知异常现象，这些现象需要被理解和评估以提高系统可靠性。

Method: 论文列举了多种机器幻象的错误类型，并提出了对这些错误进行明确定义和系统评估的必要性。

Result: 研究发现机器幻象包括多种认知错误，这些错误需要被明确分类和评估以改进系统。

Conclusion: 理解机器幻象对提升机器学习系统可靠性和构建尊重多样性的智能生态系统至关重要。
...
### [81] [Discovering Temporal Structure: An Overview of Hierarchical Reinforcement Learning](https://arxiv.org/abs/2506.14045)
*Martin Klissarov,Akhil Bagaria,Ziyan Luo,George Konidaris,Doina Precup,Marlos C. Machado*

Main category: cs.AI

TL;DR: 该论文探讨了分层强化学习（HRL）在复杂开放环境中的优势、方法及其面临的挑战。


<details>
  <summary>Details</summary>
Motivation: 解决AI在复杂开放环境中探索、规划和学习的难题，并明确HRL中良好结构的定义及其适用场景。

Method: 从决策基本挑战的角度分析HRL的益处，并总结从在线经验到离线数据集及大型语言模型（LLMs）的结构发现方法。

Result: 明确了HRL的性能权衡及其在特定领域的适用性。

Conclusion: HRL在发现时间结构方面具有潜力，但仍需解决结构定义和适用性问题。
...
### [82] [Into the Unknown: Applying Inductive Spatial-Semantic Location Embeddings for Predicting Individuals' Mobility Beyond Visited Places](https://arxiv.org/abs/2506.14070)
...
<details>
  <summary>Details</summary>
Motivation: 传统的位置嵌入方法依赖于历史移动模式，难以编码显式空间信息、整合丰富的城市语义上下文以及处理未见过的位置。

Method: CaLLiPer通过对比学习融合空间坐标和兴趣点的语义特征，生成空间显式、语义丰富且具有归纳性的位置嵌入。

Result: 在四个公共移动数据集上的实验表明，CaLLiPer在常规和归纳场景中均优于基线方法，尤其在归纳场景中表现突出。

Conclusion: 多模态、归纳性的位置嵌入有望提升人类移动预测系统的能力，研究还公开了代码和数据以促进可重复性和未来研究。
...
### [83] [FormGym: Doing Paperwork with Agents](https://arxiv.org/abs/2506.14079)
*Matthew Toles,Rattandeep Singh,Isaac Song Zhou Yu*

Main category: cs.AI

TL;DR: 论文提出了一个表单填写基准测试，并开发了FieldFinder工具以提升模型在纯图像表单中的填写能力。


<details>
  <summary>Details</summary>
Motivation: 表单填写在纯图像领域（无OCR或PDF文本）对计算机代理具有挑战性，需多模态理解和工具使用能力。

Method: 提出了包含432个字段的基准测试，并开发FieldFinder工具辅助LLMs定位表单填写位置。

Result: 基线VLA和GUI代理表现不佳（<1%和10.6-68%），FieldFinder使模型性能提升至56%。

Conclusion: FieldFinder显著提升了模型在表单填写任务中的表现，解决了定位能力不足的问题。
...
### [84] [Lightweight Relevance Grader in RAG](https://arxiv.org/abs/2506.14084)
*Taehee Jeong*

Main category: cs.AI

TL;DR: 本文提出了一种轻量级相关性评分器（relevance grader）用于RAG系统，通过微调llama-3.2-1b模型显著提升了检索文档的相关性评分精度。


<details>
  <summary>Details</summary>
Motivation: 解决RAG系统中检索文档与查询相关性不足的问题，同时降低计算资源需求。

Method: 微调轻量级模型llama-3.2-1b作为相关性评分器，替代计算密集型的大模型。

Result: 相关性评分精度从0.1301提升至0.7750，接近llama-3.1-70b的性能。
...
### [85] [Fragile Preferences: A Deep Dive Into Order Effects in Large Language Models](https://arxiv.org/abs/2506.14092)
...
<details>
  <summary>Details</summary>
Motivation: LLMs在高风险领域（如招聘和大学录取）的决策支持系统中应用广泛，但其位置顺序偏见尚未被系统研究。

Method: 通过多LLM架构和领域的实验，分析位置偏见，并引入框架分类偏好为稳健、脆弱或中立。

Result: 发现强烈的顺序效应、新颖的中心偏见和质量依赖的转变，以及位置偏见强于性别偏见。

Conclusion: LLMs表现出独特的偏见模式，需针对性缓解策略，如温度参数的新应用。
...
### [86] [Situational-Constrained Sequential Resources Allocation via Reinforcement Learning](https://arxiv.org/abs/2506.14125)
...
Result: 在医疗资源分配和农药分配场景中，SCRL在满足约束的同时保持高效，优于基线方法。
...
### [87] [Collaborative Editable Model](https://arxiv.org/abs/2506.14146)
*Kaiwen Tang,Aitong Wu,Yao Lu,Guangda Sun*

Main category: cs.AI

TL;DR: CoEM通过用户贡献的知识片段和交互式对话，结合用户评分和归因分析，实现轻量级领域适应，显著提升垂直领域LLM的生成质量。


<details>
  <summary>Details</summary>
Motivation: 垂直领域LLM的训练依赖大规模标注数据和计算资源，阻碍快速开发和迭代。

Method: 构建用户贡献的知识池，通过交互式对话和用户评分筛选高价值知识片段，并通过上下文提示注入模型。

Result: 在金融信息场景中，收集15k用户反馈，验证了CoEM显著提升领域特定生成质量。

Conclusion: CoEM避免了传统微调的时间和计算开销，实现了高效的领域适应。
...
### [88] [What's in the Box? Reasoning about Unseen Objects from Multimodal Cues](https://arxiv.org/abs/2506.14212)
...
Method: 提出神经符号模型：用神经网络解析多模态输入，再用贝叶斯模型整合信息评估假设。实验采用“盒中何物”游戏，比较模型与人类表现。

Result: 模型与人类判断高度相关，而单模态或纯神经网络基线表现较差。

Conclusion: 神经符号方法能有效模拟人类多源信息整合能力，优于传统方法。
...
### [89] [From Black Boxes to Transparent Minds: Evaluating and Enhancing the Theory of Mind in Multimodal Large Language Models](https://arxiv.org/abs/2506.14224)
*Xinyang Li,Siqi Liu,Bochao Zou,Jiansheng Chen,Huimin Ma*

Main category: cs.AI

TL;DR: 本文提出了一种基于内部机制的多模态大语言模型（MLLMs）心智理论（ToM）评估方法，构建了多模态ToM测试数据集GridToM，并通过注意力机制分析展示了模型的ToM能力。
...
Method: 构建多模态ToM测试数据集GridToM，分析注意力头对认知信息的区分能力，并提出一种无需训练的轻量级方法调整注意力方向以增强ToM表现。

Result: 注意力头能够区分多视角的认知信息，证明模型具备ToM能力；调整注意力方向显著提升了模型的ToM表现。

Conclusion: 通过内部机制分析，本研究为多模态大语言模型的ToM能力提供了可解释的评估方法，并展示了注意力机制在ToM中的关键作用。
...
TL;DR: 提出了一种基于哲学因果关系的测试方法，评估AI（如ChatGPT、DeepSeek和Gemini）的抽象因果推理能力，并扩展了神经元图中因果定义的有效性。
...
Method: 基于D. Lewis的神经元图理论设计测试，评估大型语言模型的因果推理能力。

Result: 发现ChatGPT等模型能正确识别文献中争议的因果关系，提出了更广泛的因果定义。

Conclusion: 未来哲学研究可能发展为人类与AI专业知识的互动。
...
TL;DR: 论文提出RLVR（可验证奖励的强化学习）在提升大语言模型推理能力时存在矛盾：RLVR调优的模型在$Pass@K$指标上表现不佳，原因是该指标忽略了推理路径的正确性。作者引入新指标$CoT$-$Pass@K$，要求推理路径和答案均正确，并证明RLVR能激励逻辑完整性。实验验证了新指标的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决RLVR调优模型在$Pass@K$指标上表现不佳的矛盾，揭示该指标忽略推理路径正确性的问题。

Method: 提出新评估指标$CoT$-$Pass@K$，要求推理路径和答案均正确；理论分析RLVR如何激励逻辑完整性。

Result: 实验表明，使用$CoT$-$Pass@K$时，RLVR能激励正确推理的泛化，且训练早期即显现效果。

Conclusion: RLVR能真正提升机器推理能力，$CoT$-$Pass@K$是更可靠的评估方法。
...
TL;DR: 论文提出Mxplainer算法，通过参数化搜索学习黑盒AI代理的参数，提供人类可理解的洞察，并解释其决策过程。


<details>
  <summary>Details</summary>
Motivation: 现有麻将AI代理性能接近人类职业玩家，但作为黑盒难以理解其内部机制。

Method: 引入Mxplainer算法，将参数化搜索转换为等效神经网络，学习黑盒代理参数。

Result: 实验表明，学习到的参数提供了对代理特性和风格的洞察，并能局部解释其决策。

Conclusion: Mxplainer成功解析黑盒代理，为人类理解AI决策提供了工具。
...
TL;DR: 论文提出了一种基于深度学习的测试时动态训练方法，显著提升了ARC-AGI任务的性能。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习在ARC任务上表现不佳，但其仍是多领域中最有效的策略，因此研究如何进一步利用深度学习提升ARC性能。

Method: 结合预训练LLMs和测试时微调（TTFT）及增强推理反向增强投票（AIRV）技术，动态训练神经网络和优化器。

Result: AIRV提升准确率260%，TTFT再提升300%，最终在ARC私有测试集上达到58%的准确率，赢得2023 ARCathon比赛。

Conclusion: 深度学习可通过动态训练和优化器集成显著提升ARC任务性能，为陌生领域的推理系统提供关键机制。
...
TL;DR: 本文提出了一种基于大语言模型（LLM）的可解释自动驾驶决策系统ADRD框架，通过三个核心模块实现高效决策，并在实验中表现出优于传统方法的性能。


<details>
  <summary>Details</summary>
Motivation: 构建可解释的自动驾驶决策系统是学术研究的焦点，传统方法在解释性和性能上存在不足。

Method: ADRD框架整合了信息模块、代理模块和测试模块，利用LLM生成规则化驾驶策略并通过迭代测试优化。

Result: 实验表明，ADRD在解释性、响应速度和驾驶性能上显著优于传统强化学习和先进LLM方法。

Conclusion: ADRD首次将LLM与规则系统结合用于自动驾驶决策，验证了其实际部署潜力。
...
TL;DR: 论文提出了一种名为RALA-DPO的方法，通过结合直接偏好优化（DPO）和检索增强生成（RAG）技术，优化了大型语言模型在航空理论培训中的表现，提高了回答的准确性和知识更新效率。


<details>
  <summary>Details</summary>
Motivation: 现有的航空培训体系中，专业知识的传授依赖有限的教员，而互联网上的答案准确性不足，导致培训效率低下。预训练模型在专业领域的回答也不够准确。

Method: 采用直接偏好优化（DPO）对开源预训练模型Qwen进行领域对齐，并结合检索增强生成（RAG）技术，以解决训练数据偏差、知识过时或领域知识不足导致的幻觉问题。

Result: 实验结果表明，RALA-DPO能显著提高对航空专业知识的回答准确性，并通过RAG机制实现零成本知识更新。

Conclusion: RALA-DPO通过DPO和RAG的结合，有效提升了航空理论培训的效率和准确性，同时支持低成本知识更新。
...
TL;DR: 论文提出SEAT方法，解决LLM微调中忽略安全对齐能力退化的问题，尤其是模型表达无知的能力。


<details>
  <summary>Details</summary>
Motivation: 现有研究专注于保护特定数据或任务，但忽视了安全对齐能力的退化，尤其是模型表达无知的能力。

Method: SEAT方法结合稀疏训练和实体扰动加KL散度正则化，以限制激活漂移和知识纠缠。

Result: 实验表明SEAT在保持微调性能的同时显著优于基线，保留了无知意识。
...
TL;DR: 论文通过实证研究评估了基于AST的混合图表示在图神经网络（GNN）代码克隆检测中的效果，发现不同混合表示对GNN性能影响各异，其中AST+CFG+DFG提升准确性，而FA-AST可能降低性能。GMN在标准AST下表现最优。


<details>
  <summary>Details</summary>
Motivation: 代码克隆显著增加软件维护成本和漏洞风险，现有基于AST的深度学习方法缺乏语义深度，需进一步研究混合图表示的有效性。

Method: 通过系统比较多种混合图表示（CFG、DFG、FA-AST）在不同GNN架构中的表现，评估其效果。

Result: AST+CFG+DFG提升卷积和注意力模型的准确性，FA-AST可能因结构复杂降低性能；GMN在标准AST下表现最佳。

Conclusion: 混合图表示对GNN性能影响显著，GMN在标准AST下已具备优越性能，减少了对复杂结构的依赖。
...
TL;DR: GUI-Robust是一个新的数据集，专注于评估GUI代理在真实异常情况下的表现，并通过半自动化方法显著降低标注成本。


<details>
  <summary>Details</summary>
Motivation: 现有GUI数据集在理想条件下构建，忽略了真实部署中的多样性异常，限制了研究的实用性。

Method: 采用半自动化范式，结合RPA工具收集用户动作序列，并利用MLLM生成描述，减少标注时间。

Result: 实验表明，现有GUI代理在异常场景下性能显著下降。

Conclusion: GUI-Robust强调了GUI代理鲁棒性的重要性，并提供了数据集和代码以促进未来研究。
...
Method: 通过实现和比较Boids和蚁群优化（ACO）算法，评估延迟、资源使用和行为准确性，同时考察云端和本地LLM在群体系统中的应用。

Result: LLM提供了强大的推理和抽象能力，但也带来了计算和协调上的新约束，挑战了传统群体设计的概念。

Conclusion: 研究揭示了LLM在群体系统中的机遇与局限，并讨论了现代AI研究中'群体'定义的演变。
...
TL;DR: 本文提出了一种安全优先的人性化决策框架（SF-HLDM），用于自动驾驶车辆在动态交通流中安全、舒适且符合社会规范地行驶。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在密集和交互性强的交通环境中面临挑战，数据驱动方法因人类行为的多样性和不可预测性而效果不佳。

Method: SF-HLDM框架结合了时空注意力机制、社会合规性估计模块和深度进化强化学习模型，以高效扩展搜索空间并避免局部最优。

Result: 该框架使自动驾驶AI能够动态调整决策参数，保持安全距离并符合情境化驾驶行为。

Conclusion: SF-HLDM框架提升了自动驾驶的决策灵活性、可解释性和适应性。
...
TL;DR: 论文提出了一种名为“Doppelgänger方法”的攻击手段，展示了代理被劫持的风险，并定义了“PACAT”级别评估其脆弱性。同时提出了“CAT提示”作为防御手段。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的普及，快速创建多样化的自主代理带来了安全和行为一致性的担忧，尤其是防止提示被用户恶意利用的挑战。

Method: 提出“Doppelgänger方法”攻击代理，定义“PACAT”级别评估脆弱性，并设计“CAT提示”作为防御措施。

Result: 实验表明，Doppelgänger方法能破坏代理一致性并暴露内部信息，而CAT提示能有效防御此类攻击。

Conclusion: 研究揭示了代理提示的脆弱性，并提供了有效的防御方案，强调了安全设计的重要性。
...
TL;DR: QUEST是一个面向商业文档的质量感知半监督表格提取框架，通过新颖的质量评估模型和多样性度量，显著提升了表格提取的准确性和完整性。
...
Method: QUEST提出了一种质量评估模型，通过预测F1分数评估提取表格的结构和上下文特征，并结合多样性度量（如DPP、Vendi分数、IntDiv）减少确认偏差。

Result: 在商业数据集上，QUEST将F1从64%提升至74%，空预测减少45%；在DocILE基准测试中，F1从42%提升至50%，空预测减少19%。
...
TL;DR: 论文提出了一种通过神经嵌入增强符号机器学习的方法，以解决现有神经符号AI系统在简单场景下效率不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有神经符号AI系统（如LTN和DeepProbLog）虽然功能强大，但在简单场景（如判别式机器学习）中效率较低，特别是在涉及大量常量的领域。

Method: 通过为符号机器学习方案（如TILDE）提供神经嵌入支持，特别是利用相似性谓词中的常量嵌入。

Result: 在三个实际领域实验中，该方法在F1分数上优于所有基线方法。

Conclusion: 该方法不仅适用于当前场景，还可扩展到实例间相似性、类比推理或命题化等更广泛的应用。
...
TL;DR: 提出一种新型空间基础模型，整合地理位置语义与人类移动性，以支持可扩展、可迁移的时空数据分析。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型在处理移动性数据的时空和语义复杂性方面存在局限，需要一种通用模型来支持跨地理和场景的分析。

Method: 从离散兴趣点建模转向理解动态、上下文丰富的区域（即“场所”），并提出研究方向以解决适应性、可扩展性和多粒度推理的不足。

Result: 提出开发可扩展、上下文感知的下一代地理空间智能模型。

Conclusion: 这些模型将支持从个性化场所发现到城市规划的广泛应用，实现更智能的空间决策。
...
TL;DR: AgentDistill是一种无需训练的智能体蒸馏框架，通过直接重用教师智能体生成的模块化任务解决模块（MCPs），实现高效知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有智能体蒸馏方法在动态规划和适应新环境方面表现不佳，需要一种更高效、可扩展的方法。

Method: 提出AgentDistill框架，利用教师智能体生成的MCPs模块，直接复用这些模块以训练学生智能体。

Result: 实验表明，基于小语言模型的学生智能体性能接近使用大模型（如GPT-4o）的先进系统。

Conclusion: AgentDistill为构建可扩展且成本高效的智能体提供了有效方法。
...
TL;DR: 论文提出LC-R1方法，通过Brevity和Sufficiency原则优化大型推理模型（LRMs），减少冗余推理链，实现序列长度显著减少（约50%）且准确率仅轻微下降（约2%）。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）常产生冗余推理链，尤其是无效的重复检查。为解决这一问题，论文提出Brevity和Sufficiency原则。

Method: 基于Group Relative Policy Optimization（GRPO），提出LC-R1方法，结合Length Reward和Compress Reward，优化推理链的简洁性和有效性。

Result: 实验表明，LC-R1在多个推理基准上显著减少序列长度（约50%），准确率仅轻微下降（约2%）。

Conclusion: LC-R1在Pareto前沿实现了高效压缩与性能的平衡，为开发更高效的大型推理模型提供了新思路。
...
### [107] [Knowledge Compression via Question Generation: Enhancing Multihop Document Retrieval without Fine-tuning](https://arxiv.org/abs/2506.13778)
...
<details>
  <summary>Details</summary>
Motivation: 改进RAG系统的检索效率，减少存储需求和延迟，同时提供更直观的知识访问方式。

Method: 通过生成覆盖词汇和语义空间的问题编码文本内容，结合自定义语法重排序方法。

Result: 在单跳检索中Recall@3达0.84，多跳任务中F1分数为0.52，存储需求减少80%。

Conclusion: 该方法高效、可扩展，是传统RAG的有力替代方案。
...
### [108] [XGraphRAG: Interactive Visual Analysis for Graph-based Retrieval-Augmented Generation](https://arxiv.org/abs/2506.13782)
*Ke Wang,Bo Pan,Yingchaojie Feng,Yuwei Wu,Jieyi Chen,Minfeng Zhu,Wei Chen*

Main category: cs.IR

TL;DR: 论文提出了一种可视化分析框架XGraphRAG，帮助开发者分析GraphRAG的召回效果，并通过交互式可视化提升其可解释性和可访问性。


<details>
  <summary>Details</summary>
Motivation: 传统GraphRAG由于复杂的信息处理流程和大量LLM调用，导致其可解释性和可访问性受限，开发者难以分析其效果。

Method: 研究提出了一种可视化分析框架，并开发了原型系统XGraphRAG，包含交互式可视化工具。

Result: 评估表明该方法有效且易用，系统已开源。

Conclusion: XGraphRAG提升了GraphRAG的可解释性和可访问性，为开发者提供了改进工具。
...
### [109] [AcademicBrowse: Benchmarking Academic Browse Ability of LLMs](https://arxiv.org/abs/2506.13784)
*Junting Zhou,Wang Li,Yiyan Liao,Nengyuan Zhang,Tingjia Miaoand Zhihui Qi,Yuhan Wu,Tong Yang*

Main category: cs.IR

TL;DR: 论文提出了AcademicBrowse数据集，专门用于评估大语言模型（LLMs）在学术研究中的复杂信息检索能力，填补了现有基准在学术搜索需求上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准（如OpenAI的BrowseComp）主要关注通用搜索场景，未能满足学术搜索的特定需求，如深度文献追踪、学术数据库支持、长尾知识导航及学术严谨性。

Method: 设计了AcademicBrowse数据集，具备学术实用性、高难度、简洁评估和广泛覆盖等关键特性。

Result: 数据集覆盖15个学科，通过限制条件确保答案唯一性，并提供清晰来源和解决方案解释，便于审核验证。
...
### [110] [Analysis of Anonymous User Interaction Relationships and Prediction of Advertising Feedback Based on Graph Neural Network](https://arxiv.org/abs/2506.13787)
*Yanjun Dai,Haoyang Feng,Yuan Gao*

Main category: cs.IR

TL;DR: 提出了一种解耦时序层次图神经网络（DTH-GNN），通过多通道特征提取和层次聚合，显著提升了在线广告中匿名用户行为建模的性能。
...
Method: 1. 引入时序边分解，将交互分为短时突发、日周期和长程记忆三种通道；2. 构建层次异构聚合，通过元路径条件Transformer编码器结合子图；3. 提出反馈感知对比规律和轻量策略梯度层优化节点表示。

Result: DTH-GNN的AUC提升8.2%，对数损失降低5.7%，优于基线模型。

Conclusion: DTH-GNN通过多尺度特征建模和层次聚合，显著提升了广告交互网络的表示能力。
...
### [111] [InsertRank: LLMs can reason over BM25 scores to Improve Listwise Reranking](https://arxiv.org/abs/2506.14086)
*Rahul Seetharaman,Kaustubh D. Dhole,Aman Bansal*

Main category: cs.IR

TL;DR: InsertRank是一种基于大型语言模型（LLM）的重新排序器，通过结合BM25等词汇信号提升检索性能，在多个基准测试中表现优异。
...
Method: InsertRank结合LLM的推理能力和BM25等词汇信号进行重新排序。
...
### [112] [RAGtifier: Evaluating RAG Generation Approaches of State-of-the-Art RAG Systems for the SIGIR LiveRAG Competition](https://arxiv.org/abs/2506.14412)
*Tim Cofala,Oleh Astappiev,William Xion,Hailay Teklehaymanot*

Main category: cs.IR

TL;DR: LiveRAG 2025挑战赛探索了结合检索增强生成（RAG）技术的方法，旨在提升大型语言模型（LLM）的事实准确性。最终方案结合InstructRAG、Pinecone检索器和BGE重排器，在挑战赛中排名第四。


<details>
  <summary>Details</summary>
Motivation: 通过结合LLM的内部参数知识和外部非参数知识，提升模型的事实准确性并减少幻觉现象。

Method: 使用InstructRAG结合Pinecone检索器和BGE重排器，在限制条件下（如模型参数不超过10B）优化RAG方案。

Result: 最终方案在SIGIR 2025 LiveRAG挑战赛中取得正确性得分1.13和忠实度得分0.55，排名第四。

Conclusion: InstructRAG结合特定检索器和重排器是一种有效的RAG解决方案，能在限制条件下提升模型性能。
...
### [113] [Similarity = Value? Consultation Value Assessment and Alignment for Personalized Search](https://arxiv.org/abs/2506.14437)
*Weicong Qin,Yi Xu,Weijie Yu,Teng Shi,Chenglei Shen,Ming He,Jianping Fan,Xiao Zhang,Jun Xu*

Main category: cs.IR

TL;DR: 论文提出了一种基于咨询价值评估的个性化搜索框架VAPS，通过三个新视角评估历史咨询的价值，显著提升了搜索性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅依赖语义相似性评估咨询价值，但无法准确反映其对个性化的实际贡献。

Method: 提出咨询价值评估框架，从场景范围价值、后续行动价值和时间衰减价值三个角度评估历史咨询，并基于此构建VAPS模型。

Result: 在公开和商业数据集上，VAPS在检索和排序任务中均优于基线方法。

Conclusion: VAPS通过评估咨询价值并选择性整合高价值咨询，显著提升了个性化搜索的效果。
...
### [114] [Vela: Scalable Embeddings with Voice Large Language Models for Multimodal Retrieval](https://arxiv.org/abs/2506.14445)
*Ruofan Hu,Yan Xia,Minjie Hong,Jieming Zhu,Bo Chen,Xiaoda Yang,Minghui Fang,Tao Jin*

Main category: cs.IR

TL;DR: Vela是一个新颖的多模态大语言模型框架，专注于生成通用多模态嵌入，尤其在音频领域表现优异。


<details>
  <summary>Details</summary>
Motivation: 探索多模态大语言模型在音频领域的表示能力，解决现有模型（如CLAP）在长文本和复杂检索任务中的局限性。

Method: 通过特别设计的提示和上下文学习示例，利用MLLMs生成多模态嵌入，并采用单模态（文本对）训练方法。

Result: Vela在文本-音频检索任务中优于传统CLAP模型，并在新基准测试中展现了更强的鲁棒性。

Conclusion: Vela通过结合MLLMs的能力，在多模态嵌入生成任务中表现出色，为音频领域的多模态研究提供了新方向。
...
### [115] [RMIT-ADM+S at the SIGIR 2025 LiveRAG Challenge](https://arxiv.org/abs/2506.14516)
*Kun Ran,Shuoqi Sun,Khoi Nguyen Dinh Anh,Damiano Spina,Oleg Zendel*

Main category: cs.IR

TL;DR: RMIT--ADM+S团队在SIGIR 2025 LiveRAG挑战中提出GRAG方法，通过生成假设答案辅助检索，结合LLM重排序，最终在挑战中取得优异成绩。


<details>
  <summary>Details</summary>
Motivation: 提升检索增强生成（RAG）的效果，通过生成假设答案优化检索过程，并结合LLM重排序提高答案质量。

Method: 采用GRAG方法，生成假设答案用于检索，结合LLM重排序，并通过GoP框架和N-way ANOVA进行多配置评估。

Result: 系统在私人排行榜上获得Relevance分数1.199和Faithfulness分数0.477，位列前四。

Conclusion: GRAG方法在LiveRAG挑战中表现优异，验证了其设计有效性。
...
### [116] [A Systematic Replicability and Comparative Study of BSARec and SASRec for Sequential Recommendation](https://arxiv.org/abs/2506.14692)
*Chiara D'Ercoli,Giulia Di Teodoro,Federico Siciliano*

Main category: cs.IR

TL;DR: 比较两种序列推荐系统SASRec和BSARec，验证频率增强对推荐效果的提升，发现BSARec性能优于SASRec，但提升幅度低于原作者声称。


<details>
  <summary>Details</summary>
Motivation: 验证频率增强在序列推荐系统中的实际效果，并强调实现细节对性能比较的重要性。

Method: 通过EasyRec框架重新实现SASRec和BSARec，确保公平和可复现的比较。

Result: BSARec因频率增强优于SASRec，但性能提升幅度低于预期。

Conclusion: 研究强调了实现细节对性能比较的关键作用，并提供了现有方法的概述。
...
### [117] [LittleBit: Ultra Low-Bit Quantization via Latent Factorization](https://arxiv.org/abs/2506.13771)
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型（LLM）部署中的高内存和计算成本问题，尤其是在低于1位的量化场景下性能下降的挑战。

Method: 采用低秩矩阵分解表示权重，随后二值化，并引入多尺度补偿机制（包括行、列和潜在维度）以减少信息损失。关键贡献包括Dual-SVID和残差补偿。

Result: 在0.1 BPW下，Llama2-7B的性能优于现有方法的0.7 BPW，内存减少31倍，内核级基准测试显示速度提升5倍。
...
### [118] [MobiEdit: Resource-efficient Knowledge Editing for Personalized On-device LLMs](https://arxiv.org/abs/2506.13772)
*Zhenyan Lu,Daliang Xu,Dongqi Cai,Zexi Li,Wei Liu,Fangming Liu,Shangguang Wang,Mengwei Xu*

Main category: cs.LG

TL;DR: MobiEdit是一个移动知识编辑框架，首次在商用移动设备上实现高效大型语言模型（LLM）个性化，通过量化前向梯度估计替代资源密集的反向传播。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在移动设备上处理个性化查询时产生的幻觉问题，同时避免传统知识编辑方法因反向传播资源需求高而无法在移动设备上运行的问题。

Method: 采用量化前向梯度估计替代反向传播，引入自适应提前终止机制和前缀缓存优化梯度估计效率。

Result: 在商用移动设备上实时编辑3B参数模型（Qwen2.5-3B-Instruct），内存减少7.6倍，能耗降低14.7倍，延迟减少3.6倍。

Conclusion: MobiEdit为移动设备上的LLM知识编辑提供了高效、实用的解决方案。
...
### [119] [Solving the Job Shop Scheduling Problem with Graph Neural Networks: A Customizable Reinforcement Learning Environment](https://arxiv.org/abs/2506.13781)
*Pablo Ariño Fernández*

Main category: cs.LG

TL;DR: 论文介绍了JobShopLib，一个模块化库，用于定制化解决作业车间调度问题中的图神经网络（GNN）训练问题，并通过模仿学习展示了其效果。


<details>
  <summary>Details</summary>
Motivation: 作业车间调度问题是一个NP难组合优化问题，传统方法依赖简单启发式规则。深度学习模型（如GNN）虽能学习优先级分配，但训练过程需要定制多种因素，缺乏模块化实验库。

Method: 开发了JobShopLib模块化库，支持定制图表示、节点特征、动作空间和奖励函数，并提供了强化学习环境。通过模仿学习训练调度器。
...
### [120] [Enhancing Bagging Ensemble Regression with Data Integration for Time Series-Based Diabetes Prediction](https://arxiv.org/abs/2506.13786)
...
### [121] [Hybrid Meta-Learning Framework for Anomaly Forecasting in Nonlinear Dynamical Systems via Physics-Inspired Simulation and Deep Ensembles](https://arxiv.org/abs/2506.13828)
...
<details>
  <summary>Details</summary>
Motivation: 针对非线性动态系统的非平稳和随机行为，开发一种通用的数据驱动方法，用于早期缺陷识别和预测监控，尤其是在无法获得完整物理模型的场景中。

Method: 整合了物理模拟器、CNN-LSTM时空特征提取、VAE无监督异常评分、Isolation Forests残差检测以及DA-RNN一步预测，并通过元学习器组合这些模型的输出。

Result: 混合集成模型在异常定位、泛化能力和非线性偏差鲁棒性方面优于独立模型。

Conclusion: 该框架为非线性系统提供了一种广泛适用的数据驱动方法，适用于多种复杂场景。
...
### [122] [Quantifying Structure in CLIP Embeddings: A Statistical Framework for Concept Interpretation](https://arxiv.org/abs/2506.13831)
*Jitian Zhao,Chenghui Li,Frederic Sala,Karl Rohe*

Main category: cs.LG

TL;DR: 论文提出了一种基于假设检验的框架，用于量化CLIP嵌入空间中的旋转敏感结构，并提出了后验概念分解方法，以验证和比较概念识别技术。


<details>
  <summary>Details</summary>
Motivation: 当前基于概念的方法缺乏统计严谨性，难以验证识别的概念和比较不同技术。

Method: 引入假设检验框架量化CLIP嵌入空间中的旋转敏感结构，并提出后验概念分解方法。

Result: 方法在重建误差上优于其他技术，并在去除虚假背景概念后，最差组准确率提高了22.6%。

Conclusion: 该方法在重建准确性和概念可解释性之间取得了平衡，并有效减少了数据中的虚假线索。
...
### [123] [Evolvable Conditional Diffusion](https://arxiv.org/abs/2506.13834)
*Zhao Wei,Chin Chun Ooi,Abhishek Gupta,Jian Cheng Wong,Pao-Hsiung Chiu,Sheares Xue Wen Toh,Yew-Soon Ong*

Main category: cs.LG

TL;DR: 提出一种可进化的条件扩散方法，利用黑箱、不可微的多物理模型指导生成过程，支持自主科学发现。


<details>
  <summary>Details</summary>
Motivation: 解决在多物理领域（如计算流体动力学和电磁学）中，黑箱、不可微模型难以用于生成过程的问题。

Method: 将指导建模为优化问题，通过更新去噪分布的描述统计量优化目标函数，从概率进化的角度推导出进化引导方法。

Result: 在流体拓扑和超表面自动设计中验证了方法的有效性，生成的方案更优且无需依赖可微代理。

Conclusion: 该方法为利用科学中常见的黑箱、不可微多物理模型提供了一种有效的引导扩散手段。
...
### [124] [Robustness of Reinforcement Learning-Based Traffic Signal Control under Incidents: A Comparative Study](https://arxiv.org/abs/2506.13836)
*Dang Viet Anh Nguyen,Carlos Lima Azevedo,Tomer Toledo,Filipe Rodrigues*

Main category: cs.LG

TL;DR: T-REX是一个基于SUMO的开源仿真框架，用于在动态交通事件场景下训练和评估强化学习交通信号控制（RL-TSC）方法。研究发现，独立值基和分散压力基方法在稳定交通条件下表现良好，但在事件驱动的分布变化下性能下降；而分层协调方法在大规模不规则网络中表现更稳定，但收敛较慢。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索RL-TSC在真实世界交通事件中的鲁棒性，目前这一领域的研究较少。

Method: 提出T-REX框架，模拟真实网络性能，包括驾驶员行为（如概率性改道、速度调整和车道变更），并设计了一套评估鲁棒性的指标。

Result: 实验表明，独立值基和分散压力基方法在稳定条件下表现优异，但在事件场景下性能下降；分层协调方法在大规模网络中更稳定，但收敛较慢。

Conclusion: 结论强调了鲁棒性设计和评估在RL-TSC研究中的重要性，T-REX为动态和破坏性交通场景下的RL方法提供了标准化、可复现的基准平台。
...
### [125] [Sustainable Machine Learning Retraining: Optimizing Energy Efficiency Without Compromising Accuracy](https://arxiv.org/abs/2506.13838)
*Lorena Poenaru-Olaru,June Sallou,Luis Cruz,Jan Rellermeyer,Arie van Deursen*

Main category: cs.LG

TL;DR: 本文研究了机器学习模型重训练技术的能源消耗，比较了不同方法在能源效率和准确性上的表现，提出了更可持续的重训练策略。


<details>
  <summary>Details</summary>
Motivation: 机器学习系统的可靠性受数据变化影响，重训练虽常见但能耗高，需探索更可持续的方法。

Method: 比较不同重训练技术的能源消耗和准确性，包括仅使用最新数据或按需重训练。

Result: 仅用最新数据重训练可减少25%能耗；按需重训练可减少40%能耗。

Conclusion: 研究结果为设计可持续ML系统提供了更节能的重训练技术建议。
...
### [126] [SatHealth: A Multimodal Public Health Dataset with Satellite-based Environmental Factors](https://arxiv.org/abs/2506.13842)
*Yuanlong Wang,Pengqi Wang,Changchang Yin,Ping Zhang*

Main category: cs.LG

TL;DR: SatHealth是一个结合了多模态时空数据的新数据集，用于提升AI模型在公共卫生和个人疾病风险预测中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究因缺乏长期和细粒度的时空数据，难以整合环境数据，限制了模型的性能和应用。

Method: 开发了SatHealth数据集，整合环境数据、卫星图像、疾病流行率和社会健康决定因素指标，并在两个用例中进行了实验。

Result: 实验表明，生活环境信息显著提高了AI模型的性能和时空泛化能力。

Conclusion: SatHealth为医疗研究提供了环境数据的整合框架，并建立了未来环境健康信息学的基础。
...
### [127] [StaQ it! Growing neural networks for Policy Mirror Descent](https://arxiv.org/abs/2506.13862)
*Alena Shilova,Alex Davey,Brahim Driss,Riad Akrour*

Main category: cs.LG

TL;DR: 论文提出了一种改进的Policy Mirror Descent (PMD)算法StaQ，通过仅保留最近的M个Q函数来解决传统PMD中存储所有历史Q函数的不可行问题，实现了收敛性且性能稳定。


<details>
  <summary>Details</summary>
Motivation: 在强化学习中，正则化方法（如熵奖励或KL散度约束）虽在实践中表现良好，但传统PMD框架因需存储所有历史Q函数而难以实现。

Method: 提出StaQ算法，仅保留最近的M个Q函数，确保收敛性且不引入策略更新误差。

Result: StaQ在理论上有强保证，性能与现有深度强化学习基线相当，且波动更小。
...
### [128] [Scaling Algorithm Distillation for Continuous Control with Mamba](https://arxiv.org/abs/2506.13892)
*Samuel Beaussant,Mehdi Mounsif*

Main category: cs.LG

TL;DR: 本文提出使用S6模型（选择性结构化状态空间序列模型）改进算法蒸馏（AD），以解决传统Transformer在长序列建模中的局限性，并在复杂连续环境中验证其优越性。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer在算法蒸馏（AD）中因注意力机制的二次复杂度限制，仅适用于简单离散环境，无法处理长序列任务。

Method: 采用S6模型（如Mamba）替代Transformer，利用其线性复杂度和长序列建模能力，在四个复杂连续元强化学习环境中进行实验。

Result: Mamba在AD任务中表现优于Transformer，且通过扩展上下文长度，AD的性能可媲美当前最优的在线元强化学习基线。

Conclusion: S6模型为算法蒸馏提供了更高效的解决方案，尤其在长序列和复杂环境中具有显著优势。
...
### [129] [Enhancing interpretability of rule-based classifiers through feature graphs](https://arxiv.org/abs/2506.13903)
...
<details>
  <summary>Details</summary>
Motivation: 在需要透明性和可信度的领域（如医疗），规则基系统因其可解释性而被广泛使用，但随着规则复杂性增加，理解特征贡献变得困难。

Method: 提出基于图的特征可视化策略、特征重要性度量和规则集距离度量，并在临床数据集和四种规则基方法上实验。

Result: 方法能够揭示临床特征的预测价值，识别新风险因素和生物标志物，并在15个公共基准测试中表现优异。

Conclusion: 该框架为规则基系统提供了强大的特征分析工具，有助于提升诊断准确性和决策支持系统的透明度。
...
### [130] [GITO: Graph-Informed Transformer Operator for Learning Complex Partial Differential Equations](https://arxiv.org/abs/2506.13906)
*Milad Ramezankhani,Janak M. Patel,Anirudh Deodhar,Dagnachew Birru*

Main category: cs.LG

TL;DR: 提出了一种新型的图信息变换器算子（GITO）架构，用于学习不规则几何和非均匀网格上的复杂偏微分方程系统。
...
Method: GITO由混合图变换器（HGT）和变换器神经算子（TNO）组成，HGT结合图神经网络和变换器，TNO使用线性复杂度注意力层实现离散不变性和零样本超分辨率。

Result: 在基准PDE任务中，GITO表现优于现有的基于变换器的神经算子。
...
### [131] [Few-Shot Learning for Industrial Time Series: A Comparative Analysis Using the Example of Screw-Fastening Process Monitoring](https://arxiv.org/abs/2506.13909)
*Xinyuan Tu,Haocheng Zhang,Tao Chengxu,Zuyi Chen*

Main category: cs.LG

TL;DR: 该论文研究了工业时间序列数据中的少样本学习（FSL），提出了一种标签感知的情节采样器，并在螺丝紧固过程监测中验证了其有效性。
...
Method: 采用两种FSL范式（Prototypical Network和MAML）与三种骨干网络（1D CNN、InceptionTime和Moment）结合，并引入标签感知采样器。

Result: InceptionTime + Prototypical Network组合在10-shot、3-way评估中表现最佳，F1分数达0.944（多类）和0.935（多标签），优于大模型Moment。

Conclusion: 轻量级CNN架构结合简单度量学习在数据稀缺时表现更优，挑战了大模型总是更优的假设。
...
### [132] [Logical Expressiveness of Graph Neural Networks with Hierarchical Node Individualization](https://arxiv.org/abs/2506.13911)
*Arie Soeteman,Balder ten Cate*

Main category: cs.LG

TL;DR: HEGNNs是一种基于层次化节点个性化的图神经网络扩展，能够区分图同构，实验证明其优于传统GNN架构。
...
Method: 通过层次化节点个性化扩展GNN，结合逻辑特征和局部同态计数特征。
...
### [133] [Branching Stein Variational Gradient Descent for sampling multimodal distributions](https://arxiv.org/abs/2506.13916)
*Isaias Banales,Arturo Jaramillo,Heli Ricalde Guerrero*

Main category: cs.LG

TL;DR: 提出了一种基于粒子的变分推理方法BSVGD，通过随机分支机制扩展了SVGD，适用于多模态分布。


<details>
  <summary>Details</summary>
Motivation: 解决传统SVGD在多模态分布中探索能力不足的问题。

Method: 引入随机分支机制，扩展SVGD算法，提供理论收敛保证。

Result: 数值实验验证了BSVGD的适用性，性能优于SVGD。
...
### [134] [Adaptive Guidance Accelerates Reinforcement Learning of Reasoning Models](https://arxiv.org/abs/2506.13923)
*Vaskar Nath,Elaine Lau,Anisha Gunjal,Manasi Sharma,Nikhil Baharte,Sean Hendryx*

Main category: cs.LG

TL;DR: RLVR训练通过压缩pass@k到pass@1和“能力增益”提升模型性能，新问题解决主要依赖自蒸馏。Guide算法通过动态引入提示优化训练，显著提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究RLVR训练如何通过不同机制提升模型解决新问题的能力，并探索优化训练算法。

Method: 通过实验分析RLVR机制，提出Guide算法动态引入提示并调整重要性采样比。

Result: Guide算法在7B和32B模型上提升泛化能力，数学基准测试平均提升4%。

Conclusion: Guide算法有效优化训练过程，显著提升模型性能，为RLVR训练提供新思路。
...
### [135] [ReinDSplit: Reinforced Dynamic Split Learning for Pest Recognition in Precision Agriculture](https://arxiv.org/abs/2506.13935)
*Vishesh Kumar Tanwar,Soumik Sarkar,Asheesh K. Singh,Sajal K. Das*

Main category: cs.LG

TL;DR: ReinDSplit是一种基于强化学习的分布式机器学习框架，动态调整深度神经网络的切分点，以优化异构农业设备中的资源利用和模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统的一刀切分割学习框架在农业生态系统中效率低下，无法适应设备异构性，导致资源浪费和性能下降。

Method: 采用强化学习（Q-learning）动态选择切分点，平衡设备负载和延迟，确保资源高效利用。

Result: 在三个昆虫分类数据集上测试，MobileNetV2达到94.31%的准确率。

Conclusion: ReinDSplit通过强化学习实现了分割学习在异构环境中的高效、隐私保护和可扩展性。
...
### [136] [Toward Explainable Offline RL: Analyzing Representations in Intrinsically Motivated Decision Transformers](https://arxiv.org/abs/2506.13958)
*Leonardo Guiducci,Antonio Rizzo,Giovanna Maria Dimitri*

Main category: cs.LG

TL;DR: 本文提出了一种后验可解释性框架，分析内在动机如何影响弹性决策变换器（EDT）的嵌入表示，揭示了不同动机变体如何形成不同的表征结构。


<details>
  <summary>Details</summary>
Motivation: 研究内在动机在EDT中如何通过嵌入表征影响决策性能，填补了现有研究中对表征机制理解的空白。

Method: 通过统计分析嵌入属性（如协方差结构、向量大小和正交性），比较不同内在动机变体的表征结构。

Result: 发现不同内在动机变体形成不同的表征结构，且嵌入指标与性能之间存在环境特定的相关性。

Conclusion: 内在动机不仅是探索奖励，还作为一种表征先验，通过生物合理的方式塑造嵌入几何结构，从而提升决策能力。
...
### [137] [Membership Inference Attacks as Privacy Tools: Reliability, Disparity and Ensemble](https://arxiv.org/abs/2506.13972)
*Zhiqi Wang,Chengyu Zhang,Yuetian Chen,Nathalie Baracaldo,Swanand Kadhe,Lei Yu*

Main category: cs.LG

TL;DR: 本文研究了成员推理攻击（MIAs）在隐私评估中的差异性，并提出了一种基于覆盖率和稳定性分析的新框架，以及一个集成框架来解决这些差异性问题。


<details>
  <summary>Details</summary>
Motivation: 现有MIA研究主要关注性能指标（如AUC、准确率等），而忽视了不同攻击方法之间的差异性，这对隐私评估的可靠性和完整性至关重要。

Method: 通过覆盖率和稳定性分析框架系统研究MIA的差异性，并提出一个集成框架，结合三种策略以利用现有MIA的优势。

Result: 实验揭示了MIA中的显著差异性及其潜在原因，集成框架能够构建更强大的攻击并提供更全面的隐私评估方法。

Conclusion: 本文提出的框架不仅提升了攻击能力，还为隐私评估提供了更稳健和全面的方法论。
...
### [138] [Constant Stepsize Local GD for Logistic Regression: Acceleration by Instability](https://arxiv.org/abs/2506.13974)
*Michael Crawshaw,Blake Woodworth,Mingrui Liu*

Main category: cs.LG

TL;DR: 论文分析了局部梯度下降在逻辑回归中的收敛性，允许任意步长η>0，优于现有要求η≤1/K的限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法对异构目标的局部梯度下降分析要求步长η≤1/K，限制了灵活性。本文旨在放宽这一限制，研究任意步长下的收敛性。

Method: 使用局部梯度下降方法，针对逻辑回归和可分离的异构数据，分析任意步长η>0的收敛行为。

Result: 在初始不稳定阶段后，收敛速率为O(1/ηKR)，优于现有一般平滑凸目标的O(1/R)速率。

Conclusion: 放宽步长限制后，局部梯度下降在逻辑回归中仍能高效收敛，且初始不稳定阶段与步长和局部更新相关。
...
### [139] [HAELT: A Hybrid Attentive Ensemble Learning Transformer Framework for High-Frequency Stock Price Forecasting](https://arxiv.org/abs/2506.13981)
*Thanh Dan Bui*

Main category: cs.LG

TL;DR: HAELT框架结合了ResNet降噪模块、时序自注意力机制和混合LSTM-Transformer核心，用于高频股票价格预测，在AAPL数据上表现优异。
...
Method: 提出HAELT框架，结合ResNet降噪、时序自注意力机制和混合LSTM-Transformer核心，并动态集成。

Result: 在AAPL数据上测试，HAELT取得最高F1分数，能有效识别价格涨跌。

Conclusion: HAELT在高频金融预测和算法交易中具有实用潜力。
...
### [140] [Quantum-Informed Contrastive Learning with Dynamic Mixup Augmentation for Class-Imbalanced Expert Systems](https://arxiv.org/abs/2506.13987)
*Md Abrar Jahin,Adiba Abid,M. F. Mridha*

Main category: cs.LG

TL;DR: QCL-MixNet是一种量子启发的对比学习框架，结合动态mixup，用于解决类别不平衡问题，显著优于现有方法。
...
Method: QCL-MixNet结合量子纠缠层、动态mixup策略和混合损失函数，增强少数类表示并提升分类性能。

Result: 在18个真实数据集上，QCL-MixNet在macro-F1和召回率上显著优于20种现有方法。

Conclusion: QCL-MixNet为表格数据不平衡处理设立了新基准，具有理论支持和实际优势。
...
### [141] [AssistedDS: Benchmarking How External Domain Knowledge Assists LLMs in Automated Data Science](https://arxiv.org/abs/2506.13992)
...
TL;DR: AssistedDS基准测试评估LLMs在表格预测任务中处理领域知识的能力，发现LLMs容易不加批判地采纳信息，对抗性内容显著影响性能，且在处理时间序列数据和分类变量时易出错。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否能够像人类数据科学家一样批判性地利用外部领域知识，填补自动化数据科学工作流的不足。

Method: 引入AssistedDS基准，结合合成数据集和真实Kaggle竞赛数据，提供有益和对抗性文档，评估LLMs在数据清理、特征工程和模型选择中的表现。

Result: LLMs常不加批判地采纳信息，对抗性内容显著降低性能；有益指导难以抵消对抗性影响；在处理时间序列和分类变量时易出错。

Conclusion: 当前LLMs在批判性评估和利用专家知识方面存在明显不足，需开发更鲁棒的知识感知自动化数据科学系统。
...
### [142] [Arctic Long Sequence Training: Scalable And Efficient Training For Multi-Million Token Sequences](https://arxiv.org/abs/2506.13996)
*Stas Bekman,Samyam Rajbhandari,Michael Wyatt,Jeff Rasley,Tunji Ruwase,Zhewei Yao,Aurick Qiao,Yuxiong He*

Main category: cs.LG

TL;DR: ALST（Arctic Long Sequence Training）通过单GPU和多GPU内存优化，支持Hugging Face模型训练超长序列（如500K到15M tokens），解决了开源社区中长序列训练的挑战。


<details>
  <summary>Details</summary>
Motivation: 长序列在RAG、长文档摘要等应用中至关重要，但开源社区缺乏系统支持，现有方案难以在Hugging Face模型上实现长序列训练。

Method: ALST结合注意力无关的单GPU和多GPU内存优化技术，支持多种Hugging Face模型训练超长序列。
...
### [143] [Taming Polysemanticity in LLMs: Provable Feature Recovery via Sparse Autoencoders](https://arxiv.org/abs/2506.14002)
*Siyu Chen,Heejune Sheen,Xuyuan Xiong,Tianhao Wang,Zhuoran Yang*

Main category: cs.LG

TL;DR: 本文提出了一种基于统计框架的稀疏自编码器（SAE）训练算法，通过“偏置适应”技术解决现有方法缺乏理论保证和实际局限性的问题，并在理论和实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏自编码器（SAE）训练算法缺乏严格的数学保证，且存在超参数敏感性和不稳定性等实际限制。本文旨在解决这些问题，为SAE提供理论支持。

Method: 提出了一种新的统计框架，将多义特征建模为单义概念的稀疏混合，并基于此开发了“偏置适应”训练算法。进一步提出了改进的实证变体Group Bias Adaptation（GBA）。

Result: 理论证明了该算法在输入数据符合统计模型时能正确恢复所有单义特征。实验表明GBA在1.5亿参数的大型语言模型上优于基准方法。

Conclusion: 本文为SAE训练提供了首个具有理论恢复保证的算法，推动了通过增强机制可解释性开发更透明、可信赖的AI系统。
...
### [144] [Unlearning Isn't Invisible: Detecting Unlearning Traces in LLMs from Model Outputs](https://arxiv.org/abs/2506.14003)
...
<details>
  <summary>Details</summary>
Motivation: 机器遗忘在保护数据隐私、版权执行和减少社会技术危害中起关键作用，但作者发现遗忘后模型会留下可检测的痕迹，可能带来新的风险。

Method: 通过监督分类器分析模型输出和内部激活，识别遗忘痕迹，并验证其在各种输入下的可检测性。

Result: 实验表明，遗忘痕迹在模型输出和内部激活中显著存在，分类器检测准确率超过90%，即使输入与遗忘无关，痕迹仍可被检测。

Conclusion: 遗忘行为会留下可测量的痕迹，可能被逆向工程还原遗忘信息，揭示了机器遗忘的新风险。
...
### [145] [Bures-Wasserstein Flow Matching for Graph Generation](https://arxiv.org/abs/2506.14020)
...
<details>
  <summary>Details</summary>
Motivation: 现有图生成方法（如扩散和基于流的模型）假设数据在欧几里得空间中，独立建模节点和边的演化，忽略了图的非欧几里得结构和互联模式，导致采样收敛性风险。

Method: 通过将图表示为MRF参数化的连接系统，利用MRF对象间的最优传输位移设计概率路径，提出BWFlow框架，支持连续和离散流匹配算法。

Result: 实验表明，BWFlow在普通图生成和2D/3D分子生成中表现优异，具有稳定训练和采样收敛性。
...
### [146] [Robust Physics-Informed Neural Network Approach for Estimating Heterogeneous Elastic Properties from Noisy Displacement Data](https://arxiv.org/abs/2506.14036)
*Tatthapong Srikitrungruang,Sina Aghaee Dabaghan Fard,Matthew Lemon,Jaesung Lee,Yuxiao Zhou*

Main category: cs.LG

TL;DR: 提出了一种基于物理信息神经网络的逆弹性参数估计方法（IE-PINN），用于从噪声位移数据中稳健重建弹性参数分布。
...
Method: IE-PINN采用三种神经网络分别建模位移场、应变场和弹性参数分布，并引入两阶段估计策略和多种创新技术（如位置编码、正弦激活函数等）。

Result: 实验表明，IE-PINN能有效克服现有方法的局限性，在强噪声条件下仍能准确估计绝对尺度弹性参数。

Conclusion: 该方法在临床成像诊断和机械特性表征等领域具有重要应用潜力。
...
### [147] [Load Balancing Mixture of Experts with Similarity Preserving Routers](https://arxiv.org/abs/2506.14038)
*Nabil Omi,Siddhartha Sen,Ali Farhadi*

Main category: cs.LG

TL;DR: 提出一种新的负载均衡损失函数，用于稀疏混合专家模型（MoE），通过保持令牌间关系结构，提高训练一致性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有负载均衡机制可能导致路由行为不一致，模型学习冗余知识，限制了性能。
...
### [148] [Scientifically-Interpretable Reasoning Network (ScIReN): Uncovering the Black-Box of Nature](https://arxiv.org/abs/2506.14054)
*Joshua Fan,Haodi Xu,Feng Tao,Md Nasim,Marc Grimson,Yiqi Luo,Carla P. Gomes*

Main category: cs.LG

TL;DR: ScIReN是一个结合可解释神经网络和基于过程的模型的透明框架，用于科学发现。
...
Method: 使用可解释编码器预测科学参数，并通过可微分基于过程的解码器预测输出。

Result: 在碳流动和生态系统呼吸任务中，ScIReN预测准确性优于黑盒模型，并提供科学解释。

Conclusion: ScIReN在保持科学解释性的同时提高了预测性能，揭示了新的科学机制。
...
### [149] [A Regret Perspective on Online Selective Generation](https://arxiv.org/abs/2506.14067)
*Minjae Lee,Yoonjae Jung,Sangdon Park*

Main category: cs.LG

TL;DR: 论文提出了一种在线学习算法，用于在部分反馈下进行选择性生成，以减少生成模型的幻觉效应。


<details>
  <summary>Details</summary>
Motivation: 生成模型的虚假回答引发担忧，选择性生成（即不确定时拒绝回答）是控制幻觉的有效方法，但目前缺乏在非随机环境和部分反馈下的学习方法。

Method: 将选择性生成问题转化为多臂老虎机问题，利用反馈解锁技术提升收敛速度，并连接老虎机的遗憾保证与选择性生成的假发现率（FDR）保证。

Result: 算法在多样化数据环境中有效控制了FDR，同时保持了合理的选择效率（非拒绝回答的比例）。

Conclusion: 提出的在线选择性生成算法在部分反馈下表现优异，为控制生成模型的幻觉提供了实用解决方案。
...
### [150] [Comprehensive Verilog Design Problems: A Next-Generation Benchmark Dataset for Evaluating Large Language Models and Agents on RTL Design and Verification](https://arxiv.org/abs/2506.14074)
*Nathaniel Pinckney,Chenhui Deng,Chia-Tung Ho,Yun-Da Tsai,Mingjie Liu,Wenfei Zhou,Brucek Khailany,Haoxing Ren*

Main category: cs.LG

TL;DR: CVDP是一个新的硬件设计基准数据集，包含783个问题，覆盖13个任务类别，旨在推动LLM和智能体在硬件设计领域的研究。


<details>
  <summary>Details</summary>
Motivation: 当前模型在硬件设计自动化方面的能力有限，CVDP旨在提供一个更具挑战性和现实性的基准，以促进相关研究。

Method: CVDP包含非智能体和智能体任务格式，使用开源工具和模型评分基础设施进行评估，包括BLEU和基于LLM的评分。

Result: 现有模型在代码生成任务中的最高通过率仅为34%，智能体任务（如RTL重用和验证）尤为困难。
...
### [151] [Multi-Scale Finetuning for Encoder-based Time Series Foundation Models](https://arxiv.org/abs/2506.14087)
...
Method: 提出MSFT框架，通过显式建模多尺度信息来优化微调过程。

Result: 实验表明，MSFT在多种TSFMs上表现优于传统微调方法和现有深度学习方法。

Conclusion: MSFT能有效提升TSFMs在下游任务中的性能，是一种通用且高效的微调框架。
...
### [152] [Transformers Learn Faster with Semantic Focus](https://arxiv.org/abs/2506.14095)
*Parikshit Ram,Kenneth L. Clarkson,Tim Klinger,Shashanka Ubaru,Alexander G. Gray*

Main category: cs.LG

TL;DR: 研究发现输入依赖的稀疏注意力模型比标准注意力模型收敛更快且泛化更好，而输入无关的稀疏注意力模型无此优势。理论分析解释了这一现象，并验证了语义聚焦（输入依赖稀疏注意力）的优越性。


<details>
  <summary>Details</summary>
Motivation: 探索稀疏注意力在Transformer中的学习能力和泛化性能，而非仅关注效率。

Method: 通过实证研究多种注意力机制，结合理论分析稀疏注意力对软最大稳定性和损失函数Lipschitz性质的影响。

Result: 输入依赖稀疏注意力模型表现更优，输入无关稀疏注意力模型无显著优势。

Conclusion: 语义聚焦（输入依赖稀疏注意力）能加速学习并提升泛化性能，理论分析支持这一结论。
...
### [153] [Toward a Graph Foundation Model: Pre-Training Transformers With Random Walks](https://arxiv.org/abs/2506.14098)
*Ziyuan Tang,Jie Chen*

Main category: cs.LG

TL;DR: 本文提出了一种基于Transformer的图基础模型，通过随机游走表示节点，解决了序列模型编码不同大小和领域图的挑战。


<details>
  <summary>Details</summary>
Motivation: 探索是否能为图数据构建类似GPT的基础模型，利用预训练和Transformer架构处理多样化的图数据。

Method: 将节点表示为多个随机游走，利用Transformer从序列中提取节点、边和图表示，并设计了新的上下文预测损失函数。

Result: 理论分析了随机游走的表达能力，展示了模型在预训练和下游任务中的潜力。

Conclusion: 该模型为图结构数据的处理和推理提供了基础，展示了图基础模型的可行性。
...
### [154] [SKOLR: Structured Koopman Operator Linear RNN for Time-Series Forecasting](https://arxiv.org/abs/2506.14113)
*Yitian Zhang,Liheng Ma,Antonios Valkanas,Boris N. Oreshkin,Mark Coates*

Main category: cs.LG

TL;DR: 论文通过建立Koopman算子与线性RNN的联系，提出SKOLR方法，结合可学习的谱分解和MLP，实现了高效的动态系统预测。


<details>
  <summary>Details</summary>
Motivation: Koopman算子理论虽能线性化非线性动态系统，但其无限维特性限制了实际应用，因此需要找到有限维近似方法。

Method: 通过扩展状态（包含滞后观测）建立Koopman算子与线性RNN的等价性，提出SKOLR方法，结合谱分解和MLP实现结构化Koopman算子。

Result: 在多个预测基准和动态系统实验中，SKOLR表现出卓越性能。

Conclusion: 基于Koopman理论的设计简化了模型，同时实现了高效的动态系统预测。
...
### [155] [Evaluating Loss Functions for Graph Neural Networks: Towards Pretraining and Generalization](https://arxiv.org/abs/2506.14114)
*Khushnood Abbas,Ruizhe Hou,Zhou Wengang,Dong Shi,Niu Ling,Satyaki Nan,Alireza Abbasi*

Main category: cs.LG

TL;DR: 该论文通过大规模评估研究了GNN模型与多种损失函数的组合效果，发现混合损失函数在归纳任务中表现更优，GIN架构表现最佳，而MPNN表现较差。


<details>
  <summary>Details</summary>
Motivation: 研究GNN模型与损失函数组合在不同任务中的表现，填补了此前缺乏大规模评估的空白。

Method: 评估了7种GNN架构和30种损失函数（包括混合损失），在3个真实数据集上使用21种评估指标进行测试。

Result: 混合损失函数表现更优，GIN架构表现最佳，GAT在某些任务中表现突出，MPNN表现较差。

Conclusion: 混合损失函数和GIN架构的组合在归纳任务中表现最佳，为GNN设计和优化提供了实用指导。
...
### [156] [CLGNN: A Contrastive Learning-based GNN Model for Betweenness Centrality Prediction on Temporal Graphs](https://arxiv.org/abs/2506.14122)
*Tianming Zhang,Renbo Zhang,Zhengyi Yang,Yunjun Gao,Bin Cao,Jing Fan*

Main category: cs.LG

TL;DR: 论文提出了一种基于对比学习的图神经网络（CLGNN），用于高效准确地预测时间中心性（TBC），解决了现有方法在处理数据不平衡和忽略时间依赖性的问题。


<details>
  <summary>Details</summary>
Motivation: 时间中心性（TBC）的计算成本高且分布极不平衡，导致学习模型容易过拟合，无法准确预测中心节点。现有方法无法有效处理这些问题。

Method: CLGNN通过构建实例图保留路径有效性和时间顺序，使用双聚合机制（均值和边到节点的多头注意力）编码结构和时间特征，并引入稳定性聚类对比模块（KContrastNet）和回归模块（ValueNet）来缓解类别不平衡问题。

Result: CLGNN在多个基准测试中表现出色，比现有方法快663.7倍，MAE降低31.4倍，Spearman相关性提高16.7倍。

Conclusion: CLGNN是一种高效且准确的时间中心性预测方法，能够有效处理数据不平衡和时间依赖性问题。
...
### [157] [Less is More: Undertraining Experts Improves Model Upcycling](https://arxiv.org/abs/2506.14126)
...
<details>
  <summary>Details</summary>
Motivation: 挑战现有假设，即专家微调的改进会自然传播到后续模型合并阶段。

Method: 分析专家微调对模型合并的影响，并提出任务依赖的早期停止策略。

Result: 长时间微调导致合并性能下降，而早期停止策略显著改善性能。

Conclusion: 任务依赖的早期停止策略是优化模型合并性能的有效方法。
...
### [158] [Leveraging Predictive Equivalence in Decision Trees](https://arxiv.org/abs/2506.14143)
...
<details>
  <summary>Details</summary>
Motivation: 决策树因其可解释性被广泛使用，但存在预测等价性问题，即不同结构的决策树可能具有相同的决策边界，导致模型选择困难。

Method: 提出了一种布尔逻辑表示方法，避免了预测等价性，并应用于多个下游任务。

Result: 研究表明决策树对特征值缺失具有鲁棒性，解决了变量重要性量化问题，并提出了一种优化预测成本的算法。

Conclusion: 布尔逻辑表示方法有效解决了决策树的预测等价性问题，并在实际应用中表现出色。
...
### [159] [Common Benchmarks Undervalue the Generalization Power of Programmatic Policies](https://arxiv.org/abs/2506.14162)
*Amirhossein Rajabpour,Kiarash Aghakasiri,Sandra Zilles,Levi H. S. Lelis*

Main category: cs.LG

TL;DR: 论文指出，现有基准低估了程序化表示在分布外（OOD）问题上的泛化能力，通过简单调整神经网络策略的训练流程，可以使其泛化能力与程序化策略相当。


<details>
  <summary>Details</summary>
Motivation: 现有研究普遍认为程序化策略在OOD问题上泛化能力优于神经网络策略，但作者认为这一结论源于基准设计的不足。

Method: 分析了四篇文献的实验，通过简化神经网络架构、使用稀疏观察和调整奖励函数（如鼓励安全策略）来提升神经网络策略的OOD泛化能力。

Result: 实验表明，经过调整的神经网络策略在OOD问题上表现与程序化策略相当。

Conclusion: 建议设计更全面的基准问题，突出OOD泛化所需的概念（如算法结构），以更公平地评估不同策略的泛化能力。
...
### [160] [Light Aircraft Game : Basic Implementation and training results analysis](https://arxiv.org/abs/2506.14164)
*Hanzhong Cao*

Main category: cs.LG

TL;DR: 论文研究了多智能体强化学习（MARL）在部分可观测、合作-竞争的战斗环境LAG中的应用，比较了HAPPO和HASAC两种算法的性能。


<details>
  <summary>Details</summary>
Motivation: 探索在复杂战斗环境中，不同MARL算法的表现及其适应性。

Method: 使用HAPPO（基于PPO的分层算法）和HASAC（基于软演员-批评家的离策略方法）在LAG环境中进行实验。

Result: HASAC在无武器的简单协调任务中表现良好，而HAPPO在涉及导弹战斗的动态场景中更具适应性。

Conclusion: 研究揭示了在MARL中，策略内和策略外方法在不同任务中的权衡。
...
### [161] [Structured and Informed Probabilistic Modeling with the Thermodynamic Kolmogorov-Arnold Model](https://arxiv.org/abs/2506.14167)
*Prithvi Raj*

Main category: cs.LG

TL;DR: 将Kolmogorov-Arnold表示定理应用于生成建模，通过逆变换采样将其内函数重新解释为概率空间之间的马尔可夫核。提出了一种可解释、易设计且高效的生成模型。


<details>
  <summary>Details</summary>
Motivation: 将经典表示定理与现代概率建模结合，平衡训练稳定性、推理速度以及生成样本的质量和多样性。

Method: 结合Kolmogorov-Arnold网络生成器和独立能量基先验，通过最大似然训练。引入混合分布和Langevin蒙特卡洛方法以提升灵活性。

Result: 模型支持快速推理，并能在训练前融入先验知识，提高学习效率和样本质量。先验可恢复和可视化。

Conclusion: 该方法成功连接了经典表示定理与现代生成建模，同时解决了先验与后验不匹配的问题。
...
### [162] [A Variational Information Theoretic Approach to Out-of-Distribution Detection](https://arxiv.org/abs/2506.14194)
*Sudeepta Mondal,Zhuolin Jiang,Ganesh Sundaramoorthi*

Main category: cs.LG

TL;DR: 提出了一种基于信息论的神经网络OOD检测特征构建理论，通过KL散度和信息瓶颈优化特征分布，并预测了优于现有方法的新特征。


<details>
  <summary>Details</summary>
Motivation: 解决神经网络在分布外（OOD）检测中特征构建的理论基础问题，提升检测性能。

Method: 引入信息论损失函数，结合KL散度和信息瓶颈，通过变分优化方法生成OOD特征。
...
### [163] [DiffusionBlocks: Blockwise Training for Generative Models via Score-Based Diffusion](https://arxiv.org/abs/2506.14202)
*Makoto Shing,Takuya Akiba*

Main category: cs.LG

TL;DR: 提出DiffusionBlocks框架，通过将神经网络块视为扩散过程中的去噪操作，实现内存高效训练。
...
Result: 在生成任务中实现内存显著减少，性能与传统反向传播相当。
...
### [164] [TriGuard: Testing Model Safety with Attribution Entropy, Verification, and Drift](https://arxiv.org/abs/2506.14217)
*Dipesh Tharu Mahato,Rohan Poudel,Pramod Dhungana*

Main category: cs.LG

TL;DR: TriGuard是一个统一的安全评估框架，结合形式化鲁棒性验证、显著性集中度量化和解释稳定性评分，揭示模型准确性与可解释性之间的不匹配。
...
Result: 实验表明TriGuard能发现神经网络推理中的脆弱性，且熵正则化训练可减少解释漂移。
...
### [165] [Can Large Language Models Improve Spectral Graph Neural Networks?](https://arxiv.org/abs/2506.14220)
*Kangkang Lu,Yanhua Yu,Zhiyong Huang,Tat-Seng Chua*

Main category: cs.LG

TL;DR: 本文提出了一种利用大型语言模型（LLMs）估计图同质性，并以此指导SGNN滤波器设计的新方法，显著提升了SGNN的性能。


<details>
  <summary>Details</summary>
Motivation: 在标签稀缺条件下，SGNN可能学习到次优滤波器，导致性能下降。LLMs的成功激发了探索其在GNN领域的潜力，以解决SGNN的局限性。

Method: 通过LLM估计图的同质性，生成同质性感知先验，并将其注入滤波器系数，以更好地适应图的拓扑结构。

Result: 在基准数据集上的实验表明，该方法在多种图结构下均优于现有基线，且计算和成本开销极小。

Conclusion: LLMs可以有效增强SGNN的表达能力和适应性，尤其在标签稀缺条件下表现优异。
...
### [166] [Convergence-Privacy-Fairness Trade-Off in Personalized Federated Learning](https://arxiv.org/abs/2506.14251)
*Xiyu Zhao,Qimei Cui,Weicai Li,Wei Ni,Ekram Hossain,Quan Z. Sheng,Xiaofeng Tao,Ping Zhang*

Main category: cs.LG

TL;DR: DP-Ditto是一种在差分隐私保护下的个性化联邦学习（PFL）方法，分析了隐私保证、模型收敛和性能公平性之间的权衡，并在实验中表现优于现有PFL模型。
...
### [167] [RL-Obfuscation: Can Language Models Learn to Evade Latent-Space Monitors?](https://arxiv.org/abs/2506.14261)
*Rohan Gupta,Erik Jenner*

Main category: cs.LG

TL;DR: 研究发现，通过强化学习微调的LLM可以绕过潜在空间监控器，但整体性监控器仍具鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM是否能学会规避潜在空间监控器，以评估其安全性。

Method: 引入RL-Obfuscation方法，通过强化学习微调LLM以绕过监控器。

Result: 令牌级监控器易受攻击，而整体性监控器（如最大池化或基于注意力的探针）仍稳健。

Conclusion: LLM能学会规避监控器，但需开发更鲁棒的监控方法。
...
### [168] [Knowledge Adaptation as Posterior Correction](https://arxiv.org/abs/2506.14262)
...
Result: 研究表明，更准确的后验会导致更小的修正，从而加快适应速度。

Conclusion: 后验修正可以作为一种自然机制，帮助机器快速适应。
...
### [169] [Towards Robust Learning to Optimize with Theoretical Guarantees](https://arxiv.org/abs/2506.14263)
*Qingyu Song,Wei Lin,Juncheng Wang,Hong Xu*

Main category: cs.LG

TL;DR: 论文探讨了学习优化（L2O）技术在分布外（OOD）场景下的性能与鲁棒性，提出了理论证明和新模型。


<details>
  <summary>Details</summary>
Motivation: 现有L2O方法在OOD场景中缺乏理论支持，本文旨在填补这一空白。

Method: 提出了一种简洁的梯度特征构建方法和基于梯度的历史建模方法。

Result: 新模型在InD和OOD场景下均优于基线，收敛速度提升高达10倍。

Conclusion: 论文通过理论证明和新模型设计，提升了L2O在OOD场景中的鲁棒性和性能。
...
### [170] [Improving LoRA with Variational Learning](https://arxiv.org/abs/2506.14280)
*Bai Cong,Nico Daheim,Yuesong Shen,Rio Yokota,Mohammad Emtiyaz Khan,Thomas Möllenhoff*

Main category: cs.LG

TL;DR: IVON是一种变分算法，用于改进LoRA微调，显著提升多项指标（如准确率和ECE），同时计算成本与AdamW相当。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯方法虽能改进LoRA微调的校准性，但对其他指标（如准确率）效果有限，且增加计算开销。IVON旨在解决这些问题。

Method: 使用IVON变分算法，结合简单的后验剪枝技术，对大规模LLM（如Llama和Qwen系列）进行微调。

Result: 在Llama-3.2-3B模型上，IVON比AdamW提升准确率1.3%，降低ECE 5.4%，优于其他贝叶斯方法。

Conclusion: IVON能有效改进LoRA微调，提供高性能且计算成本低的解决方案。
...
### [171] [Equivariance Everywhere All At Once: A Recipe for Graph Foundation Models](https://arxiv.org/abs/2506.14291)
*Ben Finkelshtein,İsmail İlkan Ceylan,Michael Bronstein,Ron Levie*

Main category: cs.LG

TL;DR: 本文提出了一种设计图基础模型的方法，强调对称性（节点和标签的置换等变性、特征的置换不变性）的重要性，并验证了其通用性和零样本性能。


<details>
  <summary>Details</summary>
Motivation: 解决图机器学习模型因任务和数据集特定设计而缺乏通用性的问题，探索如何构建适用于任意图和特征的图基础模型。

Method: 通过系统研究图基础模型需满足的对称性，设计线性变换层（满足节点和标签置换等变性、特征置换不变性），并证明其通用逼近能力。

Result: 在29个真实节点分类数据集上验证了模型的零样本性能和随训练图数量增加的改进。

Conclusion: 提出的图基础模型设计方法具有通用性和可扩展性，适用于节点属性预测任务。
...
### [172] [Fair for a few: Improving Fairness in Doubly Imbalanced Datasets](https://arxiv.org/abs/2506.14306)
*Ata Yalcin,Asli Umay Ozturk,Yigit Sever,Viktoria Pauw,Stephan Hachinger,Ismail Hakki Toroslu,Pinar Karagoz*

Main category: cs.LG

TL;DR: 论文探讨了在标签和敏感属性双重不平衡的数据集上实现公平性的挑战，并提出了一种多标准解决方案。


<details>
  <summary>Details</summary>
Motivation: 机器学习中的公平性是一个重要问题，但现有方法在双重不平衡数据集上表现不佳。

Method: 通过探索性分析揭示限制，并提出基于多标准的采样和分布优化方案。

Result: 提出的方法在公平性和分类准确性上表现更优。

Conclusion: 多标准方法为双重不平衡数据集上的公平性提供了有效解决方案。
...
### [173] [IntelliLung: Advancing Safe Mechanical Ventilation using Offline RL with Hybrid Actions and Clinically Aligned Rewards](https://arxiv.org/abs/2506.14375)
*Muhammad Hamza Yousuf,Jason Li,Sahar Vahdati,Raphael Theilen,Jakob Wittenstein,Jens Lehmann*

Main category: cs.LG

TL;DR: 论文提出了一种优化离线强化学习（RL）的方法，用于机械通气（MV）控制，解决了混合动作空间的挑战，并引入了临床相关的奖励函数。


<details>
  <summary>Details</summary>
Motivation: 机械通气（MV）设置优化复杂且易出错，现有方法因动作空间的离散化而受限。

Method: 提出动作空间缩减优化，并改进离线RL算法（IQL和EDAC）以直接处理混合动作空间，同时设计基于临床目标的奖励函数。

Result: AI辅助的MV优化可提升患者安全并实现个性化支持。
...
### [174] [ResNets Are Deeper Than You Think](https://arxiv.org/abs/2506.14386)
*Christian H. X. Ali Mehmeti-Göpel,Michael Wand*

Main category: cs.LG

TL;DR: 残差连接在现代神经网络中广泛使用，其优势不仅在于优化，还在于其独特的函数空间和归纳偏置。


<details>
  <summary>Details</summary>
Motivation: 研究残差网络与普通前馈网络性能差异的根本原因，提出残差网络的优势不仅源于优化，还与其函数空间和归纳偏置有关。

Method: 设计了一个后训练对比实验，隔离了泛化性能和可训练性的影响，比较可变深度架构与固定深度网络的性能。

Result: 可变深度架构（类似ResNet）在优化无关的情况下仍优于固定深度网络。

Conclusion: 残差连接的性能优势源于其与自然数据结构一致的归纳偏置，而不仅仅是优化效果。
...
### [175] [Enclosing Prototypical Variational Autoencoder for Explainable Out-of-Distribution Detection](https://arxiv.org/abs/2506.14390)
*Conrad Orglmeister,Erik Bochinski,Volker Eiselein,Elvira Fleig*

Main category: cs.LG

TL;DR: 论文提出了一种结合原型变分模型和自编码器的OOD检测方法，通过定义ID区域和引入限制损失，提升了模型的可靠性和解释性。
...
Result: 在常见OOD检测基准和实际铁路应用数据集上表现优异，优于现有方法。

Conclusion: 该方法通过提升模型的解释性和OOD检测能力，为安全相关应用提供了可靠支持。
...
### [176] [HiLight: A Hierarchical Reinforcement Learning Framework with Global Adversarial Guidance for Large-Scale Traffic Signal Control](https://arxiv.org/abs/2506.14391)
*Yaqiao Zhu,Hongkai Wen,Geyong Min,Man Luo*

Main category: cs.LG

TL;DR: HiLight是一个分层强化学习框架，通过全局对抗性指导解决大规模交通信号控制问题，结合高层元策略和低层子策略，实现高效协调。
...
Method: HiLight采用分层结构，高层元策略分区生成子目标，低层子策略控制单个交叉口，并通过对抗训练机制提升协调性。

Result: 实验表明，HiLight在大规模场景中表现优越，并在不同规模的基准测试中保持竞争力。
...
### [177] [One Size Fits None: Rethinking Fairness in Medical AI](https://arxiv.org/abs/2506.14400)
...
TL;DR: 该论文探讨了机器学习模型在临床决策中的应用，强调了对患者子群进行性能评估的重要性，以确保公平性和透明度。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的医疗数据集通常存在噪声、不完整和不平衡的问题，导致模型在不同患者子群中的性能差异，可能加剧边缘化群体的劣势。

Method: 通过分析多个医疗预测任务，研究模型性能如何随患者特征变化，并强调子群级评估的必要性。

Result: 研究发现，尽管模型整体性能良好，但在子群级别存在显著差异，需在临床实践中考虑这些差异。

Conclusion: 论文提倡在医疗机器学习模型的开发和部署中关注子群敏感性，以促进公平性和透明度。
...
### [178] [Adaptive Reinforcement Learning for Unobservable Random Delays](https://arxiv.org/abs/2506.14411)
*John Wikman,Alexandre Proutiere,David Broman*

Main category: cs.LG

TL;DR: 论文提出了一种名为“交互层”的通用框架，用于处理强化学习中不可观测和时变的延迟问题，并开发了ACDA算法，显著优于现有方法。
...
Method: 引入交互层框架，生成未来动作矩阵以应对延迟和丢包；基于此开发ACDA算法，动态适应延迟模式。
...
### [179] [Unsupervised Skill Discovery through Skill Regions Differentiation](https://arxiv.org/abs/2506.14420)
*Ting Xiao,Jiakun Zheng,Rushuai Yang,Kang Xu,Qiaosheng Zhang,Peng Liu,Chenjia Bai*

Main category: cs.LG

TL;DR: 提出了一种新的技能发现目标，通过最大化技能间状态密度的差异，结合条件自编码器和内在奖励，提升了无监督强化学习的效果。


<details>
  <summary>Details</summary>
Motivation: 解决熵探索和互信息估计在大规模状态空间中的局限性，提升技能多样性和探索能力。
...
### [180] [MoORE: SVD-based Model MoE-ization for Conflict- and Oblivion-Resistant Multi-Task Adaptation](https://arxiv.org/abs/2506.14436)
*Shen Yuan,Yin Zheng,Taifeng Wang,Binbin Liu,Hongteng Xu*

Main category: cs.LG

TL;DR: 提出了一种名为MoORE的新方法，通过SVD分解和可学习路由器调整预训练模型的权重矩阵，以解决多任务适应中的任务冲突和遗忘问题。
...
Conclusion: MoORE方法在多任务适应中表现出色，解决了任务冲突和遗忘问题。
...
### [181] [sHGCN: Simplified hyperbolic graph convolutional neural networks](https://arxiv.org/abs/2506.14438)
*Pol Arévalo,Alexis Molina,Álvaro Ciudad*

Main category: cs.LG

TL;DR: 本文提出简化双曲神经网络中的关键操作，以解决其计算效率和精度问题，显著提升运行时间和性能。
...
Conclusion: 简化操作使双曲神经网络适用于更广泛的应用场景。
...
### [182] [A General Framework for Off-Policy Learning with Partially-Observed Reward](https://arxiv.org/abs/2506.14439)
*Rikiya Takehi,Masahiro Asami,Kosuke Kawakami,Yuta Saito*

Main category: cs.LG

TL;DR: 本文研究了在部分观测目标奖励的情况下，如何利用密集观测的次级奖励来优化决策策略，提出了HyPeR方法，并在理论和实验上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 在部分观测目标奖励的情况下，现有离策略学习（OPL）效果显著下降，而仅依赖次级奖励可能导致策略学习不佳。因此，需要一种方法能同时利用目标奖励和次级奖励来优化策略。

Method: 提出了HyPeR方法，通过结合部分观测的目标奖励和密集观测的次级奖励，实现有效的策略优化。同时探讨了同时优化目标奖励和次级奖励的情况。

Result: 理论和实验表明，HyPeR在多种场景下优于现有方法，且同时优化两种奖励对目标奖励的优化也有帮助。

Conclusion: HyPeR为部分观测奖励的OPL问题提供了一种有效的解决方案，并展示了多目标优化的潜在优势。
...
### [183] [Detecting immune cells with label-free two-photon autofluorescence and deep learning](https://arxiv.org/abs/2506.14449)
...
TL;DR: 该论文提出了一种基于深度学习的无标记多光子显微镜（MPM）图像分析方法，用于免疫细胞分类，展示了其在提高无标记成像特异性方面的潜力。


<details>
  <summary>Details</summary>
Motivation: 无标记成像技术（如MPM）避免了复杂的染色过程，适用于体内成像，但其特异性有限。深度学习模型可以增强无标记图像的特异性，但此前在MPM中应用较少。

Method: 使用卷积神经网络（CNN）对无标记MPM图像中的免疫细胞进行分类，数据集包括5075个细胞的二元分类和3424个细胞的多类分类任务。

Result: 低复杂度的SqueezeNet架构在二元分类（ROC-AUC为0.89）和多类分类（F1得分为0.689）任务中表现良好，且模型对细胞外环境不敏感。

Conclusion: 深度学习模型可以显著提升无标记MPM的特异性，未来有望直接用于体内内窥镜成像中的免疫细胞检测。
...
### [184] [Dataset distillation for memorized data: Soft labels can leak held-out teacher knowledge](https://arxiv.org/abs/2506.14457)
*Freya Behrens,Lenka Zdeborová*

Main category: cs.LG

TL;DR: 研究发现，在数据集蒸馏中，学生模型通过教师的软标签可以从未直接观察到的记忆数据中获得非平凡准确率，甚至在纯记忆情况下也能恢复教师功能。


<details>
  <summary>Details</summary>
Motivation: 探讨数据集蒸馏中记忆信息的传递机制，尤其是在教师未泛化时学生如何从软标签中学习。

Method: 使用有限随机独立同分布数据集，分析学生在纯记忆情况下的学习能力，并研究温度参数的影响。

Result: 学生可以从软标签中学习到记忆数据的信息，甚至在某些情况下达到完美准确率，且这种现象受温度参数影响。

Conclusion: 数据集蒸馏中记忆信息的传递是可能的，且受温度参数调控，这一现象在不同网络结构和数据组成中普遍存在。
...
### [185] [A Model-Mediated Stacked Ensemble Approach for Depression Prediction Among Professionals](https://arxiv.org/abs/2506.14459)
...
Method: 使用堆叠集成学习方法，结合多个基础学习器和逻辑回归模型，分析来自Kaggle的抑郁症专业数据集。

Result: 模型在训练和测试数据上分别达到99.64%和98.75%的准确率，精确率、召回率和F1分数均超过98%。

Conclusion: 集成学习在心理健康分析中表现优异，对早期检测和干预策略具有潜在应用价值。
...
### [186] [Zeroth-Order Optimization is Secretly Single-Step Policy Optimization](https://arxiv.org/abs/2506.14460)
...
<details>
  <summary>Details</summary>
Motivation: 探索ZOO方法与RL中PO的联系，以改进ZOO算法的理论基础和实际效果。

Method: 通过数学证明ZOO与PO的等价性，并提出ZoAR算法，引入平均基线和查询重用技术。

Result: 理论分析表明ZoAR降低方差并加速收敛，实验验证其性能优于其他方法。
...
### [187] [Leveraging External Factors in Household-Level Electrical Consumption Forecasting using Hypernetworks](https://arxiv.org/abs/2506.14472)
*Fabien Bernier,Maxime Cordy,Yves Le Traon*

Main category: cs.LG

TL;DR: 本文提出了一种基于超网络架构的方法，通过结合外部因素（如天气指标）来提升全球电力消费预测模型的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测依赖于历史模式和时序依赖，但引入外部因素可能降低全局模型的性能。本文旨在解决这一问题。

Method: 采用超网络架构，根据每个消费者的特点调整模型权重，并结合天气指标、节假日等外部因素。

Result: 实验表明，超网络方法在结合外部因素时优于现有方法，显著降低了预测误差。

Conclusion: 超网络架构能有效提升全局电力消费预测模型的准确性，同时保留全局模型的优势。
...
### [188] [Train Once, Forget Precisely: Anchored Optimization for Efficient Post-Hoc Unlearning](https://arxiv.org/abs/2506.14515)
...
Method: FAMR通过最小化遗忘集的均匀预测损失，并使用ℓ2惩罚锚定模型参数，实现遗忘。
...
### [189] [Two-Player Zero-Sum Games with Bandit Feedback](https://arxiv.org/abs/2506.14518)
*Elif Yılmaz,Christos Dimitrakakis*

Main category: cs.LG

TL;DR: 论文研究了在未知支付矩阵下，通过带反馈的强盗算法学习纯策略纳什均衡的双人零和游戏，提出了两种算法：ETC-TPZSG和ETC-TPZSG-AE，并分析了其性能。


<details>
  <summary>Details</summary>
Motivation: 探索在对抗性游戏环境中应用ETC算法的可行性，特别是针对纯策略纳什均衡的学习。

Method: 提出ETC-TPZSG算法，并改进为ETC-TPZSG-AE，后者利用ε-纳什均衡性质通过动作对消除策略提高效率。

Result: 两种算法分别实现了O(Δ+√T)和O(log(TΔ²)/Δ)的实例依赖后悔上界。

Conclusion: ETC算法在对抗性游戏环境中表现良好，其后悔界与现有方法相当，并通过实例依赖分析提供了新见解。
...
### [190] [Towards Improved Research Methodologies for Industrial AI: A case study of false call reduction](https://arxiv.org/abs/2506.14521)
*Korbinian Pfab,Marcel Rothering*

Main category: cs.LG

TL;DR: 本文通过工业AI案例研究，揭示了当前AI研究方法的不足，并提出改进建议。
...
Method: 通过工业AI案例（自动光学检测中的误报减少）分析，识别并实验验证了七种常见弱点。

Result: 研究发现，当前最佳实践方法在该案例中会失败，并提出改进方向。

Conclusion: 呼吁研究者批判性评估方法，以确保应用AI研究的成功。
...
### [191] [Automated Decision-Making on Networks with LLMs through Knowledge-Guided Evolution](https://arxiv.org/abs/2506.14529)
...
Method: LLMNet通过一组代理构建图相关知识库，并利用RAG技术支持GNN的自动配置和优化，代理通过知识库交互提取任务和结构信息。

Result: 在三个图学习任务的十二个数据集中，LLMNet表现出色，验证了其自动设计GNN模型的有效性。

Conclusion: LLMNet为GNN的自动化设计提供了一种高效方法，显著减少了人工配置的需求。
...
### [192] [Aligning Evaluation with Clinical Priorities: Calibration, Label Shift, and Error Costs](https://arxiv.org/abs/2506.14540)
*Gerardo A. Flores,Alyssa H. Smith,Julia A. Fukuyama,Ashia C. Wilson*

Main category: cs.LG

TL;DR: 提出了一种基于校准阈值分类器的临床决策支持系统评估框架，考虑了类别分布不确定性和成本不对称性。
...
Result: 该评估方法简单易用，对临床部署条件敏感，优先选择校准且鲁棒的模型。

Conclusion: 新框架为临床决策支持系统提供了更实用且符合临床需求的评估标准。
...
### [193] [TGDPO: Harnessing Token-Level Reward Guidance for Enhancing Direct Preference Optimization](https://arxiv.org/abs/2506.14574)
*Mingkang Zhu,Xi Chen,Zhongdao Wang,Bei Yu,Hengshuang Zhao,Jiaya Jia*

Main category: cs.LG

TL;DR: 该论文提出了一种方法，将序列级的PPO分解为一系列基于token级的PPO问题，并利用token级奖励指导DPO，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以将token级奖励用于DPO，因为DPO是序列级的bandit问题。

Method: 将序列级PPO分解为token级PPO问题，推导出token级最优策略和奖励，并基于此设计DPO的损失函数。

Result: 实验显示，该方法在多个基准测试中显著优于DPO，最高提升7.5个百分点。

Conclusion: 该方法成功解决了token级奖励指导DPO的挑战，并取得了显著性能提升。
...
### [194] [Single-Example Learning in a Mixture of GPDMs with Latent Geometries](https://arxiv.org/abs/2506.14563)
*Jesse St. Amand,Leonardo Gizzi,Martin A. Giese*

Main category: cs.LG

TL;DR: GPDMM是一种结合GPDM和概率混合专家框架的模型，用于单样本学习人类运动数据，适用于数据有限且需模型可解释性的场景。


<details>
  <summary>Details</summary>
Motivation: 解决在数据有限（如医疗应用中）且需要模型可解释性时，人类运动建模的挑战。

Method: 结合多个GPDM，利用几何特征在单一潜在空间中编码多样化序列，实现分类和生成。

Result: 在分类准确性和生成能力上表现优异，优于LSTM、VAE和Transformer。

Conclusion: GPDMM在单样本学习和模型可解释性方面具有显著优势，适用于特定应用场景。
...
### [195] [Object-Centric Neuro-Argumentative Learning](https://arxiv.org/abs/2506.14577)
...
<details>
  <summary>Details</summary>
Motivation: 随着深度学习在关键决策中的应用增加，对其安全性、可靠性和可解释性的担忧日益凸显。

Method: NAL架构结合神经和符号组件：前者通过对象中心学习将图像分割并编码为事实，后者应用ABA学习构建框架以支持图像预测。

Result: 在合成数据上的实验表明，NAL架构的性能与现有先进技术相当。
...
### [196] [SCISSOR: Mitigating Semantic Bias through Cluster-Aware Siamese Networks for Robust Classification](https://arxiv.org/abs/2506.14587)
...
<details>
  <summary>Details</summary>
Motivation: 研究揭示了语义分布不平衡导致的虚假语义相关性，影响了模型的鲁棒性，需解决这一问题。

Method: 提出SCISSOR方法，通过重新映射语义空间，抑制被用作捷径的潜在聚类，无需数据增强或重写。

Result: 在多个基准测试中，SCISSOR显著提升了模型性能，如GYAFC（+5.3 F1）、Yelp（+7.3 F1）、Chest-XRay（+7.7 F1）和Not-MNIST（+1 F1）。
...
### [197] [Deep Learning Surrogates for Real-Time Gas Emission Inversion](https://arxiv.org/abs/2506.14597)
*Thomas Newman,Christopher Nemeth,Matthew Jones,Philip Jonathan*

Main category: cs.LG

TL;DR: 提出了一种结合深度学习与蒙特卡洛算法的时空反演框架，用于实时推断动态流场中的温室气体排放率和源位置，替代传统CFD计算，实现高精度与快速预测。


<details>
  <summary>Details</summary>
Motivation: 实时识别和量化瞬态大气条件下的温室气体排放是环境监测中的关键挑战，需要兼顾物理精确性和计算效率。

Method: 将多层感知机作为CFD的替代模型嵌入序列蒙特卡洛算法，进行贝叶斯推断，捕捉气体扩散的时空异质性。

Result: 在验证数据集上表现出与CFD和高斯羽流模型相当的精度，但运行速度快数个数量级，且在复杂环境中表现稳健。

Conclusion: 该框架在物理精确性与计算可行性之间取得平衡，为工业排放监测等时间敏感的时空反演任务提供了可扩展的解决方案。
...
### [198] [Expressive Score-Based Priors for Distribution Matching with Geometry-Preserving Regularization](https://arxiv.org/abs/2506.14607)
*Ziyu Gong,Jim Lim,David I. Inouye*

Main category: cs.LG

TL;DR: 论文提出了一种基于分数匹配的新型分布匹配方法，解决了现有方法的局限性，如固定先验偏差和训练难度。


<details>
  <summary>Details</summary>
Motivation: 现有分布匹配方法存在可扩展性差、不稳定性和模式崩溃等问题，而基于似然的方法又因固定先验或显式密度模型而受限。

Method: 通过使用基于分数的先验分布，利用去噪分数匹配训练先验，避免了固定先验偏差和显式密度模型的挑战。

Result: 实验表明，该方法在稳定性和计算效率上优于其他扩散先验方法，并在多个任务中表现优异。

Conclusion: 该方法为分布匹配提供了一种稳定且高效的解决方案，代码已开源。
...
### [199] [Feasibility-Driven Trust Region Bayesian Optimization](https://arxiv.org/abs/2506.14619)
*Paolo Ascia,Elena Raponi,Thomas Bäck,Fabian Duddeck*

Main category: cs.LG

TL;DR: 提出了一种名为FuRBO的可行性驱动信任区域贝叶斯优化算法，用于解决高维空间中昂贵约束的优化问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在昂贵约束和高维空间中难以快速找到可行解，优化预算浪费严重。

Method: FuRBO通过迭代定义信任区域，结合目标和约束代理模型信息，自适应调整搜索区域。

Result: 在BBOB-constrained COCO基准测试和其他物理启发基准上表现优异，优于现有方法。

Conclusion: FuRBO能快速发现高质量可行解，适用于高维和严重约束问题。
...
### [200] [Towards Desiderata-Driven Design of Visual Counterfactual Explainers](https://arxiv.org/abs/2506.14698)
*Sidney Bender,Jan Herrmann,Klaus-Robert Müller,Grégoire Montavon*

Main category: cs.LG

TL;DR: 本文提出了一种新的视觉反事实解释方法（SCE），旨在解决现有方法在解释的全面性（如保真度、可理解性和充分性）上的不足。
...
### [201] [On the Hardness of Bandit Learning](https://arxiv.org/abs/2506.14746)
...
<details>
  <summary>Details</summary>
Motivation: 旨在建立一个类似于PAC分类框架的多臂老虎机学习理论，明确哪些函数类是可学习的及其学习方法。

Method: 通过组合维度和计算复杂性分析，研究了函数类的可学习性，并构造了一个需要多项式时间求解的奖励函数类。

Result: 发现组合维度无法表征老虎机学习的可学习性，并证明了某些函数类在计算上是困难的，除非RP=NP。

Conclusion: 结构化老虎机学习存在局限性，计算复杂性是其固有特性，同时探讨了噪声下的学习及查询复杂度与遗憾最小化的关系。
...
### [202] [A Systematic Review of User-Centred Evaluation of Explainable AI in Healthcare](https://arxiv.org/abs/2506.13904)
*Ivania Donoso-Guzmán,Kristýna Sirka Kacafírková,Maxwell Szymanski,An Jacobs,Denis Parra,Katrien Verbert*

Main category: cs.HC

TL;DR: 该研究填补了可解释人工智能（XAI）在医疗领域实际应用中缺乏明确评估指南的空白，提出了一个框架和指南，以支持用户中心化的XAI系统评估。


<details>
  <summary>Details</summary>
Motivation: 当前XAI方法在实际应用中的价值和可信度缺乏充分验证，尤其是在医疗领域，缺乏明确的用户评估指南。

Method: 通过系统综述82项医疗领域的用户研究，结合预定义的编码方案和迭代开发的归纳代码，分析XAI评估实践。

Result: 研究总结了当前评估实践，揭示了医疗XAI中以人为本的趋势，解释了属性间的相互关系，并提出了更新的框架和指南。

Conclusion: 该研究为跨学科团队提供了针对特定应用场景设计有效XAI评估策略的工具，推动了医疗XAI的实际应用。
...
### [203] [StorySage: Conversational Autobiography Writing Powered by a Multi-Agent Framework](https://arxiv.org/abs/2506.14159)
*Shayan Talaei,Meijin Li,Kanu Grover,James Kent Hippler,Diyi Yang,Amin Saberi*

Main category: cs.HC

TL;DR: StorySage是一个多代理框架驱动的系统，旨在通过灵活的对话和结构化方法帮助用户整理个人记忆并完成自传写作。


<details>
  <summary>Details</summary>
Motivation: 现有的对话写作助手通常依赖通用交互和预定义指南，难以捕捉个人记忆并逐步完成自传。

Method: StorySage采用多代理框架（包括采访者、会话记录员、规划者、章节作者和会话协调员），迭代收集用户记忆并更新自传。
...
### [204] [Balancing Caregiving and Self-Care: Exploring Mental Health Needs of Alzheimer's and Dementia Caregivers](https://arxiv.org/abs/2506.14196)
*Jiayue Melissa Shi,Keran Wang,Dong Whi Yoo,Ravi Karkar,Koustuv Saha*

Main category: cs.HC

TL;DR: 研究探讨了阿尔茨海默病及相关痴呆症（AD/ADRD）家庭照顾者的心理健康需求，分析了他们在不同阶段的应对策略和技术使用，提出了动态干预设计的建议。
...
Method: 通过25名家庭照顾者的半结构化访谈，分析心理健康挑战的原因、影响及应对策略，并绘制心理健康随时间演变的阶段图。

Result: 研究发现照顾者心理健康需求随护理阶段变化，现有技术需改进为可访问、可扩展且个性化的解决方案。
...
### [205] [Controlling Context: Generative AI at Work in Integrated Circuit Design and Other High-Precision Domains](https://arxiv.org/abs/2506.14567)
*Emanuel Moss,Elizabeth Watkins,Christopher Persaud,Passant Karunaratne,Dawn Nafus*

Main category: cs.HC

TL;DR: 论文探讨了生成式AI工具在高精度工程领域中的使用挑战，尤其是准确性和交互上下文控制问题。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI工具的普及和准确性提升，研究高精度领域工程师如何保持对错误的警觉性及其使用中的其他问题。

Method: 通过采访硬件和软件工程师及其合作者，分析生成式AI工具在集成电路设计中的使用及其问题。

Result: 研究发现工程师面临的主要挑战是交互上下文的控制，并列举了其他问题。
...
### [206] [StreetLens: Enabling Human-Centered AI Agents for Neighborhood Assessment from Street View Imagery](https://arxiv.org/abs/2506.14670)
...
Conclusion: StreetLens为社区研究提供了灵活、可扩展的AI工具，加速研究进程。
...
### [207] [Design an Editable Speech-to-Sign-Language Transformer System: A Human-Centered AI Approach](https://arxiv.org/abs/2506.14677)
*Yingchao Li*

Main category: cs.HC

TL;DR: 本文提出了一种基于Transformer的实时语音到手语动画系统，通过用户可编辑的JSON中间层提升了自然性和用户参与度。
...
Method: 结合流式Conformer编码器和自回归Transformer-MDN解码器，实现语音到3D虚拟形象的同步动作生成，支持用户编辑和反馈优化。
...
### [208] [LLM-Driven Data Generation and a Novel Soft Metric for Evaluating Text-to-SQL in Aviation MRO](https://arxiv.org/abs/2506.13785)
*Patrick Sutanto,Jonathan Kenrick,Max Lorenz,Joan Santoso*

Main category: cs.DB

TL;DR: 本文提出了一种基于F1分数的‘软’指标和LLM驱动的数据生成方法，以解决文本到SQL任务中的评估指标僵化和领域数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在文本到SQL任务中的应用有望推动数据民主化，但现有评估指标（如执行准确率）过于僵化且领域数据稀缺，阻碍了进展。

Method: 引入基于F1分数的‘软’评估指标，量化生成SQL与真实SQL结果的信息重叠；提出LLM驱动的管道，从数据库模式合成真实的问答对。

Result: 实验表明，软指标比严格准确率提供更深入的分析，数据生成技术能有效创建领域特定基准。
...
### [209] [Sketched Sum-Product Networks for Joins](https://arxiv.org/abs/2506.14034)
*Brian Tsan,Abylay Amanbayev,Asoke Datta,Florin Rusu*

Main category: cs.DB

TL;DR: 论文提出了一种使用Sum-Product Networks动态近似草图的方法，以解决传统草图在查询优化中仅适用于预定义查询的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统草图在多路连接基数估计中虽高效，但仅适用于预定义查询，限制了其在新查询中的应用。

Method: 利用Sum-Product Networks分解和建模多元分布，通过线性组合单变量分布动态近似草图，并应用于Fast-AGMS和Bound Sketch方法。

Result: 通过动态近似草图，实现了对新查询的高效基数估计，为查询优化提供了实用方案。

Conclusion: Sum-Product Networks的动态近似草图方法扩展了草图在查询优化中的适用性，提升了灵活性。
...
### [210] [A Hybrid Neural Network -- Polynomial Series Scheme for Learning Invariant Manifolds of Discrete Dynamical Systems](https://arxiv.org/abs/2506.13950)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在建模复杂结构时存在局限性，需要结合多项式的高效性和神经网络的灵活性。

Method: 结合多项式级数和浅层神经网络，利用多项式保证局部指数收敛，神经网络处理复杂结构。

Result: 在三个基准测试中，混合方案在数值逼近精度上优于纯多项式或神经网络方法。

Conclusion: 混合方案在降阶模型构建中表现出更高的准确性和效率。
...
### [211] [A Survey of Physics-Informed AI for Complex Urban Systems](https://arxiv.org/abs/2506.13777)
...
TL;DR: 论文综述了物理信息AI方法在城市应用中的分类与应用，提出了三种范式，并分析了八大城市领域的应用，指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 城市系统是复杂系统的典型例子，结合物理模型与AI可提升预测准确性、可解释性和决策能力。

Method: 提出了三种物理-AI集成范式（物理集成AI、物理-AI混合集成、AI集成物理），并详细介绍了七种代表性方法。

Result: 分析了八大城市领域的应用，展示了物理定律与数据驱动模型如何解决城市挑战，提升系统可靠性、效率和适应性。

Conclusion: 通过综合现有方法及其应用，识别了关键差距并提出了未来研究方向，为下一代智能城市系统建模铺平了道路。
...
### [212] [Infected Smallville: How Disease Threat Shapes Sociality in LLM Agents](https://arxiv.org/abs/2506.13783)
*Soyeon Choi,Kangwook Lee,Oliver Sng,Joshua M. Ackerman*

Main category: physics.soc-ph

TL;DR: 研究发现，通过生成式代理模型（GABM）模拟，传染病威胁显著降低了代理的社交参与度，包括减少社交活动、公共场所访问和对话频率。


<details>
  <summary>Details</summary>
Motivation: 探讨传染病威胁如何影响生成式代理的社交行为，验证行为免疫系统的假设。

Method: 使用基于大型语言模型的生成式代理模型（GABM），通过三组模拟实验比较代理在传染病新闻影响下的社交行为变化。

Result: 接触传染病新闻的代理社交参与度显著降低，包括减少社交活动、公共场所访问和对话频率，且能区分传染与非传染疾病。
...
### [213] [hyperFA*IR: A hypergeometric approach to fair rankings with finite candidate pool](https://arxiv.org/abs/2506.14349)
*Mauritz N. Cartier van Dissel,Samuel Martin-Gutierrez,Lisette Espín-Noboa,Ana María Jaramillo,Fariba Karimi*

Main category: cs.CY

TL;DR: 论文提出了一种名为hyperFA*IR的框架，用于评估和确保有限候选池中排名的公平性，基于超几何分布生成过程，解决了现有方法在有限样本和随机性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有群体公平框架通常依赖保护群体的比例表示，但无法有效处理排名的随机性或候选池的有限性，因此需要更准确的公平评估方法。

Method: 采用超几何分布生成过程模拟现实场景，提出Monte Carlo算法高效检测不公平排名，并通过加权采样定义平权政策。

Result: 与广泛使用的二项模型相比，hyperFA*IR更准确地模拟了有限总体的统计特性，并在实验和分析中验证了其优越性。

Conclusion: hyperFA*IR为有限候选池中的排名公平性提供了更准确的评估和操作框架，适用于保护群体较小或选择规模较大的场景。
...
### [214] [Contemporary AI foundation models increase biological weapons risk](https://arxiv.org/abs/2506.13798)
*Roger Brent,T. Greg McKelvey*

Main category: cs.CY

TL;DR: 论文指出现有AI模型的安全评估低估了其促进生物武器开发的风险，原因包括错误假设和不完善的评估方法。通过案例和框架分析，发现先进AI模型能指导用户完成复杂生物技术任务，呼吁改进评估标准。


<details>
  <summary>Details</summary>
Motivation: 人工智能的快速发展可能被用于生物武器开发，但现有安全评估低估了这种风险，亟需重新审视。

Method: 通过案例分析（如无专业知识者完成复杂任务）和框架构建（识别生物武器开发的“成功要素”），评估AI模型的潜在风险。

Result: 发现Llama 3.1 405B、ChatGPT-4o和Claude 3.5 Sonnet等模型能准确指导用户完成活体脊髓灰质炎病毒的合成，反驳了当前模型生物安全风险低的观点。

Conclusion: 需改进评估方法，但可能已错过最佳实施时机。
...
### [215] [Dr. GPT Will See You Now, but Should It? Exploring the Benefits and Harms of Large Language Models in Medical Diagnosis using Crowdsourced Clinical Cases](https://arxiv.org/abs/2506.13805)
*Bonam Mingole,Aditya Majumdar,Firdaus Ahmed Choudhury,Jennifer L. Kraschnewski,Shyam S. Sundar,Amulya Yadav*

Main category: cs.CY

TL;DR: 论文探讨了大型语言模型（LLMs）在回答日常健康问题时的有效性，通过大学竞赛和众包方法评估了LLMs的表现，发现76%的回答被医生认为准确，并探讨了RAG版本LLMs的改进潜力。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽视了LLMs在回答普通用户日常健康问题时的实际表现，而这是更普遍的应用场景。本文旨在填补这一研究空白。

Method: 通过大学竞赛，34名参与者向4个公开可用的LLMs提出了212个真实或想象的健康问题，由9名认证医生评估回答质量，并探讨了RAG版本LLMs的改进效果。

Result: 76%的LLM回答被医生认为准确，RAG版本的LLMs可能进一步提高回答质量。

Conclusion: 论文为LLMs在现实世界健康沟通中的表现提供了更实际的见解，并指出了改进方向。
...
### [216] [Students' Reliance on AI in Higher Education: Identifying Contributing Factors](https://arxiv.org/abs/2506.13845)
*Griffin Pitts,Neha Rani,Weedguet Mildort,Eva-Marie Cook*

Main category: cs.CY

TL;DR: 研究探讨了大学生对AI工具的依赖模式，包括过度依赖、适当依赖和不足依赖，发现这些模式与编程自我效能、认知需求等因素相关。


<details>
  <summary>Details</summary>
Motivation: 随着AI在教育中的普及，学生过度依赖AI可能导致学习效果下降，因此研究依赖模式的成因具有重要意义。

Method: 结合前后测调查和实验任务，观察学生在面对不同可靠性AI建议时的依赖行为。

Result: 适当依赖与编程自我效能、认知需求正相关，过度依赖与对AI的信任和满意度相关，不足依赖与编程能力负相关。

Conclusion: 研究结果为设计促进适当依赖AI的干预措施提供了依据，对教育技术整合有启示。
...
### [217] [Computational Studies in Influencer Marketing: A Systematic Literature Review](https://arxiv.org/abs/2506.14602)
*Haoyang Gui,Thales Bertaglia,Catalina Goanta,Gerasimos Spanakis*

Main category: cs.CY

TL;DR: 本文通过系统文献综述（SLR）分析了69项研究，总结了计算研究在网红营销中的关键主题、方法和未来方向，并提出了多学科研究议程。


<details>
  <summary>Details</summary>
Motivation: 网红营销在数字营销中日益重要，但计算研究领域缺乏系统性综述，导致科学测量稀缺，影响平台外利益相关者（如监管者和其他领域研究者）。

Method: 基于PRISMA模型进行系统文献综述，分析69项研究，识别研究主题和方法（机器学习与非机器学习技术）。

Result: 发现四大研究主题：网红识别与特征、广告策略与参与度、赞助内容分析与发现、公平性；商业优化为主，监管合规和伦理关注不足。

Conclusion: 建议未来研究结合上下文因素（如语言、平台、行业类型），提升模型可解释性和数据集可复现性，并推动与监管合规技术的多学科合作。
...
### [218] [The Synthetic Mirror -- Synthetic Data at the Age of Agentic AI](https://arxiv.org/abs/2506.13818)
*Marcelle Momha*

Main category: cs.CY

TL;DR: 论文探讨了合成数据在隐私和政策制定中的影响，强调需要新的政策工具和法律框架调整以确保信任和问责。


<details>
  <summary>Details</summary>
Motivation: 合成数据的广泛使用导致了对现实的表现和潜在扭曲，引发了信任和问责问题。

Method: 通过分析合成数据的特性及其对隐私和政策的影响，提出针对性修改现有法律框架的建议。

Result: 合成数据需要被视为独特的监管类别，现有法律框架需调整以适应其特性。

Conclusion: 最实用的方法是对现有法律框架进行针对性修改，而非创建全新制度。
...
### [219] [Rigor in AI: Doing Rigorous AI Work Requires a Broader, Responsible AI-Informed Conception of Rigor](https://arxiv.org/abs/2506.14652)
...
<details>
  <summary>Details</summary>
Motivation: 当前AI研究过于关注方法论严谨性，忽视了其他方面的严谨性，导致对AI能力的夸大宣称，需要更全面的严谨性框架。

Method: 提出一个扩展的严谨性框架，涵盖方法论、认识论、规范性、概念性、报告性和解释性六个方面。

Result: 为AI社区提供了一个更全面的严谨性语言和框架，促进多方对话。

Conclusion: 呼吁AI研究与实践采用更广泛的严谨性概念，以解决当前问题并推动负责任AI的发展。
...
### [220] [NeuralPDR: Neural Differential Equations as surrogate models for Photodissociation Regions](https://arxiv.org/abs/2506.14270)
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率望远镜（如JWST和ALMA）需要更小尺度的天体化学模型，但传统模拟的计算成本极高，因此需要高效的替代方法。

Method: 使用Latent Augmented Neural ODEs作为代理模型，训练于三个复杂度递增的数据集，包括来自3D-PDR模拟的数据。

Result: 代理模型能显著加速计算，并准确复现原始观测数据中的柱密度分布。

Conclusion: 该方法支持快速化学推断，可用于高效统计推断或提高天体物理模拟的分辨率。
...
### [221] [A Silent Speech Decoding System from EEG and EMG with Heterogenous Electrode Configurations](https://arxiv.org/abs/2506.13835)
...
TL;DR: 该论文提出了一种神经网络方法，用于处理异质电极放置的EEG/EMG数据，并通过多任务训练在大规模数据集上实现无声语音解码的高性能。
...
Method: 引入神经网络模型，支持异质电极放置的EEG/EMG数据，并通过多任务训练在大规模数据集上进行训练。

Result: 在健康参与者中达到95.3%的单词分类准确率，在语言障碍患者中达到54.5%，显著优于单受试者数据训练的模型。
...
### [222] [Asymptotically Smaller Encodings for Graph Problems and Scheduling](https://arxiv.org/abs/2506.14042)
*Bernardo Subercaseaux*

Main category: cs.LO

TL;DR: 论文提出了一种新的图问题编码方法，将CNF子句数量从$Ω(|V|^2)$减少到$O(|V|^2 / \lg |V|)$，并展示了在密集区间图中独立集问题的新编码，成功应用于字符串压缩和调度问题。


<details>
  <summary>Details</summary>
Motivation: 传统编码方法需要$Ω(|V|^2)$子句，效率较低。研究旨在探索更高效的编码方式，减少计算复杂度。

Method: 利用Erdős等人的双团覆盖理论，提出新的CNF编码方法，并针对密集区间图设计独立集问题的优化编码。

Result: 成功将子句数量减少到$O(|V|^2 / \lg |V|)$，并在密集区间图中进一步优化为$O(|V| \lg |V|)$。调度问题的编码规模从$O(NMT^2)$降至$O(NMT + M T^2 \lg T)$。

Conclusion: 新编码方法显著提高了效率，为预处理工具的成功提供了理论支持，并在实际应用中验证了其有效性。
...
### [223] [DeepSeq: High-Throughput Single-Cell RNA Sequencing Data Labeling via Web Search-Augmented Agentic Generative AI Foundation Models](https://arxiv.org/abs/2506.13817)
*Saleem A. Al Dajani,Abel Sanchez,John R. Williams*

Main category: q-bio.GN

TL;DR: 提出了一种基于生成式AI基础模型的自动化标注方法，用于单细胞RNA测序数据，准确率达82.5%，解决了监督学习中的标注瓶颈。


<details>
  <summary>Details</summary>
Motivation: 解决结构化组学数据中手动标注的低效和误差问题，提升标注吞吐量。

Method: 利用具有实时网络搜索功能的代理基础模型，自动化标注实验数据。

Result: 实现了82.5%的标注准确率，支持下游任务如细胞分型和扰动预测。

Conclusion: 该方法展示了在健康监测和诊断中的领域创新潜力，未来可能超越人类标注性能。
...
TL;DR: 论文分析了在一般Hamming空间中突变和交叉重组的概率，推导了优化参数的条件，并比较了突变和重组在进化中的不同作用。


<details>
  <summary>Details</summary>
Motivation: 受Fisher几何方法的启发，研究如何通过突变和重组优化进化过程，特别是在Hamming空间中。

Method: 使用几何和组合分析，推导了距离最优解的转移概率的闭式表达式，并优化了突变和重组的参数。

Result: 发现突变和重组在进化中具有不同特性：突变概率随距离减小而降低，而重组在种群定义的子空间中平衡，可能加速进化。

Conclusion: 重组可以补充突变，特别是在接近最优解时，可能提高进化速率。
...
### [225] [Safe Domains of Attraction for Discrete-Time Nonlinear Systems: Characterization and Verifiable Neural Network Estimation](https://arxiv.org/abs/2506.13961)
...
<details>
  <summary>Details</summary>
Motivation: 非线性自治系统的吸引域估计存在保守性或低维限制，尤其是在考虑状态约束时更具挑战性。

Method: 推导新的Zubov方程，其解对应精确的安全吸引域；采用物理信息神经网络近似解，并通过验证框架（如α,β-CROWN和dReal）获得可验证的估计。

Result: Zubov方程的解在状态空间上唯一且连续；通过数值示例验证了方法的有效性。

Conclusion: 提出的框架为非线性系统安全吸引域估计提供了准确且可验证的解决方案。
...
### [226] [Beyond Shapley Values: Cooperative Games for the Interpretation of Machine Learning Models](https://arxiv.org/abs/2506.13900)
...
<details>
  <summary>Details</summary>
Motivation: 尽管Shapley值在可解释性中被广泛采用，但其公理基础与特征归因的相关性仍存争议，作者希望提供更灵活且理论扎实的工具。

Method: 作者回顾了合作博弈论，介绍了Weber和Harsanyi集，并提出了一个三步框架来构建特征归因方法。

Result: 论文扩展了可解释性工具的范围，提供了更丰富的归因灵活性，并提出了一个理论扎实的框架。

Conclusion: 作者呼吁超越固定公理，为XAI社区提供一个设计有意义且稳健归因方法的框架。
...
### [227] [Mirror Descent Using the Tempesta Generalized Multi-parametric Logarithms](https://arxiv.org/abs/2506.13984)
*Andrzej Cichocki*

Main category: stat.ML

TL;DR: 本文提出了一类广泛的Mirror Descent（MD）算法，利用Bregman散度和Tempesta多参数变形对数作为链接函数，生成了灵活且适应性强的MD更新方法。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够适应数据分布或几何结构的MD算法，通过多参数对数调整算法性能。

Method: 利用Bregman散度和Tempesta多参数变形对数作为链接函数，估计广义指数函数以推导MD更新。

Result: 生成了一个广泛且灵活的MD和mirror-less MD更新家族，可通过超参数调整适应数据。

Conclusion: 多参数对数的应用为MD算法提供了新的灵活性和适应性。
...
### [228] [Rademacher learning rates for iterated random functions](https://arxiv.org/abs/2506.13946)
*Nikola Sandrić*

Main category: stat.ML

TL;DR: 论文探讨了在非独立同分布（非i.i.d.）数据下，基于迭代随机函数的监督机器学习问题，提出了数据分布依赖的学习率。
...
Result: 证明了近似经验风险最小化算法的可学习性，并推导了其学习率上界，该上界依赖于数据分布。
...
### [229] [Meta Optimality for Demographic Parity Constrained Regression via Post-Processing](https://arxiv.org/abs/2506.13947)
*Kazuto Fukuchi*

Main category: stat.ML

TL;DR: 本文提出了验证公平极小极大最优回归算法适用性的元定理，并展示了通过后处理方法实现公平回归的可行性。


<details>
  <summary>Details</summary>
Motivation: 解决在人口统计平等约束下的回归问题，验证现有公平回归算法的广泛适用性。

Method: 提出元定理，适用于多种场景验证公平极小极大最优性，并采用后处理方法实现公平回归。

Result: 证明了公平极小极大最优回归可通过后处理实现，使传统回归方法能高效适配公平需求。

Conclusion: 研究为公平回归提供了通用验证框架，并简化了公平回归的实现流程。
...
### [230] [Bridging Unsupervised and Semi-Supervised Anomaly Detection: A Theoretically-Grounded and Practical Framework with Synthetic Anomalies](https://arxiv.org/abs/2506.13955)
*Matthew Lau,Tian-Yi Zhou,Xiangchi Yuan,Jizhou Chen,Wenke Lee,Xiaoming Huo*

Main category: stat.ML

TL;DR: 论文提出了一种半监督异常检测的理论框架，结合已知和合成异常数据，提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 半监督异常检测中，现有方法未能充分利用有限的标记异常数据，需要一种理论支持的方法来优化检测效果。

Method: 提出理论框架，结合已知和合成异常数据训练分类器，并引入数学公式分析半监督异常检测。

Result: 实验验证了框架在五个基准数据集上的性能提升，并证明合成异常数据能优化低密度区域建模和分类器收敛。
...
### [231] [Estimation of Treatment Effects in Extreme and Unobserved Data](https://arxiv.org/abs/2506.14051)
*Jiyuan Tan,Jose Blanchet,Vasilis Syrgkanis*

Main category: stat.ML

TL;DR: 本文提出了一种新框架，用于评估罕见事件中的因果效应，结合极值理论和多元正则变化理论。
...
Method: 采用多元正则变化理论建模极端事件，开发了一种一致的极端处理效应估计器。

Result: 通过合成和半合成数据验证了估计器的性能。

Conclusion: 该框架为罕见事件中的因果效应估计提供了有效工具。
...
### [232] [Universal Rates of ERM for Agnostic Learning](https://arxiv.org/abs/2506.14110)
*Steve Hanneke,Mingyue Xu*

Main category: stat.ML

TL;DR: 本文研究了在不可实现（agnostic）设置下，通过经验风险最小化（ERM）进行二元分类的通用学习速率问题，揭示了三种可能的速率类型，并提供了完整的分类特征。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中在可实现（realizable）设置下的通用学习速率，而不可实现设置下的研究较少，本文旨在填补这一空白。

Method: 通过分析ERM在不可实现设置下的学习曲线，探索了通用速率的可能性，并提出了三种速率的分类标准。

Result: 揭示了三种可能的通用速率：指数衰减（e^{-n}）、亚平方根衰减（o(n^{-1/2})）和任意慢速衰减，并提供了分类的完整特征。

Conclusion: 本文为不可实现设置下的通用学习速率提供了完整的理论框架，并扩展了目标依赖和贝叶斯依赖的通用速率研究。
...
### [233] [Adjustment for Confounding using Pre-Trained Representations](https://arxiv.org/abs/2506.14329)
...
<details>
  <summary>Details</summary>
Motivation: 非表格数据（如图像和文本）可能作为混杂因素影响ATE估计，忽略这些因素会导致偏差和错误的科学结论。

Method: 利用预训练神经网络的潜在特征，结合双机器学习方法，调整混杂因素。

Result: 神经网络能够适应学习问题的内在稀疏性和维度，实现快速收敛速率。

Conclusion: 尽管高维和非可识别性带来挑战，神经网络仍能有效调整混杂因素，提升ATE估计的准确性。
...
### [234] [Adaptive Data Augmentation for Thompson Sampling](https://arxiv.org/abs/2506.14479)
*Wonyoung Kim*

Main category: stat.ML

TL;DR: 本文提出了一种新的线性上下文赌博机方法，通过自适应增强和耦合假设样本的估计器，实现了近乎极小极大最优的遗憾边界。


<details>
  <summary>Details</summary>
Motivation: 尽管Thompson Sampling在实证中表现良好，但其遗憾边界并非最优。本文旨在改进这一缺陷。

Method: 开发了一种新颖的估计器，结合自适应增强和假设样本的耦合，以高效学习参数。
...
### [235] [Sharp Generalization Bounds for Foundation Models with Asymmetric Randomized Low-Rank Adapters](https://arxiv.org/abs/2506.14530)
...
### [236] [Uniform Mean Estimation for Heavy-Tailed Distributions via Median-of-Means](https://arxiv.org/abs/2506.14673)
*Mikael Møller Høgsgaard,Andrea Paudice*

Main category: stat.ML

TL;DR: 本文研究了中位数均值（MoM）在重尾数据中的性能，提出了一种新的对称化技术，改进了样本复杂度界限，并应用于无界输入的k均值聚类和一般损失线性回归。


<details>
  <summary>Details</summary>
Motivation: 研究MoM在仅具有前p矩（p∈(1,2]）的数据分布中对函数类均值估计的性能。

Method: 使用一种新的对称化技术，分析MoM的样本复杂度。

Result: 证明了新的样本复杂度界限，并改进了k均值聚类和线性回归的现有结果。

Conclusion: MoM在重尾数据中表现优异，新对称化技术具有独立意义，应用效果显著。
...
### [237] [Agile Orchestration at Will: An Entire Smart Service-Based Security Architecture Towards 6G](https://arxiv.org/abs/2505.22963)
...
<details>
  <summary>Details</summary>
Motivation: 6G网络的安全性和弹性需求突出，但现有架构难以应对异构网络和多样化安全需求的挑战，因此需要重新设计安全架构。

Method: 提出ES3A架构，基于六项高级原则（层次性、灵活性、可扩展性、弹性、内生性和信任与隐私）和三条部署指南（服务化安全、端到端保护和智能安全自动化），采用三层三域结构和两阶段编排机制。

Result: 在基于软件定义无线电（SDR）的真实无线电系统上原型验证了ES3A的有效性，并通过案例展示了其优越性。

Conclusion: ES3A为6G网络提供了定制化、智能化的安全保护，能够有效应对动态和异构网络环境中的安全挑战。
...
### [238] [Markov Regime-Switching Intelligent Driver Model for Interpretable Car-Following Behavior](https://arxiv.org/abs/2506.14762)
*Chengyuan Zhang,Cathy Wu,Lijun Sun*

Main category: stat.AP

TL;DR: 论文提出了一种基于状态切换的框架（FHMM-IDM），通过分离驾驶行为和交通场景，提高了跟车模型的准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统跟车模型（如IDM）因单一状态结构无法捕捉人类驾驶的多模态行为，导致模型参数难以解释且精度受限。

Method: 采用因子隐马尔可夫模型（FHMM-IDM），通过两个独立的隐马尔可夫过程分离驾驶行为和交通场景，并使用贝叶斯推断估计参数。

Result: 在HighD数据集上的实验表明，FHMM-IDM能够有效分离驾驶行为和交通场景，揭示动态状态切换模式。

Conclusion: 该框架为不确定性下的上下文依赖驾驶行为建模提供了可行方案，提升了交通模拟的精度和ADAS的开发效果。
...
### [239] [Investigating the Potential of Large Language Model-Based Router Multi-Agent Architectures for Foundation Design Automation: A Task Classification and Expert Selection Study](https://arxiv.org/abs/2506.13811)
*Sompote Youwai,David Phim,Vianne Gayl Murcia,Rianne Clair Onas*

Main category: cs.MA

TL;DR: 研究了基于路由器的多智能体系统，通过智能任务分类和专家选择自动化基础设计计算，性能优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 探索多智能体系统在基础设计自动化中的应用，以提高计算效率和准确性。

Method: 评估了三种方法：单智能体处理、多智能体设计-检查架构和基于路由器的专家选择，使用多个基线模型进行性能测试。

Result: 基于路由器的配置在浅基础和桩基础设计中分别达到95.00%和90.63%的性能，优于传统方法。

Conclusion: 基于路由器的多智能体系统是基础设计自动化的最优选择，但仍需人类监督以确保安全性。
...
### [240] [Enhancing Clinical Decision Support and EHR Insights through LLMs and the Model Context Protocol: An Open-Source MCP-FHIR Framework](https://arxiv.org/abs/2506.13800)
*Abul Ehtesham,Aditi Singh,Saket Kumar*

Main category: cs.SE

TL;DR: 本文提出了一种基于开源代理的框架，将大型语言模型（LLM）与HL7 FHIR数据集成，通过动态提取和推理电子健康记录（EHR）来增强临床决策支持（CDS）、减轻文档负担并提高患者健康素养。
...
Method: 开发了一个基于MCP-FHIR的开源框架，支持通过JSON配置动态访问FHIR资源，实现实时总结、解释和个性化通信。

Result: 框架在SMART Health IT沙盒中使用合成EHR数据进行了评估，展示了可扩展、可解释和互操作的AI驱动EHR应用。

Conclusion: 该框架为个性化数字健康解决方案提供了坚实基础，支持多种FHIR格式，优于传统的静态工作流方法。
...
### [241] [Instruction and Solution Probabilities as Heuristics for Inductive Programming](https://arxiv.org/abs/2506.13804)
*Edward McDaid,Sarah McDaid*

Main category: cs.SE

TL;DR: 论文提出了一种通过引入指令概率和解决方案概率来进一步缩小归纳编程搜索空间的方法，结合指令子集（IS）可将搜索空间减少超过100个数量级。


<details>
  <summary>Details</summary>
Motivation: 通过引入指令概率和解决方案概率作为启发式方法，进一步优化归纳编程（IP）的搜索空间，以提高效率。

Method: 利用代码样本中指令出现的频率计算指令概率，并基于此计算解决方案概率；通过设定解决方案概率阈值来剪枝搜索空间。

Result: 实验表明，该方法能进一步将IP搜索空间缩小数十个数量级，结合IS可减少超过100个数量级。交叉验证显示启发式方法对未见代码也有效。

Conclusion: 该方法显著提升了搜索效率，未来可进一步优化启发式方法。
...
### [242] [Structured Program Synthesis using LLMs: Results and Insights from the IPARC Challenge](https://arxiv.org/abs/2506.13820)
*Shraddha Surana,Ashwin Srinivasan,Michael Bain*

Main category: cs.SE

TL;DR: IPARC挑战赛通过合成图像任务评估程序合成能力，本文提出基于LLM的结构化归纳编程方法，成功解决所有任务类别，揭示了LLM代码生成的关键机制。
...
Method: 采用结构化归纳编程方法，结合LLM生成代码，并通过人工细化优化结果。

Result: 成功解决所有IPARC任务类别，揭示了LLM代码生成的关键机制，如代码冻结和重用效率。

Conclusion: LLM与人类协作在复杂程序合成中具有重要价值，未来可进一步优化合作机制。
...
### [243] [CRITICTOOL: Evaluating Self-Critique Capabilities of Large Language Models in Tool-Calling Error Scenarios](https://arxiv.org/abs/2506.13977)
*Shiting Huang,Zhen Fang,Zehui Chen,Siyu Yuan,Junjie Ye,Yu Zeng,Lin Chen,Qi Mao,Feng Zhao*

Main category: cs.SE

TL;DR: 论文提出CRITICTOOL，一个专门用于工具学习的全面批评评估基准，通过进化策略构建数据集，解决复杂任务中工具使用的错误问题。


<details>
  <summary>Details</summary>
Motivation: 随着任务复杂性和长期性增加，大型语言模型（LLM）在工具使用过程中可能触发各种意外错误，如何有效处理这些错误成为关键研究方向。

Method: 通过分析多个工具评估基准中的错误类型，构建CRITICTOOL基准，采用进化策略生成多样化工具使用错误数据集。

Result: 实验验证了CRITICTOOL的泛化性和有效性，并对不同LLM的工具反思能力进行了深入分析。

Conclusion: CRITICTOOL为工具学习领域提供了新的视角，代码已开源。
...
### [244] [MLDebugging: Towards Benchmarking Code Debugging Across Multi-Library Scenarios](https://arxiv.org/abs/2506.13824)
*Jinyang Huang,Xiachong Feng,Qiguang Chen,Hanjie Zhao,Zihui Cheng,Jiesong Bai,Jingxuan Zhou,Min Li,Libo Qin*

Main category: cs.SE

TL;DR: 论文提出了MLDebugging基准，用于评估多库Python代码调试问题，发现当前LLMs在此场景下表现不佳。
...
### [245] [FrontendBench: A Benchmark for Evaluating LLMs on Front-End Development via Automatic Evaluation](https://arxiv.org/abs/2506.13832)
*Hongda Zhu,Yiwen Zhang,Bing Zhao,Jingzhe Ding,Siyao Liu,Tong Liu,Dandan Wang,Yanan Liu,Zhaojian Li*

Main category: cs.SE

TL;DR: FrontendBench是一个由人类和LLMs共同开发的基准测试，用于更全面和实际地评估前端代码生成能力。
...
Result: 该框架与专家人工评估的吻合率达90.54%，并在多个先进LLMs上观察到显著的性能差异。
...
### [246] [How Does LLM Reasoning Work for Code? A Survey and a Call to Action](https://arxiv.org/abs/2506.13932)
...
Result: 提出了代码推理的全面分类、性能评估框架，并指出了未来研究的潜在方向。

Conclusion: LLMs在代码推理任务中具有潜力，但仍需进一步探索未充分研究的领域。
...
### [247] [Automatic Qiskit Code Refactoring Using Large Language Models](https://arxiv.org/abs/2506.14535)
*José Manuel Suárez,Luis Mariano Bibbó,Joaquin Bogado,Alejandro Fernandez*

Main category: cs.SE

TL;DR: 本文提出了一种利用大语言模型（LLM）重构Qiskit代码的新方法，通过提取Qiskit文档中的迁移场景分类法，并结合源代码输入LLM，实现了高效自动化代码迁移。
...
Method: 从Qiskit官方文档中提取迁移场景分类法，结合源代码输入LLM，指导其识别迁移场景并生成重构建议。

Result: 实验表明，结合领域知识的LLM能有效自动化Qiskit代码迁移，并提供了从旧版本迁移至0.46版的分类法和提示集。
...
### [248] [Low-code to fight climate change: the Climaborough project](https://arxiv.org/abs/2506.14623)
...
<details>
  <summary>Details</summary>
Motivation: 支持欧洲城市在2030年前实现碳中和，通过实时监测和评估本地气候倡议的效果。

Method: 采用低代码策略加速仪表盘开发，嵌入无代码哲学，使非技术用户也能配置和适配仪表盘。

Result: 开发了Climaborough城市平台，聚合历史与实时数据，通过用户友好仪表盘展示进展。

Conclusion: 低代码/无代码策略有效加速了气候仪表盘的部署，提升了公民参与和本地气候倡议的评估能力。
...
### [249] [ACM Survey Draft on Formalising Software Requirements with Large Language Models](https://arxiv.org/abs/2506.14627)
*Arshad Beg,Diarmuid O'Donoghue,Rosemary Monahan*

Main category: cs.SE

TL;DR: 本文档总结了94篇论文，并包含软件需求可追溯性、形式化方法及其工具、统一编程理论和机构理论等额外章节。


<details>
  <summary>Details</summary>
Motivation: 整合多篇论文内容，提供全面的研究综述，并突出与近期类似标题论文（AACS 2025和SAIV 2025）的关键区别。

Method: 通过文献综述和分类整理，结合额外章节的专题分析。

Result: 生成了一份包含94篇论文摘要及专题内容的综合文档。

Conclusion: 本文档为研究领域提供了全面的参考，并展示了与近期相关工作的差异。
...
### [250] [Navigating the growing field of research on AI for software testing -- the taxonomy for AI-augmented software testing and an ontology-driven literature survey](https://arxiv.org/abs/2506.14640)
*Ina K. Schieferdecker*

Main category: cs.SE

TL;DR: 本文综述了AI在软件测试自动化中的应用，从无自动化到全自动化，并提出了新的分类法ai4st。


<details>
  <summary>Details</summary>
Motivation: 随着AI在工程领域的突破，软件测试领域也迎来了新的机遇，本文旨在探讨AI如何增强软件测试自动化。

Method: 通过回顾近期研究，分析AI在测试自动化中的应用，并提出新的分类法ai4st。

Result: 提出了ai4st分类法，用于分类近期研究并识别未解决的问题。

Conclusion: AI为软件测试自动化带来了新的可能性，但仍有许多开放性问题需要进一步研究。
...
### [251] [Unified Software Engineering agent as AI Software Engineer](https://arxiv.org/abs/2506.14683)
*Leonhard Applis,Yuntong Zhang,Shanchao Liang,Nan Jiang,Lin Tan,Abhik Roychoudhury*

Main category: cs.SE

TL;DR: 论文探讨了LLM代理是否能成为AI软件工程师，提出了统一软件工程代理USEagent，并开发了评估基准USEbench。USEagent在多任务处理上表现优于现有通用代理，但仍存在不足。


<details>
  <summary>Details</summary>
Motivation: 研究LLM代理是否能胜任软件工程中的多任务处理，如维护和项目演化，而不仅仅是编码。

Method: 开发了统一软件工程代理USEagent，能够协调处理多种能力，并构建了评估基准USEbench。

Result: 在1,271个任务中，USEagent表现优于现有通用代理，但在某些编码任务上仍有不足。
...
Result: AMLGentex能够系统评估AML系统，模拟真实世界的复杂场景。

Conclusion: AMLGentex为AML研究提供了更接近实际的测试环境，有助于改进检测方法。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体上协调虚假行为难以与真实公共讨论区分，现有方法难以应对大规模网络数据的挑战。

Method: 利用LEN数据集，提出随机加权游走（RWW）方法生成密度感知嵌入，结合消息传递神经网络（MPNN）进行分类。

Result: 在二元和多类分类任务中，准确率分别提高了12%和5%。

Conclusion: 密度感知结构编码与MPNN结合为检测社交媒体上的协调虚假行为提供了有效框架。
...
### [254] [Reimagining Target-Aware Molecular Generation through Retrieval-Enhanced Aligned Diffusion](https://arxiv.org/abs/2506.14488)
*Dong Xu,Zhangfan Yang,Ka-chun Wong,Zexuan Zhu,Jiangqiang Li,Junkai Ji*

Main category: q-bio.BM

TL;DR: READ是一种结合检索增强生成与SE(3)-等变扩散模型的新方法，用于解决受体分子设计中几何拟合与化学约束的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 尽管高精度蛋白质结构预测取得突破，但现有方法仍难以平衡几何拟合与化学约束，因此需要一种更高效的设计方法。

Method: READ通过对比预训练编码器对齐原子级表示，并在推理时检索口袋匹配的支架图嵌入，以指导反向扩散步骤。

Result: 实验表明，READ在CBGBench中表现优异，超越现有生成模型甚至天然配体。
...
### [255] [Balancing Preservation and Modification: A Region and Semantic Aware Metric for Instruction-Based Image Editing](https://arxiv.org/abs/2506.13827)
*Zhuoying Li,Zhu Xu,Yuxin Peng,Yang Liu*

Main category: cs.GR

TL;DR: 提出了一种名为BPM的新指标，专门用于评估基于指令的图像编辑质量，通过分离编辑相关和无关区域进行综合评估。


<details>
  <summary>Details</summary>
Motivation: 现有指标要么成本高，要么缺乏任务针对性，无法全面评估编辑质量和无关区域保留。
...
### [256] [Innovating China's Intangible Cultural Heritage with DeepSeek + MidJourney: The Case of Yangliuqing theme Woodblock Prints](https://arxiv.org/abs/2506.14104)
*RuiKun Yang,ZhongLiang Wei,Longdi Xian*

Main category: cs.GR

TL;DR: 该研究结合DeepSeek和MidJourney生成杨柳青木版年画，主题为抗疫和欢乐胜利者，通过FID评分和问卷调查验证其效果。


<details>
  <summary>Details</summary>
Motivation: 解决传统艺术保护与创新的挑战，探索AI与传统艺术的融合。

Method: 采用DeepSeek生成主题提示，MidJourney生成图像，结合原始年画，并通过FID评分和问卷调查评估效果。

Result: 混合方法获得最低FID分数（150.2）和最小变异性（σ=4.9），问卷调查显示参与者认可度高。

Conclusion: AI与传统艺术的结合有效，既能保护文化，又能满足现代需求。
...
### [257] [ReFrame: Layer Caching for Accelerated Inference in Real-Time Rendering](https://arxiv.org/abs/2506.13814)
...
### [258] [Multimodal Fusion with Semi-Supervised Learning Minimizes Annotation Quantity for Modeling Videoconference Conversation Experience](https://arxiv.org/abs/2506.13971)
...
<details>
  <summary>Details</summary>
Motivation: 视频会议中的负面体验（如交流不流畅或不愉快）研究不足，且自然数据中这些时刻较少，标注成本高。
...
### [259] [Improving Practical Aspects of End-to-End Multi-Talker Speech Recognition for Online and Offline Scenarios](https://arxiv.org/abs/2506.14204)
*Aswin Shanmugam Subramanian,Amit Das,Naoyuki Kanda,Jinyu Li,Xiaofei Wang,Yifan Gong*

Main category: eess.AS

TL;DR: 论文扩展了序列化输出训练（SOT）框架，以解决流式和离线自动语音识别（ASR）的实际需求，平衡延迟和准确性。
...
Method: 提出三项改进：1）结合连续语音分离（CSS）前端与端到端系统；2）使用双模型（流式与离线）或两阶段编码器；3）探索分段SOT（segSOT）。

Result: CSS框架提升了ASR系统在多说话者重叠场景下的准确性；双模型和segSOT分别优化了流式和离线应用。

Conclusion: 该方法在流式和离线ASR中均取得显著改进，平衡了延迟与准确性，适用于多说话者场景。
...
### [260] [Complete Characterization for Adjustment in Summary Causal Graphs of Time Series](https://arxiv.org/abs/2506.14534)
*Clément Yvernes,Emilie Devijver,Eric Gaussier*

Main category: math.ST

TL;DR: 研究了时间序列中多干预下的因果效应可识别性问题，提出了调整准则的充要条件，并给出了一个伪线性算法来判断查询是否可识别。
...
Conclusion: 该方法为时间序列中多干预的因果效应识别提供了理论和算法基础。
...
TL;DR: BraTS orchestrator是一个开源Python包，旨在简化BraTS挑战赛中先进算法的使用，促进其在科研和临床中的应用。


<details>
  <summary>Details</summary>
Motivation: 尽管BraTS挑战赛在脑肿瘤图像分析方面取得了显著进展，但其算法在科学和临床领域的应用仍然有限。BraTS orchestrator旨在通过提供易于使用的工具包来加速这些算法的传播。

Method: 开发了一个开源Python包（BraTS orchestrator），集成了BraTS挑战赛中的先进分割和合成算法，并提供直观的教程。

Result: BraTS orchestrator成功简化了复杂深度学习技术的使用，使研究人员和临床医生能够轻松部署这些算法。

Conclusion: BraTS orchestrator通过降低技术门槛，促进了BraTS社区专业知识在神经放射学和神经肿瘤学中的广泛应用。
...
TL;DR: 论文提出了一种基于短波红外光谱的双模态AI框架，结合CNN和光二极管传感器，用于非侵入性血糖监测。
...
Method: 使用BioMedCLIP和ConvNeXt模型，评估了零样本分类、线性探测和微调三种学习场景。
...
Conclusion: orGAN显著提升了生成逼真注释数据集的效率、伦理性和成本效益，支持AI在手术中的广泛应用。
...
### [266] [NeuroCoreX: An Open-Source FPGA-Based Spiking Neural Network Emulator with On-Chip Learning](https://arxiv.org/abs/2506.14138)
*Ashish Gautam,Prasanna Date,Shruti Kulkarni,Robert Patton,Thomas Potok*

Main category: cs.NE

TL;DR: NeuroCoreX是一个基于FPGA的SNN仿真器，支持灵活的拓扑设计和测试，具有生物启发的学习机制和开源特性。


<details>
  <summary>Details</summary>
Motivation: 传统人工神经网络（ANNs）在能量效率和拓扑灵活性上存在局限，而SNNs因其事件驱动特性更适合高效计算。

Method: 开发了NeuroCoreX仿真器，支持全连接拓扑和STDP学习机制，使用LIF神经元模型和UART接口配置。

Result: NeuroCoreX实现了灵活的SNN设计和硬件执行，为生物启发的高效计算研究提供了工具。

Conclusion: NeuroCoreX作为开源框架，有望推动SNN在高效计算领域的研究和应用。
...
### [267] [A Scalable Hybrid Training Approach for Recurrent Spiking Neural Networks](https://arxiv.org/abs/2506.14464)
*Maximilian Baronig,Yeganeh Bahariasl,Ozan Özdenizci,Robert Legenstein*

Main category: cs.NE

TL;DR: HYPR是一种结合并行化和近似在线前向学习的高效算法，用于训练循环脉冲神经网络（RSNN），解决了BPTT的在线训练和内存限制问题。


<details>
  <summary>Details</summary>
Motivation: BPTT在训练RSNN时存在在线训练和内存消耗的局限性，而现有的前向梯度学习方法在性能和速度上表现不佳。

Method: 提出HYPR算法，结合并行化和近似在线前向学习，实现高吞吐量的在线学习，且内存需求与序列长度无关。

Result: HYPR在具有振荡亚阈值动态的脉冲神经元网络中表现出色，性能接近BPTT。

Conclusion: HYPR为RSNN提供了一种高效、内存友好的在线训练方法，特别适用于复杂神经元模型。
...
### [268] [Is Selection All You Need in Differential Evolution?](https://arxiv.org/abs/2506.14425)
*Tomofumi Kitamura,Alex Fukunaga*

Main category: cs.NE

TL;DR: 论文提出了一种名为Unbounded Differential Evolution (UDE)的新框架，解决了传统差分进化算法中因固定种群大小导致的多样性不足问题。UDE通过保留所有生成的候选个体，避免了复杂的存档管理和动态种群大小调整。


<details>
  <summary>Details</summary>
Motivation: 传统差分进化算法中，固定种群大小限制了种群多样性，而存档机制引入了额外的设计复杂性。UDE旨在通过取消替换机制和存档管理，简化算法并提升性能。

Method: UDE框架取消了传统差分进化算法中的替换机制，保留所有生成的候选个体，仅依赖选择机制进行优化。

Result: UDE提供了一种更简单但更强大的搜索算法，避免了传统方法中的存档管理和动态种群大小调整的复杂性。

Conclusion: UDE是一种全新的差分进化方法，通过取消替换机制和存档管理，简化了算法设计，同时提升了搜索能力。
...
### [269] [Refining music sample identification with a self-supervised graph neural network](https://arxiv.org/abs/2506.14684)
*Aditya Bhattacharjee,Ivan Meresman Higgs,Mark Sandler,Emmanouil Benetos*

Main category: cs.SD

TL;DR: 本文提出了一种轻量级且可扩展的编码架构，结合图神经网络和对比学习框架，用于自动样本识别（ASID）。该系统仅使用当前最先进系统9%的可训练参数，性能相当（mAP 44.2%），并引入两阶段检索方法提升检索质量。


<details>
  <summary>Details</summary>
Motivation: 自动样本识别（ASID）在音频检索中是一个重要但具有挑战性的任务，尤其是在面对音乐制作中的常见修改（如时间拉伸、音高变换等）时表现不佳。

Method: 提出了一种基于图神经网络和对比学习框架的轻量级编码架构，采用两阶段检索方法（粗粒度相似性搜索和交叉注意力分类器）提升检索质量。

Result: 系统仅使用9%的可训练参数，达到44.2%的mAP，性能与当前最先进系统相当。

Conclusion: 该方法在轻量化和性能上取得了平衡，并通过两阶段检索和短查询优化提升了实用性。
...
### [270] [A Survey on World Models Grounded in Acoustic Physical Information](https://arxiv.org/abs/2506.13833)
*Xiaoliang Chen,Le Chang,Xin Yu,Yunhe Huang,Xianling Tu*

Main category: cs.SD

TL;DR: 本文综述了基于声学物理信息的世界模型，涵盖理论、方法、技术进展及其在多个领域的应用，同时提出未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 探索声学信号如何编码物理信息，以支持高保真环境感知、因果推理和动态事件预测。

Method: 结合物理信息神经网络（PINNs）、生成模型和多模态自监督学习框架。

Result: 声学世界模型在机器人、自动驾驶、医疗和金融等领域有重要应用。

Conclusion: 未来研究需解决技术和伦理挑战，发展更鲁棒、因果、不确定性感知和负责任的声学智能。
...
### [271] [Acoustic scattering AI for non-invasive object classifications: A case study on hair assessment](https://arxiv.org/abs/2506.14148)
*Long-Vu Hoang,Tuan Nguyen,Tran Huy Dat*

Main category: cs.SD

TL;DR: 本文提出了一种基于声学散射的非侵入式物体分类方法，并通过头发评估案例展示了其效果。利用AI驱动的深度学习声音分类技术，实现了头发类型和湿度的分类。
...
Method: 通过发射声学刺激并捕捉头发样本的散射信号，采用四种深度学习策略（全监督学习、基于嵌入的分类、监督基础模型微调、自监督模型微调）进行分类。

Result: 最佳策略（自监督模型微调）实现了近90%的分类准确率。

Conclusion: 声学散射技术具有广泛的应用潜力，可作为隐私保护的非接触式分类方法。
...
### [272] [Pushing the Performance of Synthetic Speech Detection with Kolmogorov-Arnold Networks and Self-Supervised Learning Models](https://arxiv.org/abs/2506.14153)
*Tuan Dat Phuong,Long-Vu Hoang,Huy Dat Tran*

Main category: cs.SD

TL;DR: 论文提出了一种用Kolmogorov-Arnold Network（KAN）替代XLSR-Conformer模型中传统多层感知机的方法，显著提升了合成语音检测性能。
...
Result: 在ASVspoof2021数据集上，LA和DF集的性能相对提升60.55%，21LA集的EER达到0.70%。

Conclusion: 将KAN整合到基于自监督学习的模型中，是提升合成语音检测性能的有效方向。
...
### [273] [Fretting-Transformer: Encoder-Decoder Model for MIDI to Tablature Transcription](https://arxiv.org/abs/2506.14223)
*Anna Hamberger,Sebastian Murgul,Jochen Schmidt,Michael Heizmann*

Main category: cs.SD

TL;DR: Fretting-Transformer是一个基于T5架构的编码器-解码器模型，用于将MIDI序列自动转录为吉他指法谱，解决了弦-品模糊性和可演奏性问题。


<details>
  <summary>Details</summary>
Motivation: MIDI符号缺乏吉他演奏的关键信息，而现有方法在转录吉他指法谱时存在挑战，如弦-品模糊性和物理可演奏性。

Method: 使用T5变换器架构，将任务视为符号翻译问题，采用新颖的数据预处理和标记化策略，并整合上下文敏感处理和调弦/变调夹条件。

Result: Fretting-Transformer在DadaGP、GuitarToday和Leduc数据集上表现优于A*和Guitar Pro等基线方法。

Conclusion: 该模型为自动化吉他转录奠定了坚实基础，未来可通过进一步优化提升性能。
...
### [274] [Making deep neural networks work for medical audio: representation, compression and domain adaptation](https://arxiv.org/abs/2506.13970)
*Charles C Onu*

Main category: cs.SD

TL;DR: 论文探讨了机器学习在医疗音频信号分析中的应用，重点研究了婴儿哭声预测疾病的技术挑战和方法创新。


<details>
  <summary>Details</summary>
Motivation: 医疗音频信号（如肺音、心音和语音）包含重要健康信息，但当前主要依赖专家听觉分析。自动化分析可标准化处理、支持低资源环境筛查，并发现人类难以察觉的细微模式。

Method: 1. 利用成人语音数据通过迁移学习提升婴儿哭声分析模型；2. 提出端到端模型压缩方法，适用于资源受限设备；3. 提出音频模型领域适应技术；4. 发布开源婴儿哭声数据集。

Result: 开发了高精度、轻量化的婴儿哭声分析模型，解决了数据稀缺和设备资源限制问题，并提升了跨领域泛化能力。

Conclusion: 研究为将婴儿哭声作为生命体征奠定了基础，展示了AI音频监测在未来可及、低成本医疗中的潜力。
...
### [275] [SLEEPING-DISCO 9M: A large-scale pre-training dataset for generative music modeling](https://arxiv.org/abs/2506.14293)
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集多为合成或重录音乐，未能反映真实音乐风格，限制了生成音乐模型的应用。
...
### [276] [Unifying Streaming and Non-streaming Zipformer-based ASR](https://arxiv.org/abs/2506.14434)
*Bidisha Sharma,Karthik Pandia Durai,Shankar Venkatesan,Jeena J Prakash,Shashi Kumar,Malolan Chetlur,Andreas Stolcke*

Main category: cs.SD

TL;DR: 提出了一种统一流式和非流式ASR模型的框架，通过动态右上下文和分块注意力掩码训练zipformer模型，显著降低错误率并灵活控制延迟与准确性的权衡。


<details>
  <summary>Details</summary>
Motivation: 减少流式和非流式ASR模型的开发、训练和部署成本，实现统一框架。

Method: 使用动态右上下文和分块注意力掩码训练zipformer模型，分析不同右上下文帧数对准确性和延迟的影响。

Result: 在Librispeech和内部数据集上测试，相对降低7.9%的词错误率，流式性能接近非流式模型。

Conclusion: 该方法有效统一了流式和非流式ASR模型，提供了灵活的延迟-准确性权衡控制。
...
### [277] [Adaptive Accompaniment with ReaLchords](https://arxiv.org/abs/2506.14723)
...
TL;DR: ReaLchords是一种在线生成模型，用于即兴伴奏用户旋律，通过强化学习微调以实现实时协作。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型无法实时与其他音乐家协作，ReaLchords旨在填补这一空白。
...
### [278] [Exploring Speaker Diarization with Mixture of Experts](https://arxiv.org/abs/2506.14750)
*Gaobin Yang,Maokui He,Shutong Niu,Ruoyu Wang,Hang Chen,Jun Du*

Main category: cs.SD

TL;DR: 提出了一种基于记忆感知多说话人嵌入和序列到序列架构的神经说话人日志系统（NSD-MS2S），并进一步引入共享和软专家混合模块（SS-MoE）提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决说话人日志任务中模型偏差和性能不足的问题。

Method: 结合记忆感知多说话人嵌入模块和Seq2Seq架构，并引入SS-MoE模块。

Result: 在多个复杂数据集上取得最先进结果，展示了鲁棒性和泛化能力的提升。

Conclusion: 提出的方法在真实场景中表现出色，具有实际应用潜力。
...
### [279] [Accurate and scalable exchange-correlation with deep learning](https://arxiv.org/abs/2506.14665)
...
TL;DR: Skala是一种基于深度学习的交换相关（XC）泛函，通过直接从数据中学习表示，避免了手工设计特征的复杂性，实现了化学精度，同时保持了半局域DFT的计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有XC泛函依赖于手工设计的特征，难以同时实现高精度和计算效率，无法满足实验室实验的预测需求。

Method: 利用深度学习从大量高精度参考数据中学习XC泛函表示，数据来自计算密集的波函数方法。

Result: Skala在小分子原子化能上达到化学精度（误差低于1 kcal/mol），并在主族化学中与最佳混合泛函竞争。

Conclusion: Skala通过数据驱动方法显著提升了DFT的预测能力，随着数据量的增加，其性能有望进一步提升。
...
TL;DR: 论文提出了一种针对大型语言模型（LLMs）的新型威胁：通过对抗性输入利用模型的过度推理行为，显著增加计算开销而不影响模型性能。作者提出了一种包含三个损失组件的框架，实验结果显示推理长度增加了3到9倍。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，LLMs在复杂任务中表现优异，但存在因过度推理导致的高计算成本问题。作者发现对抗性输入可以进一步加剧这一问题。

Method: 提出了一种新的损失框架，包括优先级交叉熵损失、过度推理损失和延迟终止损失，旨在通过优化这些损失来延长推理过程。

Result: 在GSM8K和ORCA数据集上的实验表明，推理长度增加了3到9倍，同时保持了模型性能。对抗性输入还表现出对其他模型的迁移性。

Conclusion: 该研究揭示了对抗性输入对LLMs计算开销的潜在威胁，并提出了一种有效的攻击框架，为未来防御策略提供了参考。
...
### [281] [LLM-Powered Intent-Based Categorization of Phishing Emails](https://arxiv.org/abs/2506.14337)
...
<details>
  <summary>Details</summary>
Motivation: 钓鱼攻击对网络安全构成重大威胁，传统检测系统依赖用户不可见的元数据，而LLMs能通过文本意图识别钓鱼邮件。

Method: 研究使用LLMs对钓鱼邮件进行二元分类和意图分类，并构建了包含合法与钓鱼邮件的自定义数据集。

Result: 结果表明，现有LLMs能有效检测和分类钓鱼邮件。
...
### [282] [Busting the Paper Ballot: Voting Meets Adversarial Machine Learning](https://arxiv.org/abs/2506.14582)
*Kaleel Mahmood,Caleb Manicke,Ethan Rathbun,Aayushi Verma,Sohaib Ahmad,Nicholas Stamatakis,Laurent Michel,Benjamin Fuller*

Main category: cs.CR

TL;DR: 论文探讨了在美国选举计票器中使用机器学习分类器的安全风险，展示了对抗攻击可能对选举结果产生重大影响。


<details>
  <summary>Details</summary>
Motivation: 研究动机是揭示机器学习模型在选举计票中的潜在漏洞，尤其是对抗攻击对选举结果的威胁。

Method: 方法包括引入四个新数据集、训练多种模型（如SVM、CNN、ViT）、分析梯度掩蔽问题并提出解决方案，以及在物理世界中实施对抗攻击。

Result: 结果表明，传统白盒攻击因梯度掩蔽无效，但通过改进方法可在物理世界中实现5%攻击成功率，足以改变选举结果。

Conclusion: 结论指出，即使低成功率的对抗攻击也可能对选举安全构成严重威胁，需进一步研究防御措施。
...
### [283] [The Perception of Phase Intercept Distortion and its Application in Data Augmentation](https://arxiv.org/abs/2506.14571)
*Venkatakrishnan Vaidyanathapuram Krishnan,Nathaniel Condit-Schultz*

Main category: eess.SP

TL;DR: 论文研究了相位截断失真（一种频率无关的相位偏移），假设其虽显著改变信号波形但不可感知，并通过实验验证了假设。此外，探讨了其在机器学习数据增强中的应用，实验表明该方法能提升音频任务的性能。


<details>
  <summary>Details</summary>
Motivation: 研究相位截断失真的感知特性及其在机器学习中的潜在应用。

Method: 通过人类受试者实验验证相位截断失真的不可感知性，并将其应用于音频数据增强，进行多组实验。

Result: 实验证实相位截断失真不可感知，且作为数据增强方法能提升音频机器学习任务的性能。
...
### [284] [Projecting U.S. coastal storm surge risks and impacts with deep learning](https://arxiv.org/abs/2506.13963)
...
Method: 采用深度学习风暴潮模型，结合90万次合成热带气旋事件，考虑未来热带气旋行为和海平面变化。

Result: 历史100年风暴潮事件与观测数据吻合良好；本世纪末，热带气旋强度和海平面上升将使风险人口增加50%。

Conclusion: 研究揭示了佛罗里达州风险显著增加，并在乔治亚州和南卡罗来纳州识别出关键阈值。
...
### [285] [AI-Informed Model Analogs for Subseasonal-to-Seasonal Prediction](https://arxiv.org/abs/2506.14022)
*Jacob B. Landsberg,Elizabeth A. Barnes,Matthew Newman*

Main category: physics.ao-ph

TL;DR: 论文提出了一种基于可解释AI的模型类比预测方法，用于改进次季节到季节（S2S）预测，并在多个任务中优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 次季节到季节预测对公共卫生、灾害准备和农业至关重要，但预测难度较大。

Method: 使用人工神经网络学习权重掩码以优化类比选择，并在三个不同预测任务中验证其效果。

Result: AI-informed方法在确定性及概率性指标上优于传统方法，并能更好地预测极端温度和不确定性。
...
### [286] [Hard Contacts with Soft Gradients: Refining Differentiable Simulators for Learning and Control](https://arxiv.org/abs/2506.14186)
...
Method: 分析了惩罚模拟器的接触计算，提出DiffMJX（结合自适应积分与MuJoCo XLA）改进硬接触下的梯度质量；引入CFD机制，在不接触时也能生成有用梯度。

Result: DiffMJX显著提高了硬接触下的梯度质量；CFD在非接触情况下仍能提供信息性梯度。

Conclusion: DiffMJX和CFD方法有效解决了接触力梯度优化的问题，同时保持了物理真实性。
...
TL;DR: AMPLIFY框架利用大规模无动作视频数据，通过关键点轨迹生成紧凑的运动标记，分离视觉运动预测与动作推断，提升策略学习效果。
...
Method: AMPLIFY通过关键点轨迹生成运动标记，分别训练前向动态模型（无动作视频）和逆向动态模型（少量动作标注数据），实现独立扩展。

Result: 动态模型准确性显著提升（MSE提高3.7倍，像素预测精度提高2.5倍），策略学习效果提升1.2-2.2倍，首次实现零分布动作数据的LIBERO任务泛化。

Conclusion: AMPLIFY提供了一种利用异构数据构建高效、可泛化世界模型的新范式。
...
### [288] [Steering Robots with Inference-Time Interactions](https://arxiv.org/abs/2506.14287)
*Yanwei Wang*

Main category: cs.RO

TL;DR: 该研究提出了一种方法，允许用户在推理时通过交互指导预训练策略的行为，而无需微调模型。


<details>
  <summary>Details</summary>
Motivation: 预训练策略在部署时可能因泛化不足而出现错误，传统方法需要额外数据微调，效率低下。

Method: 提出两种框架：(1) 推理时引导，通过用户交互切换离散技能；(2) 任务和运动模仿，允许用户编辑连续动作以满足任务约束。

Result: 这些框架无需额外训练即可纠正策略预测偏差，提升预训练模型的实用性。

Conclusion: 通过用户交互指导行为生成，实现了高效且灵活的部署，避免了微调的需求。
...
### [289] [Uncertainty-Driven Radar-Inertial Fusion for Instantaneous 3D Ego-Velocity Estimation](https://arxiv.org/abs/2506.14294)
*Prashant Kumar Rai,Elham Kowsari,Nataliya Strokina,Reza Ghabcheloo*

Main category: cs.RO

TL;DR: 提出了一种结合高分辨率成像雷达和惯性测量单元的自主导航方法，通过神经网络处理雷达数据并估计瞬时线性速度及其不确定性，再与惯性数据融合，显著提高了运动估计的精度。


<details>
  <summary>Details</summary>
Motivation: 传统雷达运动估计方法存在局限性，需要更精确且鲁棒的技术。
...
### [290] [GAMORA: A Gesture Articulated Meta Operative Robotic Arm for Hazardous Material Handling in Containment-Level Environments](https://arxiv.org/abs/2506.14513)
*Farha Abdul Wasay,Mohammed Abdul Rahman,Hania Ghouse*

Main category: cs.RO

TL;DR: GAMORA是一种基于VR的机器人系统，用于远程执行高风险实验室任务，通过自然手势控制实现高精度操作。


<details>
  <summary>Details</summary>
Motivation: 随着生物危害复杂性增加，减少直接人类暴露并保持操作精度成为关键需求。

Method: 结合Oculus Quest 2、NVIDIA Jetson Nano和ROS，实现实时沉浸式控制、数字孪生模拟和逆运动学操作。

Result: 平均位置偏差2.2毫米，移液精度0.2毫升内，重复性1.2毫米，能耗降低50%。

Conclusion: GAMORA为生物医学研究提供了一种可扩展、沉浸式的机器人控制解决方案，确保生物安全和操作精度。
...
<details>
  <summary>Details</summary>
Motivation: 避免奖励工程，同时解决PbRL中反馈和样本效率低的问题。
...
TL;DR: Casper是一种辅助遥操作系统，利用预训练视觉语言模型的常识知识进行实时意图推断和灵活技能执行，适用于多样化任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法局限于简单预定义场景或任务特定数据分布，无法满足现实世界辅助需求。

Method: Casper结合开放世界感知模块、基于VLM的意图推断机制和技能库，支持多样化移动操作任务。

Result: 实验表明Casper提升任务性能、降低认知负荷，用户满意度高于直接遥操作和基线方法。
...
### [293] [Connecting phases of matter to the flatness of the loss landscape in analog variational quantum algorithms](https://arxiv.org/abs/2506.13865)
*Kasidit Srimahajariyapong,Supanut Thanasilp,Thiparat Chotibut*

Main category: quant-ph

TL;DR: 研究探讨了基于无序Ising链的模拟VQA ansätze，比较了热化相和MBL相的表现，发现MBL相在避免贫瘠高原方面表现更优，并提出了一种MBL初始化策略。
...
Method: 通过调节无序强度，将每个淬火置于热化相或MBL相，分析ansätze的表达能力和损失方差。

Result: 热化相和MBL相在大M时均达到最大表达能力，但热化相的贫瘠高原出现得更早。MBL初始化策略在中间淬火M时表现更优。

Conclusion: MBL相在VQA可训练性方面具有优势，为模拟硬件VQA的扩展提供了实用指导。
...
### [294] [Hamiltonian Formalism for Comparing Quantum and Classical Intelligence](https://arxiv.org/abs/2506.14456)
*Elija Perrier*

Main category: quant-ph

TL;DR: 论文提出了一种哈密顿形式主义，用于描述经典和量子AGI任务，以对比它们与环境的交互。


<details>
  <summary>Details</summary>
Motivation: 研究动机是开发数学框架，直接比较AGI在经典和量子环境中的运行方式。

Method: 方法是通过哈密顿形式主义分解AGI动态，生成核心功能（如归纳、推理、递归、学习、测量和记忆）的哈密顿生成器。

Result: 结果是为量子与经典代理通过环境交互的差异提供了精确的数学语言。

Conclusion: 结论是该形式主义有助于理解量子与经典AGI在环境交互中的区别。
...
### [295] [Evolutionary chemical learning in dimerization networks](https://arxiv.org/abs/2506.14006)
*Alexei V. Tkachenko,Bortolo Matteo Mognetti,Sergei Maslov*

Main category: cond-mat.stat-mech

TL;DR: 提出了一种基于竞争性二聚体网络（CDNs）的化学学习框架，通过体外定向进化实现复杂学习任务，如多类分类，无需数字硬件或显式参数调整。
...
Method: 利用DNA/RNA寡聚物等分子物种的可逆二聚化，通过突变、选择和扩增训练网络，结合亲和力作为可调突触权重。

Result: CDNs能有效区分噪声输入模式，输出对比度高，输入与输出间互信息强，性能与梯度下降训练相关。
...
### [296] [Non-planar Object Detection and Identification by Features Matching and Triangulation Growth](https://arxiv.org/abs/2506.13769)
*Filippo Leveni*

Main category: cs.CV

TL;DR: 提出了一种基于特征的模板检测与识别方法，通过增量分组特征匹配来应对场景图像中的变形模板。


<details>
  <summary>Details</summary>
Motivation: 解决传统几何模型（如单应性）在非平面或变形模板检测中的局限性。

Method: 利用Delaunay三角剖分作为图结构，通过局部一致性和几何/光度特性逐步评估特征匹配。

Result: 在变形较小的情况下性能与基于单应性的RANSAC相当，变形较大时表现更优。

Conclusion: 该方法在非平面或变形模板检测中具有优势，扩展了传统方法的适用性。
...
### [297] [Hidden Bias in the Machine: Stereotypes in Text-to-Image Models](https://arxiv.org/abs/2506.13780)
*Sedat Porikli,Vedat Porikli*

Main category: cs.CV

TL;DR: 论文研究了文本到图像（T2I）模型如何复制和放大社会偏见，通过生成16,000多张图像并分析其性别、种族等差异。


<details>
  <summary>Details</summary>
Motivation: 探讨T2I模型在生成图像时可能复制和强化社会偏见的问题。

Method: 使用Stable Diffusion 1.5和Flux-1模型生成16,000多张图像，并与8,000张Google搜索图像对比，分析人类相关因素的差异。

Result: 发现生成图像中存在显著的性别、种族、年龄等差异，这些差异强化了社会中的有害刻板印象。
...
### [298] [RadFabric: Agentic AI System with Reasoning Capability for Radiology](https://arxiv.org/abs/2506.14142)
...
TL;DR: RadFabric是一个多代理、多模态推理框架，通过结合视觉和文本分析提升CXR诊断的全面性和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前自动化系统在胸部X光（CXR）诊断中存在病理覆盖不足、准确性低及视觉与文本推理整合不足的问题。

Method: 基于模型上下文协议（MCP），RadFabric整合了病理检测代理、解剖解释代理和推理代理，利用多模态模型实现透明且基于证据的诊断。

Result: RadFabric在骨折检测中达到1.000准确率，整体诊断准确率（0.799）显著优于传统系统（0.229至0.527）。

Conclusion: RadFabric通过跨模态特征对齐和偏好驱动推理，推动了AI驱动的放射学向透明、解剖精确和临床可操作的CXR分析发展。
...
### [299] [Fake it till You Make it: Reward Modeling as Discriminative Prediction](https://arxiv.org/abs/2506.13846)
*Runtao Liu,Jiahao Zhan,Yingqing He,Chen Wei,Alan Yuille,Qifeng Chen*

Main category: cs.CV

TL;DR: 本文提出GAN-RM，一种高效的奖励建模框架，无需人工标注偏好数据或显式设计质量维度，通过对抗训练实现。
...
Method: 利用GAN的对抗训练思想，通过少量目标样本（Preference Proxy Data）与普通生成样本的判别训练奖励模型。

Result: 实验证明GAN-RM在多种应用中有效，如Best-of-N样本过滤、监督微调（SFT）和直接偏好优化（DPO）。

Conclusion: GAN-RM简化了奖励建模，减少了人工干预，适用于多种视觉生成模型的后训练增强。
...
### [300] [VisText-Mosquito: A Multimodal Dataset and Benchmark for AI-Based Mosquito Breeding Site Detection and Reasoning](https://arxiv.org/abs/2506.14629)
...
Method: 数据集包含1,828张标注图像用于目标检测，142张用于水面分割，以及每张图像关联的自然语言推理文本。采用YOLOv9s和YOLOv11n-Seg模型进行检测和分割，BLIP模型用于推理生成。

Result: YOLOv9s在目标检测中达到最高精度0.92926，mAP@50为0.92891；YOLOv11n-Seg的分割精度为0.91587，mAP@50为0.79795。BLIP模型的推理生成表现优异，BLEU得分54.7，BERTScore 0.91，ROUGE-L 0.87。

Conclusion: 该数据集和模型框架强调“预防胜于治疗”，展示了AI在主动应对蚊媒疾病风险中的潜力。数据集和代码已公开。
...
### [301] [ASCD: Attention-Steerable Contrastive Decoding for Reducing Hallucination in MLLM](https://arxiv.org/abs/2506.14766)
*Yujun Wang,Jinhe Bi,Yunpu Ma,Soeren Pirk*

Main category: cs.CV

TL;DR: 本文提出了一种基于注意力引导的对比解码框架，通过直接干预模型的注意力机制，有效减少多模态大语言模型（MLLM）的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法（如VCD和ICD）虽然能缓解幻觉问题，但其效果可能源于对注意力动态的深层影响，而非表面上的对数调整。因此，作者希望通过直接干预注意力机制，提出更根本的解决方案。

Method: 提出了一种注意力引导的对比解码框架，直接干预模型的注意力分布，以更原则化的方式减少幻觉。

Result: 实验表明，该方法在多个MLLM架构和不同解码方法中显著减少了幻觉，并在POPE、CHAIR和MMHal-Bench等基准测试中提升了性能，同时改善了标准VQA任务的性能。

Conclusion: 通过直接干预注意力机制，该方法为减少MLLM的幻觉问题提供了一种更有效和原则化的解决方案。
...
### [302] [Intelligent Image Sensing for Crime Analysis: A ML Approach towards Enhanced Violence Detection and Investigation](https://arxiv.org/abs/2506.13910)
*Aritra Dutta,Pushpita Boral,G Suseela*

Main category: cs.CV

TL;DR: 本文提出了一种基于机器学习的暴力检测与分类框架，结合3D卷积神经网络和双向LSTM，提升了计算资源效率和准确性。
...
Method: 使用3D卷积神经网络进行暴力检测，可分离卷积3D模型和双向LSTM进行分类，训练数据来自多样化标注数据集。

Result: 框架在计算资源效率和准确性方面表现优异。

Conclusion: 该方法为自动暴力检测提供了高效且准确的解决方案。
...
### [303] [HierVL: Semi-Supervised Segmentation leveraging Hierarchical Vision-Language Synergy with Dynamic Text-Spatial Query Alignment](https://arxiv.org/abs/2506.13925)
...
<details>
  <summary>Details</summary>
Motivation: 解决半监督语义分割在标签稀缺和领域变化下的挑战，结合视觉语言模型的语义鲁棒性和空间定位需求。

Method: 提出HierVL框架，包含分层语义查询生成器、跨模态空间对齐模块和双查询变换器解码器，并引入正则化损失。

Result: 在COCO、Pascal VOC、ADE20和Cityscapes数据集上分别提升4.4%、3.1%、5.9%和1.8%的mIoU。

Conclusion: 语言引导的分割填补了标签效率差距，实现了细粒度和实例感知的泛化。
...
### [304] [SimpleDoc: Multi-Modal Document Understanding with Dual-Cue Page Retrieval and Iterative Refinement](https://arxiv.org/abs/2506.14035)
*Chelsi Jain,Yiran Wu,Yifan Zeng,Jiale Liu,S hengyu Dai,Zhenwen Shao,Qingyun Wu,Huazheng Wang*

Main category: cs.CV

TL;DR: SimpleDoc是一个轻量级但强大的检索增强框架，用于DocVQA任务，通过双线索检索和迭代工作记忆提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决DocVQA任务中多模态信息（如图像和表格）和多页文档的挑战，提升检索和回答的效率和准确性。

Method: 采用双线索检索（嵌入相似性和页面摘要过滤与重排序），结合VLM推理代理迭代检索和回答问题。

Result: 在4个DocVQA数据集上平均性能提升3.2%，且检索页面更少。

Conclusion: SimpleDoc通过轻量级设计和高效检索机制，显著提升了DocVQA任务的性能。
...
### [305] [Image Segmentation with Large Language Models: A Survey with Perspectives for Intelligent Transportation Systems](https://arxiv.org/abs/2506.14096)
*Sanjeda Akter,Ibne Farabi Shihab,Anuj Sharma*

Main category: cs.CV

TL;DR: 综述探讨了大型语言模型（LLM）与计算机视觉结合在图像分割任务中的应用，特别是在智能交通系统（ITS）中的潜力与挑战。
...
Method: 系统回顾了LLM增强的图像分割方法，分类了基于提示机制和核心架构的当前方法。

Result: 这些创新可以提升自动驾驶、交通监控和基础设施维护中的道路场景理解。

Conclusion: 未来需解决实时性和安全性等挑战，并发展可解释、以人为本的AI技术以推动其在下一代交通系统中的成功应用。
...
### [306] [KDMOS:Knowledge Distillation for Motion Segmentation](https://arxiv.org/abs/2506.14130)
*Chunyu Cao,Jintao Cheng,Zeyu Chen,Linfan Zhan,Rui Fan,Zhijian He,Xiaoyu Tang*

Main category: cs.CV

TL;DR: 提出了一种基于logits的知识蒸馏框架KDMOS，用于运动目标分割（MOS），通过BEV投影模型（学生）和非投影模型（教师）结合，优化了精度与实时性的平衡。
...
Result: 在SemanticKITTI-MOS隐藏测试集上达到78.8%的IoU，参数数量减少7.69%，在Apollo数据集上表现竞争性。

Conclusion: KDMOS框架显著提升了MOS任务的精度与效率，解决了类别不平衡问题，并减少了过拟合。
...
### [307] [Mapping Farmed Landscapes from Remote Sensing](https://arxiv.org/abs/2506.13993)
...
Method: 使用深度学习分割模型，基于942个手动注释的航空影像图块训练，生成高分辨率地图。

Result: 模型准确识别关键栖息地，林地（96%）和农田（95%）的F1分数高，树篱（72%）的线性特征分割能力强。

Conclusion: Farmscapes为栖息地恢复和数据驱动规划提供工具，支持生物多样性战略监测，并为景观连通性分析奠定基础。
...
### [308] [SceneAware: Scene-Constrained Pedestrian Trajectory Prediction with LLM-Guided Walkability](https://arxiv.org/abs/2506.14144)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注行人间的社交互动，而忽略了环境背景对人类移动模式的重要影响。

Method: 使用Vision Transformer（ViT）编码场景信息，结合多模态大语言模型（MLLMs）生成可通行区域掩码，并通过Transformer编码轨迹，集成碰撞惩罚机制。

Result: 在ETH/UCY数据集上表现优于现有方法，提升超过50%，且在不同类型的行人运动中表现一致。

Conclusion: SceneAware证明了显式利用场景信息的有效性，能够生成准确且物理合理的预测。
...
### [309] [VideoMAR: Autoregressive Video Generatio with Continuous Tokens](https://arxiv.org/abs/2506.14168)
*Hu Yu,Biao Gong,Hangjie Yuan,DanDan Zheng,Weilong Chai,Jingdong Chen,Kecheng Zheng,Feng Zhao*

Main category: cs.CV

TL;DR: VideoMAR是一种高效的自回归图像到视频生成模型，结合了时间因果性和空间双向性，通过课程学习和渐进分辨率训练解决了长序列建模问题，性能优于现有方法。
...
Method: 提出VideoMAR模型，结合时间帧生成和空间掩码生成，采用课程学习、渐进分辨率训练和渐进温度策略。
...
### [310] [A multi-stage augmented multimodal interaction network for fish feeding intensity quantification](https://arxiv.org/abs/2506.14170)
*Shulong Zhang,Mingyuan Yao,Jiayin Zhao,Xiao Liu,Haihua Wang*

Main category: cs.CV

TL;DR: 该研究提出了一种多阶段增强多模态交互网络（MAINet），用于量化鱼类摄食强度，通过多模态数据融合和增强特征提取显著提高了模型的准确性和可靠性。


<details>
  <summary>Details</summary>
Motivation: 当前研究在多模态选择、特征提取与融合以及协同推理方面存在局限性，影响了多模态融合模型的准确性、适用性和可靠性。

Method: 提出了一种通用特征提取框架、辅助模态增强主模态机制（ARPM）以及证据推理（ER）规则，用于多模态数据融合和决策。

Result: MAINet在准确率、精确率、召回率和F1分数上均达到96.7%以上，性能显著优于对比模型。
...
### [311] [HRGS: Hierarchical Gaussian Splatting for Memory-Efficient High-Resolution 3D Reconstruction](https://arxiv.org/abs/2506.14229)
*Changbai Li,Haodong Zhu,Hanlin Chen,Juan Zhang,Tongfei Chen,Shuo Yang,Shuwei Shao,Wenhao Dong,Baochang Zhang*

Main category: cs.CV

TL;DR: HRGS是一种内存高效的层次化块级优化框架，用于解决3D高斯泼溅在高分辨率场景中的内存扩展问题。
...
Method: 首先生成低分辨率数据的全局粗高斯表示，然后将场景划分为多个块，并用高分辨率数据细化每个块。引入重要性驱动的高斯修剪（IDGP）和法线先验以优化计算和重建质量。
...
### [312] [FRIDU: Functional Map Refinement with Guided Image Diffusion](https://arxiv.org/abs/2506.14322)
*Avigail Cohen Rimon,Mirela Ben-Chen,Or Litany*

Main category: cs.CV

TL;DR: 提出了一种基于图像扩散模型的功能性映射优化方法，通过训练直接在功能性映射空间中生成准确映射，并在推理时利用点对点映射作为指导。


<details>
  <summary>Details</summary>
Motivation: 功能性映射通常表示为基变换矩阵，可以视为2D图像，但现有方法在优化映射时效率不高。

Method: 训练图像扩散模型直接在功能性映射空间中生成准确映射，推理时利用点对点映射作为指导，并鼓励正交性和与Laplace-Beltrami算子的交换性。

Result: 该方法在映射优化方面与最先进方法竞争，展示了扩散模型在功能性映射处理中的潜力。

Conclusion: 基于扩散模型的映射优化方法高效且有效，为功能性映射处理提供了新途径。
...
### [313] [MoTE: Mixture of Ternary Experts for Memory-efficient Large Multimodal Models](https://arxiv.org/abs/2506.14435)
*Hongyu Wang,Jiayu Xu,Ruiping Wang,Yan Feng,Yitao Zhai,Peng Pei,Xunliang Cai,Xilin Chen*

Main category: cs.CV

TL;DR: MoTE是一种内存高效的方法，通过训练更多低精度专家（三元参数{-1,0,1}）来替代高精度专家，显著降低内存占用，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 解决多模态混合专家模型（MoEs）在边缘设备上因高内存占用而难以部署的问题。

Method: 利用预训练的FFN作为共享专家，训练三元路由专家，结合后训练量化方法。

Result: MoTE在相同内存占用下性能与全精度基线相当，甚至在某些情况下表现更优（如3.4GB内存时提升4.3%准确率）。

Conclusion: MoTE是一种高效且可扩展的方法，适用于内存受限的设备。
...
### [314] [Model compression using knowledge distillation with integrated gradients](https://arxiv.org/abs/2506.14440)
*David E. Hernandez,Jose Chang,Torbjörn E. M. Nordling*

Main category: cs.CV

TL;DR: 提出了一种基于集成梯度（IG）的知识蒸馏方法，通过将IG图叠加到输入图像上，提升学生模型对教师模型决策的理解，显著提高了压缩模型的测试准确率。


<details>
  <summary>Details</summary>
Motivation: 在资源受限设备上部署深度学习模型需要高效的压缩方法，传统知识蒸馏方法在压缩率和准确性上仍有提升空间。

Method: 利用集成梯度（IG）作为数据增强策略，将IG图叠加到输入图像上，预计算IG图以减少训练时的运行时开销。

Result: 在CIFAR-10上达到92.6%的测试准确率，压缩率为4.1倍，比未蒸馏模型提升1.1个百分点（p<0.001），推理时间从140ms降至13ms。
...
### [315] [Foundation Model Insights and a Multi-Model Approach for Superior Fine-Grained One-shot Subset Selection](https://arxiv.org/abs/2506.14473)
...
Method: 通过实验比较FM和传统IE在子集选择上的表现，并提出RAM-APL方法，利用多个FMs的互补优势优化子集选择。

Result: FMs在细粒度数据集上表现优于传统IEs，但在粗粒度数据集上优势不明显。RAM-APL在多个细粒度数据集上达到最优性能。

Conclusion: FMs是子集选择的有力工具，尤其在细粒度数据集上。RAM-APL通过结合多个FMs的优势，进一步提升了性能。
...
### [316] [EVA02-AT: Egocentric Video-Language Understanding with Spatial-Temporal Rotary Positional Embeddings and Symmetric Optimization](https://arxiv.org/abs/2506.14356)
*Xiaoqi Wang,Yi Wang,Lap-Pui Chau*

Main category: cs.CV

TL;DR: EVA02-AT是一种基于EVA02的视频语言基础模型，通过单阶段预训练、空间-时间旋转位置嵌入和对称多相似性损失，高效解决了现有方法在预训练成本、时空编码和多实例检索中的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在预训练成本高、时空编码效果差和多实例检索学习目标不精确等方面存在问题，EVA02-AT旨在解决这些问题。

Method: 1. 单阶段预训练；2. 空间-时间旋转位置嵌入与联合注意力；3. 对称多相似性（SMS）损失。

Result: 在多个数据集上实现了最先进的性能，参数更少，多实例检索性能显著提升。
...
### [317] [DepthSeg: Depth prompting in remote sensing semantic segmentation](https://arxiv.org/abs/2506.14382)
*Ning Zhou,Shanxiong Chen,Mingting Zhou,Haigang Sui,Lieyun Hu,Han Li,Li Hua,Qiming Zhou*

Main category: cs.CV

TL;DR: 本文提出了一种名为DepthSeg的深度提示二维遥感语义分割框架，通过建模深度/高度信息解决光谱混淆和阴影遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 现有遥感语义分割方法主要关注光谱特征，忽略了目标的高程差异，导致复杂场景下的土地覆盖分类错误。

Method: DepthSeg框架包括轻量级适配器、深度提示器和语义分类解码器，分别用于特征提取、深度建模和语义预测。

Result: 在LiuZhou数据集上的实验验证了DepthSeg在土地覆盖制图任务中的优势，消融研究强调了深度提示的重要性。

Conclusion: DepthSeg通过整合深度信息显著提升了遥感语义分割的准确性，尤其在复杂场景中表现优异。
...
### [318] [Risk Estimation of Knee Osteoarthritis Progression via Predictive Multi-task Modelling from Efficient Diffusion Model using X-ray Images](https://arxiv.org/abs/2506.14560)
*David Butler,Adrian Hilton,Gustavo Carneiro*

Main category: cs.CV

TL;DR: 提出了一种可解释的机器学习方法，通过多任务预测建模估计膝关节骨关节炎（OA）进展风险，结合分类和预测解剖标志点，并利用扩散模型生成高质量未来图像。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏可解释性且复杂，临床采用受限，且无法定位解剖标志点。

Method: 采用多任务预测建模，结合分类未来OA严重程度和预测解剖标志点，利用扩散模型在类别条件潜在空间中生成未来图像。

Result: 在Osteoarthritis Initiative数据集上，AUC提升2%至0.71，推理时间缩短约9%。

Conclusion: 新方法在提升预测性能的同时增强了可解释性，为临床决策提供了可视化支持。
...
### [319] [Decoupled Classifier-Free Guidance for Counterfactual Diffusion Models](https://arxiv.org/abs/2506.14399)
...
<details>
  <summary>Details</summary>
Motivation: 标准无分类器引导（CFG）在反事实图像生成中可能导致身份保持不佳和虚假属性变化（属性放大），需要一种更灵活的方法来改进。

Method: 提出DCFG框架，采用属性分割嵌入策略，将属性分为干预组和不变组，分别应用不同的引导。
...
### [320] [Causally Steered Diffusion for Automated Video Counterfactual Generation](https://arxiv.org/abs/2506.14404)
...
<details>
  <summary>Details</summary>
Motivation: 解决视频编辑中因果依赖关系被忽视导致不真实结果的问题。

Method: 利用视觉语言模型（VLM）指导文本提示优化，基于假设的因果图控制潜在扩散模型（LDM）。

Result: 通过标准视频质量指标和反事实特定标准（如因果有效性和最小性）验证了方法的有效性。

Conclusion: 该方法兼容任何黑盒视频编辑系统，具有在医疗和数字媒体等领域生成真实反事实视频的潜力。
...
### [321] [Align Your Flow: Scaling Continuous-Time Flow Map Distillation](https://arxiv.org/abs/2506.14603)
*Amirmojtaba Sabour,Sanja Fidler,Karsten Kreis*

Main category: cs.CV

TL;DR: 论文提出了一种名为Align Your Flow的流映射模型，通过新的连续时间目标和训练技术，改进了现有一致性模型和流匹配目标，实现了高效的少步生成，并在图像生成任务中取得了领先性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散和流的生成模型需要多步采样，而一致性模型在增加步数时性能下降。流映射通过连接任意两个噪声级别，解决了这一问题。

Method: 提出了两种新的连续时间训练目标及训练技术，结合自动引导和对抗微调，提升了模型性能。

Result: 在ImageNet 64x64和512x512上实现了少步生成的领先性能，并在文本到图像任务中优于现有非对抗训练模型。

Conclusion: 流映射模型在高效生成和性能上具有显著优势，为少步生成任务提供了新的解决方案。
...
### [322] [Unsupervised Imaging Inverse Problems with Diffusion Distribution Matching](https://arxiv.org/abs/2506.14605)
*Giacomo Meanti,Thomas Ryckeboer,Michael Arbel,Julien Mairal*

Main category: cs.CV

TL;DR: 提出了一种基于非配对数据集的图像恢复方法，通过条件流匹配和分布匹配损失，适用于真实场景中前向模型未知或数据配对困难的情况。


<details>
  <summary>Details</summary>
Motivation: 传统方法通常需要完整的前向模型或配对数据，但在真实场景中这些条件难以满足。本文旨在解决这一问题。
...
### [323] [Compositional Attribute Imbalance in Vision Datasets](https://arxiv.org/abs/2506.14418)
*Jiayi Chen,Yanbiao Ma,Andi Zhang,Weidong Tang,Wei Dai,Bowei Liu*

Main category: cs.CV

TL;DR: 本文提出了一种基于CLIP的框架，用于解决图像分类中的视觉属性不平衡问题，通过调整样本采样概率和结合数据增强技术，显著提升了模型的鲁棒性和公平性。
...
Method: 定义图像的一级和二级属性，构建视觉属性字典，分析单属性和组合属性不平衡，调整样本采样概率并结合多种数据增强技术。
...
### [324] [Adapting Lightweight Vision Language Models for Radiological Visual Question Answering](https://arxiv.org/abs/2506.14451)
*Aditya Shourya,Michel Dumontier,Chang Sun*

Main category: cs.CV

TL;DR: 该研究通过微调轻量级视觉语言模型，解决了放射学视觉问答（VQA）中的数据、建模和评估挑战，展示了小模型在适当调优后的高性能表现。


<details>
  <summary>Details</summary>
Motivation: 放射学VQA模型面临数据获取困难、图像模式复杂以及评估不足的挑战，需要一种高效且成本可控的解决方案。

Method: 采用轻量级3B参数视觉语言模型，通过合成问答对生成和多阶段微调，结合放射学专用数据集（如ROCO v2.0、MedPix v2.0）进行训练。

Result: 尽管模型规模远小于当前最优模型（如LLaVA-Med），但在开放和封闭式问题上均表现出色，且训练数据规模有限。

Conclusion: 研究表明，小模型通过精心调优和专用数据训练，可以在放射学VQA中实现高性能，同时提供轻量级诊断工具以识别模型失败模式。
...
### [325] [Cost-Aware Routing for Efficient Text-To-Image Generation](https://arxiv.org/abs/2506.14753)
*Qinchan,Li,Kenneth Chen,Changyue,Su,Wittawat Jitkrittum,Qi Sun,Patsorn Sangkloy*

Main category: cs.CV

TL;DR: 提出了一种根据提示复杂度动态调整计算成本的框架，通过自动路由到不同生成模型或步骤数，优化质量与计算成本的平衡。


<details>
  <summary>Details</summary>
Motivation: 扩散模型生成高质量图像但计算成本高，需平衡质量与效率。

Method: 自动路由提示到最适合的文本到图像生成函数，可能是不同步骤数的扩散模型或其他独立模型。

Result: 在COCO和DiffusionDB上验证，路由到九个预训练模型后，平均质量高于单一模型。

Conclusion: 通过动态路由，实现了质量与计算成本的最优权衡。
...
### [326] [Synthetic Data Augmentation for Table Detection: Re-evaluating TableNet's Performance with Automatically Generated Document Images](https://arxiv.org/abs/2506.14583)
*Krishna Sahukara,Zineddine Bettouche,Andreas Fischer*

Main category: cs.CV

TL;DR: 本文提出了一种自动化LaTeX管道，用于生成多样化的表格布局和真实掩码，以增强真实数据集Marmot，并优化TableNet的性能。


<details>
  <summary>Details</summary>
Motivation: 手动提取文档中的表格耗时且易错，需要自动化解决方案。

Method: 使用LaTeX生成多样化的两栏页面和真实掩码，合成数据集用于训练TableNet。
...
### [327] [PoseGRAF: Geometric-Reinforced Adaptive Fusion for Monocular 3D Human Pose Estimation](https://arxiv.org/abs/2506.14596)
*Ming Xu,Xu Zhang*

Main category: cs.CV

TL;DR: PoseGRAF框架通过双图卷积结构和跨注意力模块，有效捕捉骨骼和关节的局部依赖关系，解决了现有单目3D姿态估计方法在关节遮挡或快速运动变化时的问题。
...
Method: 提出PoseGRAF框架，包括双图卷积结构、跨注意力模块和动态融合模块，结合改进的Transformer编码器生成最终输出。
